...
SEH	R2, R5
SLTI	R2, R2, 21
BNE	R2, R0, L__Str_Initialize350
NOP	
J	L_Str_Initialize1
NOP	
L__Str_Initialize350:
...
BEQ	R2, R0, L__Sample_Ringbuffer355
NOP	
...
SEH	R2, R5
BEQ	R2, R0, L__Sample_Ringbuffer356
NOP	
...
SEH	R2, R5
SLTI	R2, R2, 1
BEQ	R2, R0, L__Sample_Ringbuffer361
NOP	
...
ORI	R2, R0, 63
BEQ	R3, R2, L__Sample_Ringbuffer362
NOP	
...
LBU	R3, Offset(_gcode+0)(GP)
ORI	R2, R0, 36
BEQ	R3, R2, L__Sample_Ringbuffer365
NOP	
...
LH	R2, Offset(Sample_Ringbuffer_str_len_L0+0)(GP)
SLTI	R2, R2, 2
BNE	R2, R0, L__Sample_Ringbuffer366
NOP	
...
SEH	R2, R3
SLTI	R2, R2, 4
BNE	R2, R0, L__Sample_Ringbuffer378
NOP	
...
;Protocol.c,243 :: 		report_status_message(STATUS_SETTING_READ_FAIL);
SH	R3, 20(SP)
ORI	R25, R0, 10
...
;Protocol.c,254 :: 		query = 1; //report status is ok continue
ORI	R2, R0, 1
SH	R2, Offset(Sample_Ringbuffer_query_L0+0)(GP)
;Protocol.c,255 :: 		break;
J	L_Sample_Ringbuffer19
NOP	
;Protocol.c,256 :: 		}else { // Store startup line
L_Sample_Ringbuffer43:
;Protocol.c,257 :: 		int N_Val = 0;
;Protocol.c,258 :: 		helper_var = 1;  // Set helper_var to flag storing method.
; helper_var start address is: 28 (R7)
ORI	R7, R0, 1
;Protocol.c,263 :: 		if ( gcode[0][2] >= '0'  &&  gcode[0][2] <= '9' ) {
LBU	R2, Offset(_gcode+2)(GP)
SLTIU	R2, R2, 48
...
;Protocol.c,264 :: 		char num[] = "0";
ORI	R30, R0, 48
SB	R30, 113(SP)
MOVZ	R30, R0, R0
SB	R30, 114(SP)
;Protocol.c,266 :: 		num[0] = gcode[0][2];
ADDIU	R3, SP, 113
LBU	R2, Offset(_gcode+2)(GP)
SB	R2, 0(R3)
;Protocol.c,267 :: 		N_Val = atoi(num);
MOVZ	R25, R3, R0
JAL	_atoi+0
NOP	
...
;Protocol.c,275 :: 		query = 2; //STATUS_BAD_NUMBER_FORMAT;
ORI	R2, R0, 2
SH	R2, Offset(Sample_Ringbuffer_query_L0+0)(GP)
;Protocol.c,276 :: 		break;
J	L_Sample_Ringbuffer19
NOP	
;Protocol.c,277 :: 		}
L_Sample_Ringbuffer53:
;Protocol.c,280 :: 		if (helper_var) { // Store startup line
; N_Val start address is: 16 (R4)
; helper_var start address is: 28 (R7)
BNE	R7, R0, L__Sample_Ringbuffer388
NOP	
J	L_Sample_Ringbuffer54
NOP	
L__Sample_Ringbuffer388:
; helper_var end address is: 28 (R7)
;Protocol.c,281 :: 		if(gcode[0][3] != '='){
...
; N_Val end address is: 16 (R4)
;Protocol.c,284 :: 		helper_var = strlen((gcode[0]));
LUI	R25, hi_addr(_gcode+0)
ORI	R25, R25, lo_addr(_gcode+0)
JAL	_strlen+0
NOP	
;Protocol.c,285 :: 		strncpy(str,(gcode[0]),helper_var);
ADDIU	R3, SP, 34
SEH	R27, R2
LUI	R26, hi_addr(_gcode+0)
ORI	R26, R26, lo_addr(_gcode+0)
...
JAL	_strncpy+0
NOP	
;Protocol.c,293 :: 		str_len = strlen(str);
ADDIU	R2, SP, 34
MOVZ	R25, R2, R0
JAL	_strlen+0
NOP	
...
;Protocol.c,299 :: 		int str_length = 0;
; N_Val start address is: 16 (R4)
;Protocol.c,307 :: 		settings_store_startup_line(N_Val,str+4);
ADDIU	R2, SP, 34
ADDIU	R2, R2, 4
MOVZ	R26, R2, R0
SEH	R25, R4
...
NOP	
;Protocol.c,308 :: 		query = 1; //noneed to send erro report
ORI	R2, R0, 1
SH	R2, Offset(Sample_Ringbuffer_query_L0+0)(GP)
;Protocol.c,309 :: 		}
...
J	L_Sample_Ringbuffer19
NOP	
;Protocol.c,314 :: 		case '~': //*~ (cycle start)
L_Sample_Ringbuffer57:
;Protocol.c,315 :: 		sys.execute |= EXEC_CYCLE_START;
LH	R2, Offset(_sys+26)(GP)
ORI	R2, R2, 2
SH	R2, Offset(_sys+26)(GP)
...
J	L_Sample_Ringbuffer19
NOP	
;Protocol.c,324 :: 		case '0': case '1': case '2': case'3':  case '4': case '5':
...
;Protocol.c,330 :: 		char str_val[9]={0};
ADDIU	R23, SP, 115
ADDIU	R22, R23, 9
...
JAL	___CC2DW+0
NOP	
;Protocol.c,331 :: 		int N_Val = 0;
; N_Val start address is: 24 (R6)
MOVZ	R6, R0, R0
;Protocol.c,332 :: 		float value = 0.00;
; value start address is: 8 (R2)
MTC1	R0, S1
;Protocol.c,333 :: 		if(gcode[0][2] == '='){
LBU	R3, Offset(_gcode+2)(GP)
ORI	R2, R0, 61
BEQ	R3, R2, L__Sample_Ringbuffer395
NOP	
J	L_Sample_Ringbuffer73
NOP	
L__Sample_Ringbuffer395:
;Protocol.c,335 :: 		strncpy(str_val,gcode[0]+1,1);
ADDIU	R2, SP, 115
ORI	R27, R0, 1
...
;Protocol.c,337 :: 		strncpy(str_val,gcode[0]+3,strlen(gcode[0]+3));
; N_Val start address is: 24 (R6)
...
JAL	_strncpy+0
NOP	
;Protocol.c,338 :: 		}else if(gcode[0][3] == '='){
SEH	R7, R6
...
;Protocol.c,340 :: 		strncpy(str_val,gcode[0]+1,2);
ADDIU	R2, SP, 115
ORI	R27, R0, 2
LUI	R26, hi_addr(_gcode+1)
ORI	R26, R26, lo_addr(_gcode+1)
...
JAL	_strncpy+0
NOP	
;Protocol.c,341 :: 		if(isdigit(str_val[0])){N_Val = atoi(str_val);}
ADDIU	R2, SP, 115
LBU	R25, 0(R2)
JAL	_isdigit+0
NOP	
...
;Protocol.c,342 :: 		memset(str_val,0,9);
; N_Val start address is: 20 (R5)
ADDIU	R2, SP, 115
ORI	R27, R0, 9
MOVZ	R26, R0, R0
...
JAL	_memset+0
NOP	
;Protocol.c,343 :: 		strncpy(str_val,gcode[0]+4,strlen(gcode[0]+4));
...
;Protocol.c,347 :: 		if((value < 0.0) || (!isdigit(str_val[0]))){
; N_Val start address is: 28 (R7)
MOVZ	R2, R0, R0
MTC1	R2, S0
C.LT.S 	0, S1, S0
BC1F	0, L__Sample_Ringbuffer401
NOP	
...
;Protocol.c,348 :: 		return(STATUS_UNSUPPORTED_STATEMENT);
ORI	R2, R0, 3
J	L_end_Sample_Ringbuffer
NOP	
;Protocol.c,349 :: 		}
L_Sample_Ringbuffer80:
;Protocol.c,350 :: 		value = atof(str_val);
; N_Val start address is: 28 (R7)
ADDIU	R2, SP, 115
MOVZ	R25, R2, R0
JAL	_atof+0
NOP	
;Protocol.c,355 :: 		settings_store_global_setting(N_Val,value);
MOV.S 	S12, S0
SEH	R25, R7
; N_Val end address is: 28 (R7)
JAL	_settings_store_global_setting+0
NOP	
;Protocol.c,358 :: 		query = 1; //noneed to send erro report
ORI	R2, R0, 1
SH	R2, Offset(Sample_Ringbuffer_query_L0+0)(GP)
...
;Protocol.c,366 :: 		}else if((*(*gcode+0)+0)>64 && (*(*gcode+0)+0)<91){//[A ... Z]
...
SEH	R2, R2
SLTI	R2, R2, 65
BEQ	R2, R0, L__Sample_Ringbuffer446
NOP	
...
;Protocol.c,371 :: 		i = cpy_val_from_str(temp,(*(gcode+0)),1,strlen(*(gcode+0)));
LUI	R25, hi_addr(_gcode+0)
ORI	R25, R25, lo_addr(_gcode+0)
JAL	_strlen+0
NOP	
...
;Protocol.c,373 :: 		G_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
...
SEH	R4, R2
;Protocol.c,376 :: 		if(G_Val == 28 || G_Val == 30 || G_Val == 92)
SEH	R3, R2
ORI	R2, R0, 28
...
;Protocol.c,380 :: 		G_Val = (int)(atof(temp)*10.0);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atof+0
NOP	
...
;Protocol.c,383 :: 		motion_mode = G_Mode(G_Val);
; G_Val start address is: 8 (R2)
SEH	R25, R2
; G_Val end address is: 8 (R2)
JAL	_G_Mode+0
...
JAL	_G_Mode+0
NOP	
;Protocol.c,416 :: 		break;
J	L_Sample_Ringbuffer97
NOP	
;Protocol.c,417 :: 		case 'X':case 'x':
L_Sample_Ringbuffer105:
L_Sample_Ringbuffer106:
;Protocol.c,418 :: 		case 'Y':case 'y':
L_Sample_Ringbuffer107:
L_Sample_Ringbuffer108:
;Protocol.c,419 :: 		case 'Z':case 'z':
L_Sample_Ringbuffer109:
L_Sample_Ringbuffer110:
;Protocol.c,420 :: 		case 'A':case 'a':
L_Sample_Ringbuffer111:
L_Sample_Ringbuffer112:
;Protocol.c,421 :: 		XYZ_Val = atof(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atof+0
NOP	
SWC1	S0, 24(SP)
;Protocol.c,422 :: 		status = Instruction_Values(gcode[1],&XYZ_Val);
ADDIU	R2, SP, 24
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+64)
ORI	R25, R25, lo_addr(_gcode+64)
...
J	L_Sample_Ringbuffer97
NOP	
;Protocol.c,427 :: 		case 'L':case 'l':
L_Sample_Ringbuffer113:
L_Sample_Ringbuffer114:
;Protocol.c,428 :: 		O_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
SH	R2, 32(SP)
;Protocol.c,429 :: 		status = Instruction_Values(gcode[1],&O_Val);
ADDIU	R2, SP, 32
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+64)
ORI	R25, R25, lo_addr(_gcode+64)
...
NOP	
SH	R2, 32(SP)
;Protocol.c,436 :: 		status = Instruction_Values(gcode[1],&O_Val);
ADDIU	R2, SP, 32
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+64)
ORI	R25, R25, lo_addr(_gcode+64)
...
NOP	
SH	R2, 32(SP)
;Protocol.c,443 :: 		status = Instruction_Values(gcode[1],&O_Val);
ADDIU	R2, SP, 32
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+64)
ORI	R25, R25, lo_addr(_gcode+64)
...
;Protocol.c,462 :: 		i = cpy_val_from_str(temp,(*(gcode+2)),1,strlen(*(gcode+2)));
...
; i start address is: 16 (R4)
SEH	R4, R2
;Protocol.c,463 :: 		switch(*(*(gcode+2)+0)) {
J	L_Sample_Ringbuffer122
NOP	
;Protocol.c,464 :: 		case 'G':case 'g':
L_Sample_Ringbuffer124:
L_Sample_Ringbuffer125:
;Protocol.c,465 :: 		if(i < 3){ //G00 - G99
SEH	R2, R4
; i end address is: 16 (R4)
SLTI	R2, R2, 3
...
;Protocol.c,466 :: 		G_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
; G_Val start address is: 16 (R4)
SEH	R4, R2
;Protocol.c,469 :: 		if(G_Val == 28 || G_Val == 30 || G_Val == 92)
SEH	R3, R2
ORI	R2, R0, 28
...
;Protocol.c,473 :: 		G_Val = (int)(atof(temp)*10.0);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atof+0
NOP	
...
;Protocol.c,476 :: 		motion_mode = G_Mode(G_Val);
; G_Val start address is: 8 (R2)
SEH	R25, R2
; G_Val end address is: 8 (R2)
JAL	_G_Mode+0
NOP	
;Protocol.c,480 :: 		break;
J	L_Sample_Ringbuffer123
NOP	
;Protocol.c,481 :: 		case 'X':case 'x':
L_Sample_Ringbuffer131:
L_Sample_Ringbuffer132:
;Protocol.c,482 :: 		case 'Y':case 'y':
L_Sample_Ringbuffer133:
L_Sample_Ringbuffer134:
;Protocol.c,483 :: 		case 'Z':case 'z':
L_Sample_Ringbuffer135:
L_Sample_Ringbuffer136:
;Protocol.c,484 :: 		case 'A':case 'a':
L_Sample_Ringbuffer137:
L_Sample_Ringbuffer138:
;Protocol.c,486 :: 		XYZ_Val = atof(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atof+0
NOP	
SWC1	S0, 24(SP)
;Protocol.c,487 :: 		status = Instruction_Values(gcode[2],&XYZ_Val);
ADDIU	R2, SP, 24
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+128)
ORI	R25, R25, lo_addr(_gcode+128)
...
NOP	
SH	R2, 28(SP)
;Protocol.c,491 :: 		break;
J	L_Sample_Ringbuffer123
NOP	
;Protocol.c,492 :: 		case 'L':case 'l':
L_Sample_Ringbuffer139:
L_Sample_Ringbuffer140:
;Protocol.c,493 :: 		O_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
SH	R2, 32(SP)
;Protocol.c,494 :: 		status = Instruction_Values(gcode[2],&O_Val);
ADDIU	R2, SP, 32
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+128)
ORI	R25, R25, lo_addr(_gcode+128)
...
J	L_Sample_Ringbuffer123
NOP	
;Protocol.c,499 :: 		case 'F':case 'f':
L_Sample_Ringbuffer141:
L_Sample_Ringbuffer142:
;Protocol.c,500 :: 		O_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
SH	R2, 32(SP)
;Protocol.c,501 :: 		status = Instruction_Values(gcode[2],&O_Val);
ADDIU	R2, SP, 32
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+128)
ORI	R25, R25, lo_addr(_gcode+128)
...
J	L_Sample_Ringbuffer123
NOP	
;Protocol.c,506 :: 		case 'P':case 'p':
L_Sample_Ringbuffer143:
L_Sample_Ringbuffer144:
;Protocol.c,507 :: 		O_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
SH	R2, 32(SP)
;Protocol.c,508 :: 		status = Instruction_Values(gcode[2],&O_Val);
ADDIU	R2, SP, 32
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+128)
ORI	R25, R25, lo_addr(_gcode+128)
...
NOP	
SH	R2, 28(SP)
;Protocol.c,512 :: 		break;
J	L_Sample_Ringbuffer123
NOP	
;Protocol.c,513 :: 		case 'S':case 's':
L_Sample_Ringbuffer145:
L_Sample_Ringbuffer146:
;Protocol.c,514 :: 		O_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
SH	R2, 32(SP)
;Protocol.c,515 :: 		status = Instruction_Values(gcode[2],&O_Val);
ADDIU	R2, SP, 32
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+128)
ORI	R25, R25, lo_addr(_gcode+128)
...
;Protocol.c,525 :: 		if(*(*(gcode+3)+0) != 0){
LBU	R2, Offset(_gcode+192)(GP)
BNE	R2, R0, L__Sample_Ringbuffer547
NOP	
J	L_Sample_Ringbuffer147
NOP	
L__Sample_Ringbuffer547:
;Protocol.c,526 :: 		i = cpy_val_from_str(temp,(*(gcode+3)),1,strlen(*(gcode+3)));
...
; i start address is: 16 (R4)
SEH	R4, R2
;Protocol.c,527 :: 		switch(*(*(gcode+3)+0)) {
J	L_Sample_Ringbuffer148
NOP	
;Protocol.c,528 :: 		case 'G':case 'g':
L_Sample_Ringbuffer150:
L_Sample_Ringbuffer151:
;Protocol.c,529 :: 		if(i < 3){ //G00 - G99
SEH	R2, R4
; i end address is: 16 (R4)
SLTI	R2, R2, 3
...
;Protocol.c,530 :: 		G_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
; G_Val start address is: 16 (R4)
SEH	R4, R2
;Protocol.c,533 :: 		if(G_Val == 28 || G_Val == 30 || G_Val == 92)
SEH	R3, R2
ORI	R2, R0, 28
...
;Protocol.c,537 :: 		G_Val = (int)(atof(temp)*10.0);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atof+0
NOP	
...
;Protocol.c,540 :: 		motion_mode = G_Mode(G_Val);
; G_Val start address is: 8 (R2)
SEH	R25, R2
; G_Val end address is: 8 (R2)
JAL	_G_Mode+0
...
NOP	
SWC1	S0, 24(SP)
;Protocol.c,550 :: 		status = Instruction_Values(gcode[3],&XYZ_Val);
ADDIU	R2, SP, 24
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+192)
ORI	R25, R25, lo_addr(_gcode+192)
...
J	L_Sample_Ringbuffer149
NOP	
;Protocol.c,555 :: 		case 'F': case 'f':
L_Sample_Ringbuffer167:
L_Sample_Ringbuffer168:
;Protocol.c,556 :: 		O_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
SH	R2, 32(SP)
;Protocol.c,557 :: 		status = Instruction_Values(gcode[3],&O_Val);
ADDIU	R2, SP, 32
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+192)
ORI	R25, R25, lo_addr(_gcode+192)
...
;Protocol.c,568 :: 		if(*(*(gcode+4)+0) != 0){
LBU	R2, Offset(_gcode+256)(GP)
BNE	R2, R0, L__Sample_Ringbuffer584
NOP	
J	L_Sample_Ringbuffer169
NOP	
L__Sample_Ringbuffer584:
;Protocol.c,569 :: 		i = cpy_val_from_str(temp,(*(gcode+4)),1,strlen(*(gcode+4)));
...
JAL	Protocol_cpy_val_from_str+0
NOP	
;Protocol.c,570 :: 		switch(*(*(gcode+4))) {
J	L_Sample_Ringbuffer170
NOP	
;Protocol.c,571 :: 		case 'Y':case 'y':
L_Sample_Ringbuffer172:
L_Sample_Ringbuffer173:
;Protocol.c,572 :: 		case 'Z':case 'z':
L_Sample_Ringbuffer174:
L_Sample_Ringbuffer175:
;Protocol.c,573 :: 		case 'J':case 'j':
L_Sample_Ringbuffer176:
L_Sample_Ringbuffer177:
;Protocol.c,574 :: 		XYZ_Val = atof(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atof+0
NOP	
SWC1	S0, 24(SP)
;Protocol.c,575 :: 		status = Instruction_Values(gcode[4],&XYZ_Val);
ADDIU	R2, SP, 24
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+256)
ORI	R25, R25, lo_addr(_gcode+256)
...
NOP	
;Protocol.c,580 :: 		case 'F':case 'f':
L_Sample_Ringbuffer178:
L_Sample_Ringbuffer179:
;Protocol.c,581 :: 		O_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
SH	R2, 32(SP)
;Protocol.c,582 :: 		status = Instruction_Values(gcode[4],&O_Val);
ADDIU	R2, SP, 32
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+256)
ORI	R25, R25, lo_addr(_gcode+256)
...
;Protocol.c,593 :: 		if(*(*(gcode+5)+0) != 0){
LBU	R2, Offset(_gcode+320)(GP)
BNE	R2, R0, L__Sample_Ringbuffer602
NOP	
J	L_Sample_Ringbuffer180
NOP	
L__Sample_Ringbuffer602:
;Protocol.c,594 :: 		xyz[4] = *(*(gcode+5)+0);no_of_axis++;
ADDIU	R2, SP, 107
ADDIU	R3, R2, 4
LBU	R2, Offset(_gcode+320)(GP)
SB	R2, 0(R3)
;Protocol.c,595 :: 		i = cpy_val_from_str(temp,(*(gcode+5)),1,strlen(*(gcode+5)));
...
JAL	Protocol_cpy_val_from_str+0
NOP	
;Protocol.c,596 :: 		switch(*(*(gcode+5))) {
J	L_Sample_Ringbuffer181
NOP	
;Protocol.c,597 :: 		case 'Z':case 'z':
L_Sample_Ringbuffer183:
L_Sample_Ringbuffer184:
;Protocol.c,598 :: 		case 'J':case 'j':
L_Sample_Ringbuffer185:
L_Sample_Ringbuffer186:
;Protocol.c,599 :: 		XYZ_Val = atof(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atof+0
NOP	
SWC1	S0, 24(SP)
;Protocol.c,600 :: 		Instruction_Values(gcode[5],&XYZ_Val);
ADDIU	R2, SP, 24
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+320)
ORI	R25, R25, lo_addr(_gcode+320)
...
NOP	
;Protocol.c,605 :: 		case 'F':case 'f':
L_Sample_Ringbuffer187:
L_Sample_Ringbuffer188:
;Protocol.c,606 :: 		O_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
SH	R2, 32(SP)
;Protocol.c,607 :: 		status = Instruction_Values(gcode[5],&O_Val);
ADDIU	R2, SP, 32
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+320)
ORI	R25, R25, lo_addr(_gcode+320)
...
;Protocol.c,617 :: 		i = cpy_val_from_str(temp,(*(gcode+6)),1,strlen(*(gcode+6)));
...
JAL	Protocol_cpy_val_from_str+0
NOP	
;Protocol.c,618 :: 		switch(*(*(gcode+6))) {
J	L_Sample_Ringbuffer190
NOP	
;Protocol.c,619 :: 		case 'J':case 'j':
...
NOP	
SWC1	S0, 24(SP)
;Protocol.c,621 :: 		status = Instruction_Values(gcode[6],&XYZ_Val);
ADDIU	R2, SP, 24
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+384)
ORI	R25, R25, lo_addr(_gcode+384)
...
NOP	
;Protocol.c,626 :: 		case 'F':case 'f':
L_Sample_Ringbuffer194:
L_Sample_Ringbuffer195:
;Protocol.c,627 :: 		O_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
SH	R2, 32(SP)
;Protocol.c,628 :: 		status = Instruction_Values(gcode[6],&O_Val);
ADDIU	R2, SP, 32
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+384)
ORI	R25, R25, lo_addr(_gcode+384)
...
JAL	Protocol_cpy_val_from_str+0
NOP	
;Protocol.c,670 :: 		XYZ_Val = atof(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atof+0
NOP	
...
;Protocol.c,679 :: 		if(*(*(gcode+1)+0) != 0){
LBU	R2, Offset(_gcode+64)(GP)
BNE	R2, R0, L__Sample_Ringbuffer633
NOP	
J	L_Sample_Ringbuffer213
NOP	
L__Sample_Ringbuffer633:
;Protocol.c,680 :: 		xyz[1] = *(*(gcode+1)+0);no_of_axis++;
ADDIU	R2, SP, 107
ADDIU	R3, R2, 1
LBU	R2, Offset(_gcode+64)(GP)
SB	R2, 0(R3)
;Protocol.c,681 :: 		i = cpy_val_from_str(temp,(*(gcode+1)),1,strlen(*(gcode+1)));
LUI	R25, hi_addr(_gcode+64)
ORI	R25, R25, lo_addr(_gcode+64)
JAL	_strlen+0
NOP	
...
JAL	Protocol_cpy_val_from_str+0
NOP	
;Protocol.c,682 :: 		switch(*(*(gcode+1))) {
J	L_Sample_Ringbuffer214
NOP	
;Protocol.c,683 :: 		case 'Y':case 'y':
L_Sample_Ringbuffer216:
L_Sample_Ringbuffer217:
;Protocol.c,684 :: 		case 'Z':case 'z':
L_Sample_Ringbuffer218:
L_Sample_Ringbuffer219:
;Protocol.c,685 :: 		case 'A':case 'a':
L_Sample_Ringbuffer220:
L_Sample_Ringbuffer221:
;Protocol.c,686 :: 		XYZ_Val = atof(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atof+0
NOP	
SWC1	S0, 24(SP)
;Protocol.c,687 :: 		status = Instruction_Values(gcode[1],&XYZ_Val);
ADDIU	R2, SP, 24
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+64)
ORI	R25, R25, lo_addr(_gcode+64)
...
NOP	
SH	R2, 28(SP)
;Protocol.c,691 :: 		break;
J	L_Sample_Ringbuffer215
NOP	
;Protocol.c,692 :: 		case 'F':
L_Sample_Ringbuffer222:
;Protocol.c,693 :: 		case 'f':
L_Sample_Ringbuffer223:
;Protocol.c,696 :: 		F_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
SH	R2, 30(SP)
;Protocol.c,697 :: 		status = Instruction_Values(gcode[2],&F_Val);
ADDIU	R2, SP, 30
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+128)
ORI	R25, R25, lo_addr(_gcode+128)
...
;Protocol.c,705 :: 		break;
J	L_Sample_Ringbuffer86
NOP	
;Protocol.c,706 :: 		case 'F':
L_Sample_Ringbuffer224:
;Protocol.c,708 :: 		if (*(*(gcode)+0)=='F'){
LBU	R3, Offset(_gcode+0)(GP)
ORI	R2, R0, 70
BEQ	R3, R2, L__Sample_Ringbuffer650
NOP	
J	L_Sample_Ringbuffer225
NOP	
L__Sample_Ringbuffer650:
;Protocol.c,709 :: 		i = cpy_val_from_str(temp,(*(gcode+0)),1,strlen(*(gcode+0)));
LUI	R25, hi_addr(_gcode+0)
ORI	R25, R25, lo_addr(_gcode+0)
JAL	_strlen+0
NOP	
...
JAL	Protocol_cpy_val_from_str+0
NOP	
;Protocol.c,710 :: 		F_Val = atoi(temp);
ADDIU	R2, SP, 98
MOVZ	R25, R2, R0
JAL	_atoi+0
NOP	
SH	R2, 30(SP)
;Protocol.c,711 :: 		status = Instruction_Values(gcode[0],&F_Val);
ADDIU	R2, SP, 30
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gcode+0)
ORI	R25, R25, lo_addr(_gcode+0)
...
;Protocol.c,720 :: 		report:
___Sample_Ringbuffer_report:
;Protocol.c,721 :: 		if(query == 1){status = STATUS_OK;}
LH	R3, Offset(Sample_Ringbuffer_query_L0+0)(GP)
ORI	R2, R0, 1
BEQ	R3, R2, L__Sample_Ringbuffer677
NOP	
...
;Protocol.c,722 :: 		else if(query == 2){status = STATUS_BAD_NUMBER_FORMAT;}
LH	R3, Offset(Sample_Ringbuffer_query_L0+0)(GP)
ORI	R2, R0, 2
...
;Protocol.c,723 :: 		else if(query == 3){status = STATUS_UNSUPPORTED_STATEMENT;}
LH	R3, Offset(Sample_Ringbuffer_query_L0+0)(GP)
ORI	R2, R0, 3
...
;Protocol.c,724 :: 		else if(query == 4){status = STATUS_IDLE_ERROR;}
LH	R3, Offset(Sample_Ringbuffer_query_L0+0)(GP)
ORI	R2, R0, 4
BEQ	R3, R2, L__Sample_Ringbuffer680
NOP	
...
;Protocol.c,725 :: 		else if(query == 5){status = ALARM_ABORT_CYCLE;}
LH	R3, Offset(Sample_Ringbuffer_query_L0+0)(GP)
ORI	R2, R0, 5
...
;Protocol.c,726 :: 		else if(query == 6){status = STATUS_IDLE_ERROR;}
LH	R3, Offset(Sample_Ringbuffer_query_L0+0)(GP)
ORI	R2, R0, 6
...
;Protocol.c,727 :: 		else if(query == 7){status = STATUS_SETTING_DISABLED;}
LH	R3, Offset(Sample_Ringbuffer_query_L0+0)(GP)
ORI	R2, R0, 7
...
;Protocol.c,729 :: 		report_status_message(status);
LH	R25, 28(SP)
JAL	_report_status_message+0
NOP	
;Protocol.c,732 :: 		status = Check_group_multiple_violations();
...
Protocol_strsplit:
;Protocol.c,744 :: 		static int strsplit(char arg[arr_size][str_size],char *str, char c){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Protocol.c,746 :: 		Str_Initialize(arg);
SB	R27, 4(SP)
JAL	_Str_Initialize+0
NOP	
LBU	R27, 4(SP)
;Protocol.c,747 :: 		len = strlen(str);
SW	R25, 4(SP)
MOVZ	R25, R26, R0
JAL	_strlen+0
NOP	
...
SEH	R4, R2
;Protocol.c,748 :: 		ii=kk=err=lasti=0;
...
MOVZ	R8, R0, R0
;Protocol.c,749 :: 		for (i = 0; i < len;i++){
; i start address is: 20 (R5)
MOVZ	R5, R0, R0
...
SEH	R2, R4
SLT	R2, R3, R2
BNE	R2, R0, L_Protocol_strsplit685
NOP	
J	L_Protocol_strsplit240
NOP	
L_Protocol_strsplit685:
;Protocol.c,750 :: 		err = i - lasti; //test if string in string is < 49
SUBU	R2, R5, R7
; err start address is: 36 (R9)
SEH	R9, R2
;Protocol.c,751 :: 		if(*(str+i) == c || *(str+i) == '\n'  || err > 49){
SEH	R2, R5
ADDU	R2, R26, R2
LBU	R2, 0(R2)
ANDI	R3, R2, 255
ANDI	R2, R27, 255
BNE	R3, R2, L_Protocol_strsplit687
NOP	
J	L_Protocol_strsplit343
NOP	
L_Protocol_strsplit687:
SEH	R2, R5
ADDU	R2, R26, R2
LBU	R2, 0(R2)
ANDI	R3, R2, 255
ORI	R2, R0, 10
BNE	R3, R2, L_Protocol_strsplit689
NOP	
J	L_Protocol_strsplit342
NOP	
L_Protocol_strsplit689:
SEH	R2, R9
; err end address is: 36 (R9)
SLTI	R2, R2, 50
BNE	R2, R0, L_Protocol_strsplit690
NOP	
J	L_Protocol_strsplit341
NOP	
L_Protocol_strsplit690:
J	L_Protocol_strsplit244
NOP	
; lasti end address is: 28 (R7)
L_Protocol_strsplit343:
L_Protocol_strsplit342:
L_Protocol_strsplit341:
;Protocol.c,752 :: 		arg[kk++][ii] = 0;
SEH	R2, R6
SLL	R2, R2, 6
ADDU	R3, R25, R2
SEH	R2, R8
...
ADDIU	R2, R6, 1
SEH	R6, R2
;Protocol.c,753 :: 		ii=err=0;
; ii start address is: 32 (R8)
MOVZ	R8, R0, R0
;Protocol.c,754 :: 		lasti = i;
; lasti start address is: 28 (R7)
SEH	R7, R5
;Protocol.c,755 :: 		continue;
J	L_Protocol_strsplit241
NOP	
;Protocol.c,756 :: 		}else{
L_Protocol_strsplit244:
;Protocol.c,757 :: 		arg[kk][ii++] = *(str+i);
SEH	R2, R6
SLL	R2, R2, 6
ADDU	R3, R25, R2
SEH	R2, R8
...
ADDIU	R2, R8, 1
SEH	R8, R2
;Protocol.c,759 :: 		if(*(str+i)==0)
SEH	R2, R5
ADDU	R2, R26, R2
LBU	R2, 0(R2)
ANDI	R2, R2, 255
BEQ	R2, R0, L_Protocol_strsplit691
NOP	
...
NOP	
L_Protocol_strsplit240:
;Protocol.c,762 :: 		arg[kk][0] = 0;
SEH	R2, R6
SLL	R2, R2, 6
ADDU	R2, R25, R2
SB	R0, 0(R2)
;Protocol.c,763 :: 		return kk;
SEH	R2, R6
; kk end address is: 24 (R6)
;Protocol.c,764 :: 		}
L_end_strsplit:
...
Protocol_strsplit2:
;Protocol.c,769 :: 		static int strsplit2(char arg[arr_size][str_size],char *str, char c){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Protocol.c,771 :: 		Str_Initialize(arg);
SB	R27, 4(SP)
JAL	_Str_Initialize+0
NOP	
LBU	R27, 4(SP)
;Protocol.c,772 :: 		len = strlen(str);
SW	R25, 4(SP)
MOVZ	R25, R26, R0
JAL	_strlen+0
NOP	
...
; len start address is: 16 (R4)
SEH	R4, R2
;Protocol.c,773 :: 		track_char=ii=kk=err=lasti=0;
...
MOVZ	R6, R0, R0
;Protocol.c,774 :: 		for (i = 0;i < len;i++){
; i start address is: 20 (R5)
MOVZ	R5, R0, R0
; len end address is: 16 (R4)
...
SEH	R2, R4
SLT	R2, R3, R2
BNE	R2, R0, L_Protocol_strsplit2693
NOP	
J	L_Protocol_strsplit2302
NOP	
L_Protocol_strsplit2693:
;Protocol.c,775 :: 		err = i - lasti; //test if string in string is < 49
SUBU	R2, R5, R8
; err start address is: 40 (R10)
SEH	R10, R2
;Protocol.c,776 :: 		if(*(str+i) == c || *(str+i) == '\n'  || err > 49){
SEH	R2, R5
ADDU	R2, R26, R2
LBU	R2, 0(R2)
ANDI	R3, R2, 255
ANDI	R2, R27, 255
BNE	R3, R2, L_Protocol_strsplit2695
NOP	
J	L_Protocol_strsplit2293
NOP	
L_Protocol_strsplit2695:
SEH	R2, R5
ADDU	R2, R26, R2
LBU	R2, 0(R2)
ANDI	R3, R2, 255
ORI	R2, R0, 10
BNE	R3, R2, L_Protocol_strsplit2697
NOP	
J	L_Protocol_strsplit2292
NOP	
L_Protocol_strsplit2697:
SEH	R2, R10
; err end address is: 40 (R10)
SLTI	R2, R2, 50
BNE	R2, R0, L_Protocol_strsplit2698
NOP	
J	L_Protocol_strsplit2291
NOP	
L_Protocol_strsplit2698:
...
SEH	R3, R2
;Protocol.c,778 :: 		ii=err=0;
; ii start address is: 28 (R7)
MOVZ	R7, R0, R0
;Protocol.c,779 :: 		lasti = i;
; lasti start address is: 32 (R8)
SEH	R8, R5
;Protocol.c,780 :: 		track_char++;
ADDIU	R2, R6, 1
SEH	R6, R2
;Protocol.c,781 :: 		continue;
SEH	R9, R7
...
;Protocol.c,783 :: 		if(i > 0 && (*(str+0) != '$') ){
; kk start address is: 28 (R7)
; ii start address is: 36 (R9)
SEH	R2, R5
SLTI	R2, R2, 1
BEQ	R2, R0, L_Protocol_strsplit2699
NOP	
...
;Protocol.c,784 :: 		if(!track_char && *(str+i) > 0x39){
BEQ	R6, R0, L_Protocol_strsplit2702
NOP	
J	L_Protocol_strsplit2298
NOP	
L_Protocol_strsplit2702:
SEH	R2, R5
ADDU	R2, R26, R2
LBU	R2, 0(R2)
ANDI	R2, R2, 255
SLTIU	R2, R2, 58
BEQ	R2, R0, L_Protocol_strsplit2703
NOP	
...
SEH	R7, R2
LH	R2, 4(SP)
J	L_Protocol_strsplit2294
NOP	
L_Protocol_strsplit2299:
SEH	R2, R7
SEH	R7, R8
SEH	R3, R9
L_Protocol_strsplit2294:
;Protocol.c,783 :: 		if(i > 0 && (*(str+0) != '$') ){
...
LBU	R2, 0(R2)
ANDI	R2, R2, 255
BEQ	R2, R0, L_Protocol_strsplit2704
NOP	
J	L_Protocol_strsplit2260
NOP	
...
;Protocol.c,801 :: 		static int cpy_val_from_str(char *strA,const char *strB,int indx,int num_of_char){
;Protocol.c,804 :: 		tmp = strB+indx;
SEH	R2, R27
ADDU	R2, R26, R2
; tmp start address is: 16 (R4)
MOVZ	R4, R2, R0
;Protocol.c,805 :: 		*(tmp+num_of_char)=0;
SEH	R2, R28
ADDU	R2, R4, R2
SB	R0, 0(R2)
;Protocol.c,807 :: 		i = 0;
...
; i end address is: 12 (R3)
;Protocol.c,808 :: 		while(*tmp != 0){
L_Protocol_cpy_val_from_str261:
; i start address is: 12 (R3)
; tmp start address is: 16 (R4)
LBU	R2, 0(R4)
ANDI	R2, R2, 255
BNE	R2, R0, L_Protocol_cpy_val_from_str707
NOP	
J	L_Protocol_cpy_val_from_str262
NOP	
L_Protocol_cpy_val_from_str707:
;Protocol.c,809 :: 		*strA++ = *tmp++;
LBU	R2, 0(R4)
SB	R2, 0(R25)
ADDIU	R2, R25, 1
MOVZ	R25, R2, R0
...
;Protocol.c,811 :: 		}
; tmp end address is: 16 (R4)
J	L_Protocol_cpy_val_from_str261
NOP	
L_Protocol_cpy_val_from_str262:
;Protocol.c,812 :: 		*strA = '\0';
SB	R0, 0(R25)
;Protocol.c,814 :: 		return i;
SEH	R2, R3
; i end address is: 12 (R3)
;Protocol.c,815 :: 		}
...
Protocol_str2int:
;Protocol.c,820 :: 		static int str2int(char *str,int base){
ADDIU	SP, SP, -16
SW	RA, 0(SP)
;Protocol.c,822 :: 		int result = 0;
MOVZ	R30, R0, R0
SH	R30, 14(SP)
;Protocol.c,824 :: 		len = strlen(str);
JAL	_strlen+0
NOP	
SH	R2, 12(SP)
;Protocol.c,826 :: 		for(i=0; i<len; i++){
; i start address is: 16 (R4)
MOVZ	R4, R0, R0
; i end address is: 16 (R4)
L_Protocol_str2int263:
; i start address is: 16 (R4)
SEH	R3, R4
LH	R2, 12(SP)
SLT	R2, R3, R2
BNE	R2, R0, L_Protocol_str2int709
NOP	
J	L_Protocol_str2int264
NOP	
L_Protocol_str2int709:
;Protocol.c,827 :: 		result = result * base + ( *(str+i) - 0x30 );
LH	R2, 14(SP)
MUL	R3, R2, R26
SEH	R2, R4
ADDU	R2, R25, R2
...
; i end address is: 16 (R4)
SEH	R3, R4
;Protocol.c,828 :: 		while(DMA_IsOn(1));
L_Protocol_str2int266:
; i start address is: 12 (R3)
SH	R3, 4(SP)
SH	R26, 6(SP)
SW	R25, 8(SP)
...
LH	R26, 6(SP)
LH	R3, 4(SP)
BNE	R2, R0, L_Protocol_str2int711
NOP	
J	L_Protocol_str2int267
NOP	
L_Protocol_str2int711:
J	L_Protocol_str2int266
NOP	
...
;Protocol.c,829 :: 		}
; i end address is: 16 (R4)
J	L_Protocol_str2int263
NOP	
L_Protocol_str2int264:
;Protocol.c,831 :: 		return result;
LH	R2, 14(SP)
;Protocol.c,832 :: 		}
L_end_str2int:
LW	RA, 0(SP)
...
;Protocol.c,889 :: 		void protocol_execute_runtime(){
ADDIU	SP, SP, -12
SW	RA, 0(SP)
;Protocol.c,890 :: 		if (sys.execute) { // Enter only if any bit flag is true
SW	R25, 4(SP)
LH	R2, Offset(_sys+26)(GP)
BNE	R2, R0, L__protocol_execute_runtime714
NOP	
J	L_protocol_execute_runtime268
NOP	
L__protocol_execute_runtime714:
;Protocol.c,891 :: 		int rt_exec = sys.execute; // Avoid calling volatile multiple times
; rt_exec start address is: 16 (R4)
LH	R4, Offset(_sys+26)(GP)
;Protocol.c,896 :: 		if (rt_exec & (EXEC_ALARM | EXEC_CRIT_EVENT)) {
LH	R2, Offset(_sys+26)(GP)
ANDI	R2, R2, 96
BNE	R2, R0, L__protocol_execute_runtime716
NOP	
J	L__protocol_execute_runtime345
NOP	
L__protocol_execute_runtime716:
;Protocol.c,897 :: 		sys.state = STATE_ALARM; // Set system alarm state
ORI	R2, R0, 6
SH	R2, Offset(_sys+2)(GP)
;Protocol.c,900 :: 		if (rt_exec & EXEC_CRIT_EVENT) {
ANDI	R2, R4, 64
BNE	R2, R0, L__protocol_execute_runtime718
NOP	
J	L_protocol_execute_runtime270
NOP	
L__protocol_execute_runtime718:
;Protocol.c,901 :: 		report_alarm_message(ALARM_HARD_LIMIT);
SH	R4, 8(SP)
ORI	R25, R0, 65535
JAL	_report_alarm_message+0
NOP	
;Protocol.c,902 :: 		report_feedback_message(MESSAGE_CRITICAL_EVENT);
ORI	R25, R0, 1
JAL	_report_feedback_message+0
NOP	
LH	R4, 8(SP)
;Protocol.c,903 :: 		bit_false(sys.execute,EXEC_RESET); // Disable any existing reset
LH	R3, Offset(_sys+26)(GP)
ORI	R2, R0, 65519
AND	R2, R3, R2
SH	R2, Offset(_sys+26)(GP)
...
;Protocol.c,908 :: 		} while (bit_isfalse(sys.execute,EXEC_RESET));
;Protocol.c,904 :: 		do {
...
;Protocol.c,915 :: 		report_alarm_message(ALARM_ABORT_CYCLE);
; rt_exec start address is: 16 (R4)
SH	R4, 8(SP)
ORI	R25, R0, 65534
JAL	_report_alarm_message+0
...
;Protocol.c,917 :: 		bit_false(sys.execute,(EXEC_ALARM | EXEC_CRIT_EVENT));
; rt_exec start address is: 16 (R4)
LH	R3, Offset(_sys+26)(GP)
ORI	R2, R0, 65439
AND	R2, R3, R2
...
;Protocol.c,921 :: 		if (rt_exec & EXEC_RESET) {
; rt_exec start address is: 16 (R4)
ANDI	R2, R4, 16
...
; rt_exec end address is: 16 (R4)
;Protocol.c,922 :: 		sys.abort = true;  // Only place this is set true.
ORI	R2, R0, 1
SH	R2, Offset(_sys+0)(GP)
;Protocol.c,923 :: 		return; // Nothing else to do but exit.
J	L_end_protocol_execute_runtime
NOP	
;Protocol.c,924 :: 		}
L_protocol_execute_runtime275:
;Protocol.c,927 :: 		if (rt_exec & EXEC_STATUS_REPORT) {
; rt_exec start address is: 16 (R4)
ANDI	R2, R4, 1
...
;Protocol.c,928 :: 		report_realtime_status();
SH	R4, 8(SP)
JAL	_report_realtime_status+0
NOP	
LH	R4, 8(SP)
;Protocol.c,929 :: 		bit_false(sys.execute,EXEC_STATUS_REPORT);
LH	R3, Offset(_sys+26)(GP)
ORI	R2, R0, 65534
AND	R2, R3, R2
SH	R2, Offset(_sys+26)(GP)
...
;Protocol.c,933 :: 		if (rt_exec & EXEC_FEED_HOLD) {
ANDI	R2, R4, 8
...
;Protocol.c,935 :: 		bit_false(sys.execute,EXEC_FEED_HOLD);
LH	R3, Offset(_sys+26)(GP)
ORI	R2, R0, 65527
AND	R2, R3, R2
SH	R2, Offset(_sys+26)(GP)
...
;Protocol.c,940 :: 		if (rt_exec & EXEC_CYCLE_STOP) {
ANDI	R2, R4, 4
...
;Protocol.c,942 :: 		bit_false(sys.execute,EXEC_CYCLE_STOP);
LH	R3, Offset(_sys+26)(GP)
ORI	R2, R0, 65531
AND	R2, R3, R2
SH	R2, Offset(_sys+26)(GP)
...
;Protocol.c,945 :: 		if (rt_exec & EXEC_CYCLE_START) {
ANDI	R2, R4, 2
; rt_exec end address is: 16 (R4)
...
;Protocol.c,947 :: 		if (bit_istrue(settings.flags,FLAG_AUTO_START)) {
LHU	R2, Offset(_settings+50)(GP)
ANDI	R2, R2, 2
ANDI	R2, R2, 65535
...
;Protocol.c,948 :: 		sys.auto_start = true; // Re-enable auto start after feed hold.
ORI	R2, R0, 1
SH	R2, Offset(_sys+24)(GP)
;Protocol.c,949 :: 		}
L_protocol_execute_runtime280:
;Protocol.c,950 :: 		bit_false(sys.execute,EXEC_CYCLE_START);
LH	R3, Offset(_sys+26)(GP)
ORI	R2, R0, 65533
AND	R2, R3, R2
SH	R2, Offset(_sys+26)(GP)
...
;Protocol.c,960 :: 		void protocol_system_check(){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Protocol.c,964 :: 		if (sys.abort) {
SW	R25, 4(SP)
LH	R2, Offset(_sys+0)(GP)
BNE	R2, R0, L__protocol_system_check735
NOP	
J	L_protocol_system_check281
NOP	
L__protocol_system_check735:
;Protocol.c,977 :: 		sys_sync_current_position();
JAL	_sys_sync_current_position+0
NOP	
;Protocol.c,980 :: 		sys.abort = false;
SH	R0, Offset(_sys+0)(GP)
;Protocol.c,981 :: 		sys.execute = 0;
SH	R0, Offset(_sys+26)(GP)
;Protocol.c,982 :: 		if (bit_istrue(settings.flags,BITFLAG_AUTO_START)) { sys.auto_start = true; }
LHU	R2, Offset(_settings+50)(GP)
ANDI	R2, R2, 2
ANDI	R2, R2, 65535
BNE	R2, R0, L__protocol_system_check737
NOP	
J	L_protocol_system_check282
NOP	
L__protocol_system_check737:
ORI	R2, R0, 1
SH	R2, Offset(_sys+24)(GP)
L_protocol_system_check282:
;Protocol.c,992 :: 		if (sys.state == STATE_INIT && bit_istrue(settings.flags,BITFLAG_HOMING_ENABLE)) { sys.state = STATE_ALARM; }
LH	R3, Offset(_sys+2)(GP)
ORI	R2, R0, 1
BEQ	R3, R2, L__protocol_system_check738
NOP	
J	L__protocol_system_check348
NOP	
L__protocol_system_check738:
LHU	R2, Offset(_settings+50)(GP)
ANDI	R2, R2, 16
ANDI	R2, R2, 65535
BNE	R2, R0, L__protocol_system_check740
NOP	
J	L__protocol_system_check347
NOP	
L__protocol_system_check740:
L__protocol_system_check346:
ORI	R2, R0, 6
SH	R2, Offset(_sys+2)(GP)
L__protocol_system_check348:
L__protocol_system_check347:
;Protocol.c,996 :: 		if (sys.state == STATE_ALARM) {
LH	R3, Offset(_sys+2)(GP)
ORI	R2, R0, 6
BEQ	R3, R2, L__protocol_system_check741
NOP	
J	L_protocol_system_check286
NOP	
L__protocol_system_check741:
;Protocol.c,997 :: 		report_feedback_message(MESSAGE_ALARM_LOCK);
ORI	R25, R0, 2
JAL	_report_feedback_message+0
NOP	
;Protocol.c,998 :: 		} else {
J	L_protocol_system_check287
NOP	
L_protocol_system_check286:
;Protocol.c,1000 :: 		sys.state = STATE_IDLE;
SH	R0, Offset(_sys+2)(GP)
;Protocol.c,1002 :: 		}
L_protocol_system_check287:
;Protocol.c,1003 :: 		}
L_protocol_system_check281:
;Protocol.c,1004 :: 		}
