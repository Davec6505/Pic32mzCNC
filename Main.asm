...
NOP	
;Main.c,71 :: 		cntr = a = axis_to_run = dif = status_of_gcode = 0;
SH	R0, 2(SP)
;Main.c,75 :: 		if(read_ram_loaded_indicator()){
...
NOP	
L__main126:
;Main.c,76 :: 		settings_read_coord_data();
...
NOP	
L__main127:
;Main.c,88 :: 		modal_group = Get_modalgroup();
...
NOP	
SH	R2, 0(SP)
;Main.c,97 :: 		if(axis_to_run){
BNE	R2, R0, L__main129
NOP	
J	L_main9
NOP	
L__main129:
;Main.c,98 :: 		EnableSteppers(2);
ORI	R25, R0, 2
JAL	_EnableSteppers+0
NOP	
;Main.c,99 :: 		Modal_Group_Actions1(axis_to_run);
LH	R25, 0(SP)
JAL	_Modal_Group_Actions1+0
NOP	
;Main.c,100 :: 		axis_to_run = Rst_Axisword();
JAL	_Rst_Axisword+0
NOP	
;Main.c,101 :: 		}
L_main9:
...
J	L_main5
NOP	
;Main.c,114 :: 		case 256://MODAL_GROUP_7 [M3,M4,M5] Spindle turning
L_main15:
;Main.c,115 :: 		Modal_Group_Actions7(1);//implimentation needed
ORI	R25, R0, 1
...
NOP	
L_main3:
;Main.c,121 :: 		report_status_message(status_of_gcode);
LH	R25, 2(SP)
JAL	_report_status_message+0
NOP	
;Main.c,122 :: 		}
L_main17:
;Main.c,124 :: 		status_of_gcode = Sample_Ringbuffer();
JAL	_Sample_Ringbuffer+0
NOP	
SH	R2, 2(SP)
;Main.c,129 :: 		LED1 = TMR.clock >> 4;
LBU	R2, Offset(_TMR+0)(GP)
SRL	R3, R2, 4
_LX	
INS	R2, R3, BitPos(LED1+0), 1
_SX	
;Main.c,134 :: 		if(disable_steps <= SEC_TO_DISABLE_STEPPERS)
LHU	R2, Offset(Main_disable_steps+0)(GP)
SLTIU	R2, R2, 11
BNE	R2, R0, L__main150
NOP	
J	L_main18
NOP	
L__main150:
;Main.c,135 :: 		disable_steps = TMR.Reset(SEC_TO_DISABLE_STEPPERS,disable_steps);
...
L_main18:
;Main.c,151 :: 		protocol_execute_runtime();
JAL	_protocol_execute_runtime+0
NOP	
;Main.c,152 :: 		WDTCONSET = 0x01;
ORI	R2, R0, 1
...
;Main.c,159 :: 		int Modal_Group_Actions0(int action){
ADDIU	SP, SP, -44
SW	RA, 0(SP)
;Main.c,169 :: 		switch(action){
SW	R26, 4(SP)
SW	R27, 8(SP)
J	L_Modal_Group_Actions019
NOP	
;Main.c,170 :: 		case 2:  //NON_MODAL_DWELL
L_Modal_Group_Actions021:
;Main.c,171 :: 		i = 0;
; i start address is: 20 (R5)
MOVZ	R5, R0, R0
;Main.c,173 :: 		if(gc.S > 0){ //wait in seconds
...
;Main.c,180 :: 		}else if(gc.P > 0){ //wait in msec
...
;Main.c,181 :: 		dly_time = (unsigned long)gc.P;
LH	R2, Offset(_gc+116)(GP)
; dly_time start address is: 16 (R4)
MOVZ	R4, R2, R0
; dly_time end address is: 16 (R4)
...
NOP	
;Main.c,190 :: 		case 4:  //NON_MODAL_SET_COORDINATE_DATA
L_Modal_Group_Actions033:
;Main.c,203 :: 		if(gc.L != 2 && gc.L != 20)
LH	R3, Offset(_gc+16)(GP)
ORI	R2, R0, 2
BNE	R3, R2, L__Modal_Group_Actions0158
NOP	
...
;Main.c,207 :: 		result = Settings_Write_Coord_Data((int)gc.P,gc.next_position );
SH	R25, 12(SP)
...
NOP	
LH	R25, 12(SP)
;Main.c,208 :: 		if(result){ //response if write to flash failed
BNE	R2, R0, L__Modal_Group_Actions0163
NOP	
J	L_Modal_Group_Actions038
NOP	
L__Modal_Group_Actions0163:
;Main.c,209 :: 		return NVM_COORDINATE_WRITE_ERROR;
ORI	R2, R0, 1
J	L_end_Modal_Group_Actions0
NOP	
;Main.c,210 :: 		}
L_Modal_Group_Actions038:
;Main.c,212 :: 		if (gc.coord_select > 0) {
...
;Main.c,213 :: 		memcpy(gc.coord_system,gc.next_position,sizeof(gc.next_position));
SH	R25, 12(SP)
ORI	R27, R0, 16
LUI	R26, hi_addr(_gc+72)
ORI	R26, R26, lo_addr(_gc+72)
...
;Main.c,221 :: 		temp = indx = (gc.P-1) & 0xFF;
LH	R2, Offset(_gc+116)(GP)
ADDIU	R2, R2, -1
ANDI	R2, R2, 255
;Main.c,222 :: 		indx *= 4;
...
SEH	R3, R2
;Main.c,223 :: 		axis_cnt = 0;
; axis_cnt start address is: 16 (R4)
MOVZ	R4, R0, R0
;Main.c,227 :: 		axis_words = Get_Axisword();
SH	R4, 12(SP)
SH	R3, 14(SP)
SH	R25, 16(SP)
JAL	_Get_Axisword+0
...
; axis_words end address is: 20 (R5)
;Main.c,229 :: 		temp_axis = (axis_words >> i) & 1;
; axis_words start address is: 20 (R5)
SEH	R3, R5
SEH	R2, R6
SRAV	R2, R3, R2
ANDI	R2, R2, 1
;Main.c,231 :: 		if(temp_axis == 0){
SEH	R2, R2
...
; axis_cnt start address is: 16 (R4)
SEH	R4, R2
;Main.c,233 :: 		if(axis_cnt > 2)break;
SEH	R2, R2
SLTI	R2, R2, 3
BEQ	R2, R0, L__Modal_Group_Actions0167
NOP	
...
MOVZ	R8, R2, R0
;Main.c,239 :: 		coord_data[i] = ulong2flt(_flash);
ADDIU	R3, SP, 24
SEH	R2, R6
SLL	R2, R2, 2
ADDU	R2, R3, R2
...
;Main.c,247 :: 		coord_data[i] = gc.next_position[i];
; axis_cnt start address is: 32 (R8)
ADDIU	R3, SP, 24
SEH	R2, R6
SLL	R4, R2, 2
...
;Main.c,257 :: 		result = Settings_Write_Coord_Data((int)gc.P,coord_data);
ADDIU	R2, SP, 24
SH	R25, 12(SP)
MOVZ	R26, R2, R0
LH	R25, Offset(_gc+116)(GP)
JAL	_Settings_Write_Coord_Data+0
NOP	
;Main.c,260 :: 		memcpy(gc.coord_system,coord_data,sizeof(coord_data));
ADDIU	R2, SP, 24
ORI	R27, R0, 16
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gc+40)
...
J	L_Modal_Group_Actions020
NOP	
;Main.c,264 :: 		case  8:  //NON_MODAL_GO_HOME_0_BIT
L_Modal_Group_Actions047:
;Main.c,265 :: 		case 32:  //NON_MODAL_GO_HOME_1_BIT
L_Modal_Group_Actions048:
;Main.c,268 :: 		axis_words = Get_Axisword();
SH	R25, 12(SP)
JAL	_Get_Axisword+0
NOP	
LH	R25, 12(SP)
...
SEH	R6, R2
;Main.c,273 :: 		if (axis_words) {
BNE	R2, R0, L__Modal_Group_Actions0169
NOP	
J	L_Modal_Group_Actions049
NOP	
L__Modal_Group_Actions0169:
;Main.c,275 :: 		for (i=0; i<NoOfAxis; i++){ // Axes indices are consistent, so loop may be used.
; i start address is: 20 (R5)
MOVZ	R5, R0, R0
...
;Main.c,276 :: 		if ( bit_istrue(axis_words,bit(i+1)) ) {
ADDIU	R2, R5, 1
SEH	R3, R2
ORI	R2, R0, 1
SLLV	R2, R2, R3
...
;Main.c,277 :: 		if (gc.absolute_mode) {
...
;Main.c,278 :: 		gc.next_position[i] += gc.coord_system[i] + gc.coord_offset[i];
SEH	R2, R5
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+72)
ORI	R2, R2, lo_addr(_gc+72)
...
;Main.c,280 :: 		gc.next_position[i] += gc.position[i];
SEH	R2, R5
SLL	R3, R2, 2
LUI	R2, hi_addr(_gc+72)
ORI	R2, R2, lo_addr(_gc+72)
...
;Main.c,283 :: 		gc.next_position[i] = gc.position[i];
SEH	R2, R5
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+72)
ORI	R2, R2, lo_addr(_gc+72)
...
;Main.c,286 :: 		SingleAxisStep(gc.next_position[i],settings.default_seek_rate,i);
...
SEH	R4, R5
SEH	R3, R6
;Main.c,287 :: 		while(GET_RunState(i));
...
;Main.c,291 :: 		temp = SETTING_INDEX_G28;  //home to zero pos / at limits
; temp start address is: 24 (R6)
MOVZ	R6, R0, R0
;Main.c,293 :: 		if (action == NON_MODAL_GO_HOME_1_BIT){temp = SETTING_INDEX_G30;}
SEH	R3, R25
ORI	R2, R0, 32
BEQ	R3, R2, L__Modal_Group_Actions0177
NOP	
...
;Main.c,294 :: 		i = (temp)*4 ; //place the new data into the correct position
; temp start address is: 24 (R6)
SEH	R2, R6
SLL	R2, R2, 2
; i start address is: 16 (R4)
SEH	R4, R2
;Main.c,297 :: 		for(j = 0;j<4;j++){
...
MOVZ	R7, R2, R0
;Main.c,299 :: 		coord_system[temp].coord[j] = ulong2flt(_data);
SEH	R2, R6
SLL	R3, R2, 5
...
SEH	R7, R2
;Main.c,307 :: 		SingleAxisStep(coord_system[temp].coord[j],settings.default_seek_rate,j);
LWC1	S0, Offset(_settings+24)(GP)
CVT36.S 	S0, S0
MFC1	R4, S0
SEH	R2, R6
...
SEH	R4, R6
SEH	R3, R7
;Main.c,308 :: 		while(GET_RunState(j));
...
;Main.c,315 :: 		axis_words = Get_Axisword();
SH	R25, 12(SP)
JAL	_Get_Axisword+0
NOP	
LH	R25, 12(SP)
; axis_words start address is: 20 (R5)
SEH	R5, R2
;Main.c,319 :: 		for (i=0; i<=2; i++) { // Axes indices are consistent, so loop may be used to save flash space.
; i start address is: 24 (R6)
MOVZ	R6, R0, R0
; i end address is: 24 (R6)
...
; axis_words end address is: 20 (R5)
;Main.c,320 :: 		if ( bit_istrue(axis_words,bit(i)) ) {
; axis_words start address is: 20 (R5)
SEH	R3, R6
ORI	R2, R0, 1
SLLV	R2, R2, R3
...
;Main.c,321 :: 		if (!gc.absolute_override) {
...
;Main.c,322 :: 		if (!gc.absolute_mode) { // Do not update target in absolute override mode
LBU	R2, Offset(_gc+5)(GP)
BEQ	R2, R0, L__Modal_Group_Actions0185
NOP	
J	L_Modal_Group_Actions072
NOP	
L__Modal_Group_Actions0185:
;Main.c,323 :: 		gc.next_position[i] += gc.coord_system[i] + gc.coord_offset[i]; // Absolute mode
SEH	R2, R6
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+72)
ORI	R2, R2, lo_addr(_gc+72)
...
;Main.c,325 :: 		gc.next_position[i] += gc.position[i]; // Incremental mode
SEH	R2, R6
SLL	R3, R2, 2
LUI	R2, hi_addr(_gc+72)
ORI	R2, R2, lo_addr(_gc+72)
...
;Main.c,328 :: 		gc.next_position[i] = gc.position[i]; // No axis word in block. Keep same axis position.
SEH	R2, R6
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+72)
ORI	R2, R2, lo_addr(_gc+72)
...
J	L_Modal_Group_Actions020
NOP	
;Main.c,333 :: 		case 64:   //NON_MODAL_SET_HOME_1
L_Modal_Group_Actions075:
;Main.c,334 :: 		temp = SETTING_INDEX_G28;
; temp start address is: 16 (R4)
MOVZ	R4, R0, R0
;Main.c,335 :: 		if (action == NON_MODAL_SET_HOME_1_BIT) { temp = SETTING_INDEX_G30; }
SEH	R3, R25
ORI	R2, R0, 64
BEQ	R3, R2, L__Modal_Group_Actions0186
NOP	
...
;Main.c,336 :: 		Settings_Write_Coord_Data(temp,gc.position);
; temp start address is: 8 (R2)
SH	R25, 12(SP)
...
J	L_Modal_Group_Actions020
NOP	
;Main.c,338 :: 		case 128:  //NON_MODAL_SET_COORDINATE_OFFSET
L_Modal_Group_Actions077:
;Main.c,339 :: 		axis_words = Get_Axisword();
SH	R25, 12(SP)
JAL	_Get_Axisword+0
NOP	
LH	R25, 12(SP)
; axis_words start address is: 20 (R5)
SEH	R5, R2
;Main.c,340 :: 		if (!axis_words) { // No axis words
...
; axis_words end address is: 20 (R5)
;Main.c,341 :: 		FAIL(STATUS_INVALID_STATEMENT);
ORI	R2, R0, 6
...
;Main.c,345 :: 		for (i=0; i<=2; i++) { // Axes indices are consistent, so loop may be used.
; i start address is: 24 (R6)
; axis_words start address is: 20 (R5)
MOVZ	R6, R0, R0
; i end address is: 24 (R6)
...
SEH	R2, R6
SLTI	R2, R2, 3
BNE	R2, R0, L__Modal_Group_Actions0188
NOP	
...
;Main.c,347 :: 		gc.coord_offset[i] = gc.position[i]-gc.coord_system[i]-gc.next_position[i];
SEH	R2, R6
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+56)
ORI	R2, R2, lo_addr(_gc+56)
...
;Main.c,352 :: 		case 256: //NON_MODAL_RESET_COORDINATE_OFFSET
...
J	L_Modal_Group_Actions020
NOP	
;Main.c,356 :: 		default: action = -1; //error in action msg ???
...
;Main.c,365 :: 		int Modal_Group_Actions1(int action){
ADDIU	SP, SP, -16
SW	RA, 0(SP)
;Main.c,366 :: 		switch(action){
SW	R26, 4(SP)
SW	R27, 8(SP)
J	L_Modal_Group_Actions187
NOP	
;Main.c,367 :: 		case 1: //b0000 0001
L_Modal_Group_Actions189:
;Main.c,368 :: 		SingleAxisStep(gc.next_position[X],gc.frequency,X);
SH	R25, 12(SP)
MOVZ	R26, R0, R0
...
NOP	
;Main.c,370 :: 		case 2://b0000 0010
L_Modal_Group_Actions190:
;Main.c,371 :: 		SingleAxisStep(gc.next_position[Y],gc.frequency,Y);
SH	R25, 12(SP)
ORI	R26, R0, 1
LH	R25, Offset(_gc+14)(GP)
...
J	L_Modal_Group_Actions188
NOP	
;Main.c,373 :: 		case 3://b0000 0011
L_Modal_Group_Actions191:
;Main.c,376 :: 		DualAxisStep(gc.next_position[X], gc.next_position[Y],X,Y,gc.frequency);
SH	R25, 12(SP)
...
J	L_Modal_Group_Actions188
NOP	
;Main.c,380 :: 		case 4://b0000 0100
L_Modal_Group_Actions192:
;Main.c,381 :: 		SingleAxisStep(gc.next_position[Z],gc.frequency,Z);
SH	R25, 12(SP)
ORI	R26, R0, 2
LH	R25, Offset(_gc+14)(GP)
...
J	L_Modal_Group_Actions188
NOP	
;Main.c,383 :: 		case 5://b0000 0101
L_Modal_Group_Actions193:
;Main.c,384 :: 		DualAxisStep(gc.next_position[X], gc.next_position[Z],X,Z,gc.frequency);
SH	R25, 12(SP)
LH	R27, Offset(_gc+14)(GP)
ORI	R26, R0, 2
MOVZ	R25, R0, R0
...
J	L_Modal_Group_Actions188
NOP	
;Main.c,386 :: 		case 6://b0000 0110
L_Modal_Group_Actions194:
;Main.c,387 :: 		DualAxisStep(gc.next_position[Y], gc.next_position[Z],Y,Z,gc.frequency);
SH	R25, 12(SP)
LH	R27, Offset(_gc+14)(GP)
ORI	R26, R0, 2
ORI	R25, R0, 1
...
J	L_Modal_Group_Actions188
NOP	
;Main.c,389 :: 		case 8://b0000 1000
L_Modal_Group_Actions195:
;Main.c,390 :: 		SingleAxisStep(gc.next_position[A],gc.frequency,A);
SH	R25, 12(SP)
ORI	R26, R0, 3
LH	R25, Offset(_gc+14)(GP)
...
J	L_Modal_Group_Actions188
NOP	
;Main.c,392 :: 		case 9://b0000 1001
L_Modal_Group_Actions196:
;Main.c,393 :: 		DualAxisStep(gc.next_position[X], gc.next_position[A],X,A,gc.frequency);
SH	R25, 12(SP)
LH	R27, Offset(_gc+14)(GP)
ORI	R26, R0, 3
MOVZ	R25, R0, R0
...
J	L_Modal_Group_Actions188
NOP	
;Main.c,395 :: 		case 10://b0000 1010
L_Modal_Group_Actions197:
;Main.c,396 :: 		DualAxisStep(gc.next_position[Y], gc.next_position[A],Y,A,gc.frequency);
SH	R25, 12(SP)
LH	R27, Offset(_gc+14)(GP)
ORI	R26, R0, 3
ORI	R25, R0, 1
...
J	L_Modal_Group_Actions188
NOP	
;Main.c,398 :: 		case 12://b0000 1100
L_Modal_Group_Actions198:
;Main.c,399 :: 		DualAxisStep(gc.next_position[Z], gc.next_position[A],Z,A,gc.frequency);
SH	R25, 12(SP)
LH	R27, Offset(_gc+14)(GP)
ORI	R26, R0, 3
ORI	R25, R0, 2
...
J	L_Modal_Group_Actions188
NOP	
;Main.c,401 :: 		case 13://Homing X axis
...
J	L_Modal_Group_Actions188
NOP	
;Main.c,404 :: 		case 14://Homing Y axis
...
J	L_Modal_Group_Actions188
NOP	
;Main.c,407 :: 		case 15://Homing Y axis
L_Modal_Group_Actions1101:
;Main.c,408 :: 		r_or_ijk(150.00, 30.00, 150.00, 30.00, 0.00, -50.00, 50.00,0.00,X,Y,CW);
LUI	R5, 16880
ORI	R5, R5, 0
LUI	R4, 17174
ORI	R4, R4, 0
...
;Main.c,421 :: 		int Modal_Group_Actions3(int action){
;Main.c,426 :: 		return action;
SEH	R2, R25
;Main.c,427 :: 		}
...
;Main.c,432 :: 		int Modal_Group_Actions4(int action){
;Main.c,437 :: 		return action;
SEH	R2, R25
;Main.c,438 :: 		}
...
;Main.c,443 :: 		int Modal_Group_Actions7(int action){
;Main.c,448 :: 		return action;
SEH	R2, R25
;Main.c,449 :: 		}
...
;Main.c,467 :: 		void protocol_execute_runtime(){
ADDIU	SP, SP, -12
SW	RA, 0(SP)
;Main.c,468 :: 		if (sys.execute) { // Enter only if any bit flag is true
SW	R25, 4(SP)
...
;Main.c,469 :: 		uint8_t rt_exec = sys.execute; // Avoid calling volatile multiple times
; rt_exec start address is: 16 (R4)
LBU	R4, Offset(_sys+25)(GP)
;Main.c,474 :: 		if (rt_exec & (EXEC_ALARM | EXEC_CRIT_EVENT)) {
LBU	R2, Offset(_sys+25)(GP)
ANDI	R2, R2, 96
BNE	R2, R0, L__protocol_execute_runtime245
NOP	
J	L__protocol_execute_runtime122
NOP	
L__protocol_execute_runtime245:
;Main.c,475 :: 		sys.state = STATE_ALARM; // Set system alarm state
ORI	R2, R0, 6
SB	R2, Offset(_sys+1)(GP)
;Main.c,478 :: 		if (rt_exec & EXEC_CRIT_EVENT) {
ANDI	R2, R4, 64
...
;Main.c,479 :: 		report_alarm_message(ALARM_HARD_LIMIT);
SB	R4, 8(SP)
ORI	R25, R0, 65535
JAL	_report_alarm_message+0
NOP	
;Main.c,480 :: 		report_feedback_message(MESSAGE_CRITICAL_EVENT);
ORI	R25, R0, 1
JAL	_report_feedback_message+0
NOP	
LBU	R4, 8(SP)
;Main.c,481 :: 		bit_false(sys.execute,EXEC_RESET); // Disable any existing reset
...
;Main.c,486 :: 		} while (bit_isfalse(sys.execute,EXEC_RESET));
;Main.c,482 :: 		do {
...
;Main.c,493 :: 		report_alarm_message(ALARM_ABORT_CYCLE);
; rt_exec start address is: 16 (R4)
SB	R4, 8(SP)
ORI	R25, R0, 65534
JAL	_report_alarm_message+0
...
;Main.c,495 :: 		bit_false(sys.execute,(EXEC_ALARM | EXEC_CRIT_EVENT));
; rt_exec start address is: 16 (R4)
LBU	R3, Offset(_sys+25)(GP)
ORI	R2, R0, 159
AND	R2, R3, R2
...
NOP	
L__protocol_execute_runtime122:
;Main.c,474 :: 		if (rt_exec & (EXEC_ALARM | EXEC_CRIT_EVENT)) {
;Main.c,496 :: 		}
L_protocol_execute_runtime104:
;Main.c,499 :: 		if (rt_exec & EXEC_RESET) {
; rt_exec start address is: 16 (R4)
ANDI	R2, R4, 16
...
; rt_exec end address is: 16 (R4)
;Main.c,500 :: 		sys.abort = true;  // Only place this is set true.
ORI	R2, R0, 1
SB	R2, Offset(_sys+0)(GP)
;Main.c,501 :: 		return; // Nothing else to do but exit.
J	L_end_protocol_execute_runtime
NOP	
;Main.c,502 :: 		}
L_protocol_execute_runtime110:
;Main.c,505 :: 		if (rt_exec & EXEC_STATUS_REPORT) {
; rt_exec start address is: 16 (R4)
ANDI	R2, R4, 1
...
;Main.c,506 :: 		report_realtime_status();
SB	R4, 8(SP)
JAL	_report_realtime_status+0
NOP	
LBU	R4, 8(SP)
;Main.c,507 :: 		bit_false(sys.execute,EXEC_STATUS_REPORT);
LBU	R3, Offset(_sys+25)(GP)
ORI	R2, R0, 254
AND	R2, R3, R2
SB	R2, Offset(_sys+25)(GP)
...
;Main.c,511 :: 		if (rt_exec & EXEC_FEED_HOLD) {
ANDI	R2, R4, 8
...
;Main.c,513 :: 		bit_false(sys.execute,EXEC_FEED_HOLD);
LBU	R3, Offset(_sys+25)(GP)
ORI	R2, R0, 247
AND	R2, R3, R2
SB	R2, Offset(_sys+25)(GP)
...
;Main.c,518 :: 		if (rt_exec & EXEC_CYCLE_STOP) {
ANDI	R2, R4, 4
...
;Main.c,520 :: 		bit_false(sys.execute,EXEC_CYCLE_STOP);
LBU	R3, Offset(_sys+25)(GP)
ORI	R2, R0, 251
AND	R2, R3, R2
SB	R2, Offset(_sys+25)(GP)
...
;Main.c,523 :: 		if (rt_exec & EXEC_CYCLE_START) {
ANDI	R2, R4, 2
; rt_exec end address is: 16 (R4)
...
;Main.c,525 :: 		if (bit_istrue(settings.flags,BITFLAG_AUTO_START)) {
...
;Main.c,526 :: 		sys.auto_start = true; // Re-enable auto start after feed hold.
ORI	R2, R0, 1
SB	R2, Offset(_sys+24)(GP)
;Main.c,527 :: 		}
L_protocol_execute_runtime115:
;Main.c,528 :: 		bit_false(sys.execute,EXEC_CYCLE_START);
LBU	R3, Offset(_sys+25)(GP)
ORI	R2, R0, 253
AND	R2, R3, R2
SB	R2, Offset(_sys+25)(GP)
