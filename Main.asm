...
LHU	R2, Offset(_STPS+6)(GP)
ANDI	R4, R2, 255
;Main.c,182 :: 		run_state:= %d\t%l\t%l\t%d\t%l\t%l\t%l\n"
ADDIU	R23, SP, 4
ADDIU	R22, R23, 34
...
NOP	
ADDIU	R3, SP, 4
;Main.c,189 :: 		,gc.frequency);
LW	R2, Offset(_gc+20)(GP)
ADDIU	SP, SP, -32
SW	R2, 28(SP)
;Main.c,188 :: 		,STPS[Y].step_delay
...
;Main.c,191 :: 		}
L_main25:
;Main.c,195 :: 		protocol_system_check();
JAL	_protocol_system_check+0
NOP	
;Main.c,198 :: 		protocol_execute_runtime();
JAL	_protocol_execute_runtime+0
NOP	
;Main.c,201 :: 		status_of_gcode = Sample_Gocde_Line();
JAL	_Sample_Gocde_Line+0
NOP	
SH	R2, 0(SP)
;Main.c,213 :: 		LED1 = TMR.clock >> 4;
LBU	R2, Offset(_TMR+0)(GP)
SRL	R3, R2, 4
_LX	
INS	R2, R3, BitPos(LED1+0), 1
_SX	
;Main.c,218 :: 		if(disable_steps <= SEC_TO_DISABLE_STEPPERS)
...
NOP	
L__main194:
;Main.c,219 :: 		disable_steps = TMR.Reset(SEC_TO_DISABLE_STEPPERS,disable_steps);
...
;Main.c,231 :: 		static int Modal_Group_Actions0(int action){
ADDIU	SP, SP, -48
SW	RA, 0(SP)
;Main.c,234 :: 		unsigned int home_select = 0;
SW	R26, 4(SP)
SW	R27, 8(SP)
;Main.c,243 :: 		switch(action){
J	L_Main_Modal_Group_Actions032
NOP	
;Main.c,244 :: 		case 2:  //NON_MODAL_DWELL
L_Main_Modal_Group_Actions034:
;Main.c,245 :: 		i = 0;
; i start address is: 20 (R5)
MOVZ	R5, R0, R0
;Main.c,247 :: 		if(gc.S > 0){ //wait in seconds
...
;Main.c,250 :: 		LED2 = TMR.clock >> 1;
LBU	R2, Offset(_TMR+0)(GP)
SRL	R3, R2, 1
_LX	
INS	R2, R3, BitPos(LED2+0), 1
_SX	
;Main.c,251 :: 		Delay_ms(1);
LUI	R24, 1
ORI	R24, R24, 1130
...
;Main.c,254 :: 		}else if(gc.P > 0){ //wait in msec
...
;Main.c,255 :: 		dly_time = (unsigned long)gc.P;
LH	R2, Offset(_gc+120)(GP)
; dly_time start address is: 16 (R4)
MOVZ	R4, R2, R0
; dly_time end address is: 16 (R4)
; i end address is: 20 (R5)
;Main.c,256 :: 		while(i < dly_time){
...
;Main.c,257 :: 		LED2 = TMR.clock >> 1;
LBU	R2, Offset(_TMR+0)(GP)
SRL	R3, R2, 1
_LX	
INS	R2, R3, BitPos(LED2+0), 1
_SX	
;Main.c,258 :: 		Delay_ms(1);
LUI	R24, 1
ORI	R24, R24, 1130
...
;Main.c,469 :: 		SingleAxisStep(gc.next_position[X],gc.frequency,X);
SH	R25, 12(SP)
MOVZ	R26, R0, R0
...
;Main.c,472 :: 		SingleAxisStep(gc.next_position[Y],gc.frequency,Y);
SH	R25, 12(SP)
ORI	R26, R0, 1
LW	R25, Offset(_gc+20)(GP)
...
;Main.c,475 :: 		DualAxisStep(gc.next_position[X], gc.next_position[Y],X,Y,gc.frequency);
SH	R25, 12(SP)
...
;Main.c,478 :: 		SingleAxisStep(gc.next_position[Z],gc.frequency,Z);
SH	R25, 12(SP)
ORI	R26, R0, 2
LW	R25, Offset(_gc+20)(GP)
...
;Main.c,481 :: 		DualAxisStep(gc.next_position[X], gc.next_position[Z],X,Z,gc.frequency);
SH	R25, 12(SP)
LW	R27, Offset(_gc+20)(GP)
ORI	R26, R0, 2
MOVZ	R25, R0, R0
...
;Main.c,484 :: 		DualAxisStep(gc.next_position[Y], gc.next_position[Z],Y,Z,gc.frequency);
SH	R25, 12(SP)
LW	R27, Offset(_gc+20)(GP)
ORI	R26, R0, 2
ORI	R25, R0, 1
...
;Main.c,487 :: 		SingleAxisStep(gc.next_position[A],gc.frequency,A);
SH	R25, 12(SP)
ORI	R26, R0, 3
LW	R25, Offset(_gc+20)(GP)
...
;Main.c,490 :: 		DualAxisStep(gc.next_position[X], gc.next_position[A],X,A,gc.frequency);
SH	R25, 12(SP)
LW	R27, Offset(_gc+20)(GP)
ORI	R26, R0, 3
MOVZ	R25, R0, R0
...
;Main.c,493 :: 		DualAxisStep(gc.next_position[Y], gc.next_position[A],Y,A,gc.frequency);
SH	R25, 12(SP)
LW	R27, Offset(_gc+20)(GP)
ORI	R26, R0, 3
ORI	R25, R0, 1
...
;Main.c,496 :: 		DualAxisStep(gc.next_position[Z], gc.next_position[A],Z,A,gc.frequency);
SH	R25, 12(SP)
LW	R27, Offset(_gc+20)(GP)
ORI	R26, R0, 3
ORI	R25, R0, 2
