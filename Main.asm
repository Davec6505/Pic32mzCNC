...
;Main.c,476 :: 		sys_sync_current_position();
SH	R25, 12(SP)
JAL	_sys_sync_current_position+0
NOP	
;Main.c,477 :: 		r_or_ijk(gc.position[X],gc.position[Y],gc.next_position[X],gc.next_position[Y],gc.R,gc.I,gc.J,gc.K,X,Y,gc.DIR);
...
J	L_Main_Modal_Group_Actions188
NOP	
;Main.c,479 :: 		case ALL_AXIS://Homing X axis
L_Main_Modal_Group_Actions1100:
;Main.c,480 :: 		axis_to_home = Home(axis_to_home);
SH	R25, 12(SP)
...
;Main.c,481 :: 		LED2 = TMR.clock >> 3;
LBU	R3, Offset(_TMR+0)(GP)
SRL	R4, R3, 3
_LX	
INS	R3, R4, BitPos(LED2+0), 1
_SX	
;Main.c,486 :: 		if(axis_to_home < 2){
SEH	R2, R2
SLTI	R2, R2, 2
...
;Main.c,490 :: 		action =(ALARM_ABORT_CYCLE);
ORI	R25, R0, 65534
;Main.c,491 :: 		}
...
;Main.c,497 :: 		action = 0;
MOVZ	R25, R0, R0
;Main.c,498 :: 		for(l=0;l<NoOfAxis;l++){
...
;Main.c,501 :: 		STPS[l].steps_abs_position = 0;
SEH	R3, R5
ORI	R2, R0, 100
MULTU	R2, R3
MFLO	R3
...
ADDIU	R2, R2, 80
SW	R0, 0(R2)
;Main.c,502 :: 		sys.position[l] = STPS[l].steps_abs_position;
SEH	R2, R5
SLL	R3, R2, 2
...
LW	R2, 0(R2)
SW	R2, 0(R4)
;Main.c,505 :: 		if(STPS[l].run_state != STOP)
SEH	R3, R5
ORI	R2, R0, 100
MULTU	R2, R3
MFLO	R3
...
;Main.c,506 :: 		STPS[l].run_state = STOP;
SEH	R3, R5
ORI	R2, R0, 100
MULTU	R2, R3
MFLO	R3
...
;Main.c,510 :: 		sys_sync_current_position();
SH	R25, 12(SP)
JAL	_sys_sync_current_position+0
NOP	
LH	R25, 12(SP)
;Main.c,513 :: 		while(axis_to_home)
...
;Main.c,514 :: 		axis_to_home = Rst_Axisword();
SH	R25, 12(SP)
JAL	_Rst_Axisword+0
NOP	
LH	R25, 12(SP)
...
;Main.c,517 :: 		sys.state = STATE_IDLE;
SH	R0, Offset(_sys+2)(GP)
;Main.c,518 :: 		SV.homed = false;
LBU	R2, Offset(_SV+2)(GP)
INS	R2, R0, 2, 1
...
J	L_Main_Modal_Group_Actions188
NOP	
;Main.c,521 :: 		default: return action = 0;
...
;Main.c,532 :: 		static int Modal_Group_Actions3(int action){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Main.c,534 :: 		if(gc.inches_mode > 1)
...
;Main.c,535 :: 		FAIL(STATUS_SETTING_READ_FAIL);
SH	R25, 4(SP)
ORI	R25, R0, 10
JAL	_FAIL+0
NOP	
...
;Main.c,543 :: 		static int Modal_Group_Actions4(int action){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Main.c,549 :: 		gc.program_flow > PROGRAM_FLOW_COMPLETED)
...
;Main.c,550 :: 		FAIL(STATUS_INVALID_STATEMENT);
SH	R25, 4(SP)
ORI	R25, R0, 6
JAL	_FAIL+0
NOP	
...
;Main.c,558 :: 		static int Modal_Group_Actions7(int action){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Main.c,563 :: 		if(gc.spindle_direction < -1 || gc.spindle_direction > 1)
...
;Main.c,572 :: 		static int Modal_Group_Actions12(int action){
