...
SLTI	R2, R2, 1
BEQ	R2, R0, L_Main_Modal_Group_Actions0138
NOP	
...
SEH	R2, R4
SLT	R2, R3, R2
BNE	R2, R0, L_Main_Modal_Group_Actions0139
NOP	
...
SLTI	R2, R2, 1
BEQ	R2, R0, L_Main_Modal_Group_Actions0140
NOP	
...
SEH	R2, R4
SLT	R2, R3, R2
BNE	R2, R0, L_Main_Modal_Group_Actions0141
NOP	
...
ORI	R2, R0, 2
BNE	R3, R2, L_Main_Modal_Group_Actions0143
NOP	
J	L_Main_Modal_Group_Actions0113
NOP	
L_Main_Modal_Group_Actions0143:
LH	R3, Offset(_gc+18)(GP)
ORI	R2, R0, 20
BNE	R3, R2, L_Main_Modal_Group_Actions0145
NOP	
J	L_Main_Modal_Group_Actions0112
NOP	
L_Main_Modal_Group_Actions0145:
...
LH	R3, Offset(_gc+18)(GP)
ORI	R2, R0, 20
BEQ	R3, R2, L_Main_Modal_Group_Actions0146
NOP	
...
BNE	R2, R0, L_Main_Modal_Group_Actions0148
NOP	
...
SLTI	R2, R2, 1
BEQ	R2, R0, L_Main_Modal_Group_Actions0149
NOP	
...
SEH	R2, R6
SLTI	R2, R2, 3
BNE	R2, R0, L_Main_Modal_Group_Actions0150
NOP	
...
SEH	R2, R2
BEQ	R2, R0, L_Main_Modal_Group_Actions0151
NOP	
...
BNE	R2, R0, L_Main_Modal_Group_Actions0154
NOP	
...
;Main.c,314 :: 		axis_words = Get_Axisword();
SH	R25, 12(SP)
JAL	_Get_Axisword+0
NOP	
LH	R25, 12(SP)
; axis_words start address is: 20 (R5)
SEH	R5, R2
;Main.c,318 :: 		for (i=0; i<=2; i++) { // Axes indices are consistent, so loop may be used to save flash space.
; i start address is: 24 (R6)
MOVZ	R6, R0, R0
; i end address is: 24 (R6)
...
; axis_words end address is: 20 (R5)
;Main.c,319 :: 		if ( bit_istrue(axis_words,bit(i)) ) {
; axis_words start address is: 20 (R5)
SEH	R3, R6
ORI	R2, R0, 1
SLLV	R2, R2, R3
...
;Main.c,320 :: 		if (!gc.absolute_override) {
...
;Main.c,321 :: 		if (!gc.absolute_mode) { // Do not update target in absolute override mode
LBU	R2, Offset(_gc+5)(GP)
...
;Main.c,322 :: 		gc.next_position[i] += gc.coord_system[i] + gc.coord_offset[i]; // Absolute mode
SEH	R2, R6
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+72)
ORI	R2, R2, lo_addr(_gc+72)
...
;Main.c,324 :: 		gc.next_position[i] += gc.position[i]; // Incremental mode
SEH	R2, R6
SLL	R3, R2, 2
LUI	R2, hi_addr(_gc+72)
ORI	R2, R2, lo_addr(_gc+72)
...
;Main.c,327 :: 		gc.next_position[i] = gc.position[i]; // No axis word in block. Keep same axis position.
SEH	R2, R6
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+72)
ORI	R2, R2, lo_addr(_gc+72)
...
J	L_Main_Modal_Group_Actions015
NOP	
;Main.c,332 :: 		case 64:   //NON_MODAL_SET_HOME_1
L_Main_Modal_Group_Actions070:
;Main.c,333 :: 		temp = SETTING_INDEX_G28;
; temp start address is: 16 (R4)
MOVZ	R4, R0, R0
;Main.c,334 :: 		if (action == NON_MODAL_SET_HOME_1_BIT) { temp = SETTING_INDEX_G30; }
SEH	R3, R25
ORI	R2, R0, 64
...
;Main.c,335 :: 		settings_write_coord_data(temp,gc.position);
; temp start address is: 8 (R2)
SH	R25, 12(SP)
...
J	L_Main_Modal_Group_Actions015
NOP	
;Main.c,337 :: 		case 128:  //NON_MODAL_SET_COORDINATE_OFFSET
...
; axis_words start address is: 20 (R5)
SEH	R5, R2
;Main.c,339 :: 		if (!axis_words) { // No axis words
...
; axis_words end address is: 20 (R5)
;Main.c,340 :: 		FAIL(STATUS_INVALID_STATEMENT);
ORI	R2, R0, 6
...
;Main.c,344 :: 		for (i=0; i<=2; i++) { // Axes indices are consistent, so loop may be used.
; i start address is: 24 (R6)
; axis_words start address is: 20 (R5)
MOVZ	R6, R0, R0
; i end address is: 24 (R6)
...
;Main.c,346 :: 		gc.coord_offset[i] = gc.position[i]-gc.coord_system[i]-gc.next_position[i];
SEH	R2, R6
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+56)
ORI	R2, R2, lo_addr(_gc+56)
...
;Main.c,351 :: 		case 256: //NON_MODAL_RESET_COORDINATE_OFFSET
...
J	L_Main_Modal_Group_Actions015
NOP	
;Main.c,355 :: 		default: action = -1; //error in action msg ???
...
SEH	R3, R25
ORI	R2, R0, 2
BNE	R3, R2, L_Main_Modal_Group_Actions0177
NOP	
...
;Main.c,364 :: 		static int Modal_Group_Actions1(int action){
ADDIU	SP, SP, -16
SW	RA, 0(SP)
;Main.c,365 :: 		switch(action){
SW	R26, 4(SP)
SW	R27, 8(SP)
J	L_Main_Modal_Group_Actions182
NOP	
;Main.c,366 :: 		case 1: //b0000 0001
L_Main_Modal_Group_Actions184:
;Main.c,367 :: 		SingleAxisStep(gc.next_position[X],gc.frequency,X);
SH	R25, 12(SP)
MOVZ	R26, R0, R0
...
NOP	
;Main.c,369 :: 		case 2://b0000 0010
L_Main_Modal_Group_Actions185:
;Main.c,370 :: 		SingleAxisStep(gc.next_position[Y],gc.frequency,Y);
SH	R25, 12(SP)
ORI	R26, R0, 1
LH	R25, Offset(_gc+16)(GP)
...
J	L_Main_Modal_Group_Actions183
NOP	
;Main.c,372 :: 		case 3://b0000 0011
L_Main_Modal_Group_Actions186:
;Main.c,375 :: 		DualAxisStep(gc.next_position[X], gc.next_position[Y],X,Y,gc.frequency);
SH	R25, 12(SP)
...
J	L_Main_Modal_Group_Actions183
NOP	
;Main.c,379 :: 		case 4://b0000 0100
L_Main_Modal_Group_Actions187:
;Main.c,380 :: 		SingleAxisStep(gc.next_position[Z],gc.frequency,Z);
SH	R25, 12(SP)
ORI	R26, R0, 2
LH	R25, Offset(_gc+16)(GP)
...
J	L_Main_Modal_Group_Actions183
NOP	
;Main.c,382 :: 		case 5://b0000 0101
L_Main_Modal_Group_Actions188:
;Main.c,383 :: 		DualAxisStep(gc.next_position[X], gc.next_position[Z],X,Z,gc.frequency);
SH	R25, 12(SP)
LH	R27, Offset(_gc+16)(GP)
ORI	R26, R0, 2
MOVZ	R25, R0, R0
...
J	L_Main_Modal_Group_Actions183
NOP	
;Main.c,385 :: 		case 6://b0000 0110
L_Main_Modal_Group_Actions189:
;Main.c,386 :: 		DualAxisStep(gc.next_position[Y], gc.next_position[Z],Y,Z,gc.frequency);
SH	R25, 12(SP)
LH	R27, Offset(_gc+16)(GP)
ORI	R26, R0, 2
ORI	R25, R0, 1
...
J	L_Main_Modal_Group_Actions183
NOP	
;Main.c,388 :: 		case 8://b0000 1000
L_Main_Modal_Group_Actions190:
;Main.c,389 :: 		SingleAxisStep(gc.next_position[A],gc.frequency,A);
SH	R25, 12(SP)
ORI	R26, R0, 3
LH	R25, Offset(_gc+16)(GP)
...
J	L_Main_Modal_Group_Actions183
NOP	
;Main.c,391 :: 		case 9://b0000 1001
L_Main_Modal_Group_Actions191:
;Main.c,392 :: 		DualAxisStep(gc.next_position[X], gc.next_position[A],X,A,gc.frequency);
SH	R25, 12(SP)
LH	R27, Offset(_gc+16)(GP)
ORI	R26, R0, 3
MOVZ	R25, R0, R0
...
J	L_Main_Modal_Group_Actions183
NOP	
;Main.c,394 :: 		case 10://b0000 1010
L_Main_Modal_Group_Actions192:
;Main.c,395 :: 		DualAxisStep(gc.next_position[Y], gc.next_position[A],Y,A,gc.frequency);
SH	R25, 12(SP)
LH	R27, Offset(_gc+16)(GP)
ORI	R26, R0, 3
ORI	R25, R0, 1
...
J	L_Main_Modal_Group_Actions183
NOP	
;Main.c,397 :: 		case 12://b0000 1100
L_Main_Modal_Group_Actions193:
;Main.c,398 :: 		DualAxisStep(gc.next_position[Z], gc.next_position[A],Z,A,gc.frequency);
SH	R25, 12(SP)
LH	R27, Offset(_gc+16)(GP)
ORI	R26, R0, 3
ORI	R25, R0, 2
...
J	L_Main_Modal_Group_Actions183
NOP	
;Main.c,400 :: 		case 13://Homing X axis
...
J	L_Main_Modal_Group_Actions183
NOP	
;Main.c,403 :: 		case 14://Homing Y axis
...
J	L_Main_Modal_Group_Actions183
NOP	
;Main.c,406 :: 		case 15://Homing Y axis
L_Main_Modal_Group_Actions196:
;Main.c,407 :: 		r_or_ijk(150.00, 30.00, 150.00, 30.00, 0.00, -50.00, 50.00,0.00,X,Y,CW);
LUI	R5, 16880
ORI	R5, R5, 0
LUI	R4, 17174
ORI	R4, R4, 0
...
;Main.c,420 :: 		static int Modal_Group_Actions3(int action){
...
;Main.c,431 :: 		static int Modal_Group_Actions4(int action){
...
;Main.c,442 :: 		static int Modal_Group_Actions7(int action){
...
;Main.c,453 :: 		static int Modal_Group_Actions12(int action){
...
;Main.c,477 :: 		void protocol_execute_runtime(){
ADDIU	SP, SP, -12
SW	RA, 0(SP)
;Main.c,478 :: 		if (sys.execute) { // Enter only if any bit flag is true
SW	R25, 4(SP)
...
;Main.c,479 :: 		uint8_t rt_exec = sys.execute; // Avoid calling volatile multiple times
; rt_exec start address is: 16 (R4)
LBU	R4, Offset(_sys+25)(GP)
;Main.c,484 :: 		if (rt_exec & (EXEC_ALARM | EXEC_CRIT_EVENT)) {
ANDI	R2, R4, 96
...
;Main.c,485 :: 		sys.state = STATE_ALARM; // Set system alarm state
ORI	R2, R0, 6
SB	R2, Offset(_sys+1)(GP)
;Main.c,488 :: 		if (rt_exec & EXEC_CRIT_EVENT) {
ANDI	R2, R4, 64
...
;Main.c,489 :: 		report_alarm_message(ALARM_HARD_LIMIT);
SB	R4, 8(SP)
ORI	R25, R0, 65535
JAL	_report_alarm_message+0
NOP	
;Main.c,490 :: 		report_feedback_message(MESSAGE_CRITICAL_EVENT);
ORI	R25, R0, 1
JAL	_report_feedback_message+0
NOP	
LBU	R4, 8(SP)
;Main.c,491 :: 		bit_false(sys.execute,EXEC_RESET); // Disable any existing reset
...
NOP	
L__protocol_execute_runtime116:
;Main.c,496 :: 		} while (bit_isfalse(sys.execute,EXEC_RESET));
;Main.c,492 :: 		do {
...
NOP	
J	L__protocol_execute_runtime116
NOP	
...
;Main.c,503 :: 		report_alarm_message(ALARM_ABORT_CYCLE);
; rt_exec start address is: 16 (R4)
SB	R4, 8(SP)
ORI	R25, R0, 65534
JAL	_report_alarm_message+0
...
;Main.c,505 :: 		bit_false(sys.execute,(EXEC_ALARM | EXEC_CRIT_EVENT));
; rt_exec start address is: 16 (R4)
LBU	R3, Offset(_sys+25)(GP)
ORI	R2, R0, 159
AND	R2, R3, R2
...
;Main.c,509 :: 		if (rt_exec & EXEC_RESET) {
; rt_exec start address is: 16 (R4)
ANDI	R2, R4, 16
...
; rt_exec end address is: 16 (R4)
;Main.c,510 :: 		sys.abort = true;  // Only place this is set true.
ORI	R2, R0, 1
SB	R2, Offset(_sys+0)(GP)
;Main.c,511 :: 		return; // Nothing else to do but exit.
J	L_end_protocol_execute_runtime
NOP	
;Main.c,512 :: 		}
L_protocol_execute_runtime105:
;Main.c,515 :: 		if (rt_exec & EXEC_STATUS_REPORT) {
; rt_exec start address is: 16 (R4)
ANDI	R2, R4, 1
...
;Main.c,516 :: 		report_realtime_status();
SB	R4, 8(SP)
JAL	_report_realtime_status+0
NOP	
LBU	R4, 8(SP)
;Main.c,517 :: 		bit_false(sys.execute,EXEC_STATUS_REPORT);
LBU	R3, Offset(_sys+25)(GP)
ORI	R2, R0, 254
AND	R2, R3, R2
SB	R2, Offset(_sys+25)(GP)
...
;Main.c,521 :: 		if (rt_exec & EXEC_FEED_HOLD) {
ANDI	R2, R4, 8
...
;Main.c,523 :: 		bit_false(sys.execute,EXEC_FEED_HOLD);
LBU	R3, Offset(_sys+25)(GP)
ORI	R2, R0, 247
AND	R2, R3, R2
SB	R2, Offset(_sys+25)(GP)
...
;Main.c,528 :: 		if (rt_exec & EXEC_CYCLE_STOP) {
ANDI	R2, R4, 4
...
;Main.c,530 :: 		bit_false(sys.execute,EXEC_CYCLE_STOP);
LBU	R3, Offset(_sys+25)(GP)
ORI	R2, R0, 251
AND	R2, R3, R2
SB	R2, Offset(_sys+25)(GP)
...
;Main.c,533 :: 		if (rt_exec & EXEC_CYCLE_START) {
ANDI	R2, R4, 2
; rt_exec end address is: 16 (R4)
...
;Main.c,535 :: 		if (bit_istrue(settings.flags,BITFLAG_AUTO_START)) {
...
;Main.c,536 :: 		sys.auto_start = true; // Re-enable auto start after feed hold.
ORI	R2, R0, 1
SB	R2, Offset(_sys+24)(GP)
;Main.c,537 :: 		}
L_protocol_execute_runtime110:
;Main.c,538 :: 		bit_false(sys.execute,EXEC_CYCLE_START);
LBU	R3, Offset(_sys+25)(GP)
ORI	R2, R0, 253
AND	R2, R3, R2
SB	R2, Offset(_sys+25)(GP)
