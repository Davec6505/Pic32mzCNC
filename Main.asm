...
L__main142:
L__main141:
;Main.c,189 :: 		protocol_system_check();
JAL	_protocol_system_check+0
NOP	
;Main.c,192 :: 		protocol_execute_runtime();
...
NOP	
SH	R2, 0(SP)
;Main.c,207 :: 		LED1 = TMR.clock >> 4;
LBU	R2, Offset(_TMR+0)(GP)
SRL	R3, R2, 4
_LX	
INS	R2, R3, BitPos(LED1+0), 1
_SX	
;Main.c,212 :: 		if(disable_steps <= SEC_TO_DISABLE_STEPPERS)
...
NOP	
L__main180:
;Main.c,213 :: 		disable_steps = TMR.Reset(SEC_TO_DISABLE_STEPPERS,disable_steps);
...
;Main.c,225 :: 		static int Modal_Group_Actions0(int action){
ADDIU	SP, SP, -36
SW	RA, 0(SP)
;Main.c,228 :: 		unsigned int home_select = 0;
SW	R26, 4(SP)
SW	R27, 8(SP)
;Main.c,237 :: 		switch(action){
J	L_Main_Modal_Group_Actions026
NOP	
;Main.c,238 :: 		case 2:  //NON_MODAL_DWELL
L_Main_Modal_Group_Actions028:
;Main.c,239 :: 		i = 0;
; i start address is: 20 (R5)
MOVZ	R5, R0, R0
;Main.c,241 :: 		if(gc.S > 0){ //wait in seconds
LH	R2, Offset(_gc+90)(GP)
SLTI	R2, R2, 1
BEQ	R2, R0, L_Main_Modal_Group_Actions0183
NOP	
J	L_Main_Modal_Group_Actions029
NOP	
L_Main_Modal_Group_Actions0183:
;Main.c,242 :: 		dly_time = gc.S * 1000;
...
;Main.c,248 :: 		}else if(gc.P > 0){ //wait in msec
...
;Main.c,249 :: 		dly_time = (unsigned long)gc.P;
LH	R2, Offset(_gc+88)(GP)
; dly_time start address is: 16 (R4)
MOVZ	R4, R2, R0
; dly_time end address is: 16 (R4)
; i end address is: 20 (R5)
;Main.c,250 :: 		while(i < dly_time){
...
NOP	
;Main.c,258 :: 		case 4:  //NON_MODAL_SET_COORDINATE_DATA
L_Main_Modal_Group_Actions040:
;Main.c,271 :: 		if(gc.L != 2 && gc.L != 20)
...
;Main.c,275 :: 		result = settings_write_coord_data((int)gc.P,gc.next_position );
SH	R25, 12(SP)
...
NOP	
LH	R25, 12(SP)
;Main.c,277 :: 		if(result){ //response if write to flash failed new result
BNE	R2, R0, L_Main_Modal_Group_Actions0193
NOP	
J	L_Main_Modal_Group_Actions045
NOP	
L_Main_Modal_Group_Actions0193:
;Main.c,278 :: 		return NVM_COORDINATE_WRITE_ERROR;
ORI	R2, R0, 1
J	L_end_Modal_Group_Actions0
NOP	
;Main.c,279 :: 		}
L_Main_Modal_Group_Actions045:
;Main.c,282 :: 		if (gc.coord_select > 0) {
...
;Main.c,283 :: 		memcpy(gc.coord_system,gc.next_position,sizeof(gc.next_position));
SH	R25, 12(SP)
ORI	R27, R0, 8
LUI	R26, hi_addr(_gc+52)
ORI	R26, R26, lo_addr(_gc+52)
...
;Main.c,292 :: 		temp = indx = (gc.P-1) & 0xFF;
LH	R2, Offset(_gc+88)(GP)
ADDIU	R2, R2, -1
ANDI	R2, R2, 255
;Main.c,293 :: 		indx *= 4;
...
SEH	R3, R2
;Main.c,294 :: 		axis_cnt = 0;
; axis_cnt start address is: 16 (R4)
MOVZ	R4, R0, R0
;Main.c,298 :: 		axis_words = Get_Axisword();
SH	R4, 12(SP)
SH	R3, 14(SP)
SH	R25, 16(SP)
JAL	_Get_Axisword+0
...
; axis_words end address is: 20 (R5)
;Main.c,300 :: 		temp_axis = (axis_words >> i) & 1;
; axis_words start address is: 20 (R5)
SEH	R3, R5
SEH	R2, R6
SRAV	R2, R3, R2
ANDI	R2, R2, 1
;Main.c,302 :: 		if(temp_axis == 0){
SEH	R2, R2
...
; axis_cnt start address is: 16 (R4)
SEH	R4, R2
;Main.c,304 :: 		if(axis_cnt > 2)break;
SEH	R2, R2
SLTI	R2, R2, 3
...
MOVZ	R8, R2, R0
;Main.c,310 :: 		coord_data[i] = ulong2flt(_flash);
ADDIU	R3, SP, 24
SEH	R2, R6
SLL	R2, R2, 2
ADDU	R2, R3, R2
...
;Main.c,321 :: 		coord_data[i] = gc.next_position[i];
; axis_cnt start address is: 32 (R8)
ADDIU	R3, SP, 24
SEH	R2, R6
SLL	R4, R2, 2
...
;Main.c,333 :: 		result = settings_write_coord_data((int)gc.P,coord_data);
ADDIU	R2, SP, 24
SH	R25, 12(SP)
MOVZ	R26, R2, R0
LH	R25, Offset(_gc+88)(GP)
JAL	_settings_write_coord_data+0
NOP	
;Main.c,336 :: 		memcpy(gc.coord_system,coord_data,sizeof(coord_data));
ADDIU	R2, SP, 24
ORI	R27, R0, 8
MOVZ	R26, R2, R0
LUI	R25, hi_addr(_gc+36)
...
J	L_Main_Modal_Group_Actions027
NOP	
;Main.c,340 :: 		case  8:  //NON_MODAL_GO_HOME_0_BIT
L_Main_Modal_Group_Actions054:
;Main.c,341 :: 		case 32:  //NON_MODAL_GO_HOME_1_BIT
L_Main_Modal_Group_Actions055:
;Main.c,344 :: 		axis_words = Get_Axisword();
SH	R25, 12(SP)
JAL	_Get_Axisword+0
NOP	
LH	R25, 12(SP)
...
;Main.c,358 :: 		if (gc.absolute_mode) {
...
;Main.c,359 :: 		gc.next_position[i] += gc.coord_system[i] + gc.coord_offset[i];
SEH	R2, R5
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+52)
ORI	R2, R2, lo_addr(_gc+52)
...
;Main.c,361 :: 		gc.next_position[i] += gc.position[i];
SEH	R2, R5
SLL	R3, R2, 2
LUI	R2, hi_addr(_gc+52)
ORI	R2, R2, lo_addr(_gc+52)
...
;Main.c,364 :: 		gc.next_position[i] = gc.position[i];
SEH	R2, R5
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+52)
ORI	R2, R2, lo_addr(_gc+52)
...
;Main.c,370 :: 		SingleAxisStep(gc.next_position[i],settings.default_seek_rate,i);
...
SEH	R4, R5
SEH	R3, R6
;Main.c,371 :: 		while(GET_RunState(i));
...
;Main.c,375 :: 		temp = SETTING_INDEX_G28;  //home to zero pos / at limits
; temp start address is: 24 (R6)
ORI	R6, R0, 10
;Main.c,377 :: 		if (action == NON_MODAL_GO_HOME_1_BIT){temp = SETTING_INDEX_G30;}
SEH	R3, R25
ORI	R2, R0, 32
...
;Main.c,378 :: 		i = (temp)*4 ; //place the new data into the correct position
; temp start address is: 24 (R6)
SEH	R2, R6
SLL	R2, R2, 2
; i start address is: 16 (R4)
SEH	R4, R2
;Main.c,381 :: 		for(j = 0;j<4;j++){
...
MOVZ	R7, R2, R0
;Main.c,383 :: 		coord_system[temp].coord[j] = ulong2flt(_data);
SEH	R2, R6
SLL	R3, R2, 4
...
SEH	R7, R2
;Main.c,391 :: 		SingleAxisStep(coord_system[temp].coord[j],settings.default_seek_rate,j);
LWC1	S0, Offset(_settings+12)(GP)
CVT36.S 	S0, S0
MFC1	R4, S0
SEH	R2, R6
...
SEH	R4, R6
SEH	R3, R7
;Main.c,392 :: 		while(GET_RunState(j));
...
;Main.c,398 :: 		home_select = SETTING_INDEX_G28;
; home_select start address is: 16 (R4)
ORI	R4, R0, 10
;Main.c,399 :: 		if (action == NON_MODAL_SET_HOME_1) { home_select = SETTING_INDEX_G30; }
SEH	R3, R25
ORI	R2, R0, 6
...
;Main.c,400 :: 		settings_write_coord_data(home_select,gc.position);
; home_select start address is: 8 (R2)
SH	R25, 12(SP)
...
;Main.c,403 :: 		axis_words = Get_Axisword();
SH	R25, 12(SP)
JAL	_Get_Axisword+0
NOP	
LH	R25, 12(SP)
...
; axis_words end address is: 20 (R5)
;Main.c,409 :: 		if ( bit_istrue(axis_words,bit(i)) ) {
; axis_words start address is: 20 (R5)
SEH	R3, R6
ORI	R2, R0, 1
SLLV	R2, R2, R3
...
;Main.c,410 :: 		if (!gc.absolute_override) {
...
;Main.c,411 :: 		if (!gc.absolute_mode) { // Do not update target in absolute override mode
LBU	R2, Offset(_gc+5)(GP)
BEQ	R2, R0, L_Main_Modal_Group_Actions0216
NOP	
J	L_Main_Modal_Group_Actions080
NOP	
L_Main_Modal_Group_Actions0216:
;Main.c,412 :: 		gc.next_position[i] += gc.coord_system[i] + gc.coord_offset[i]; // Absolute mode
SEH	R2, R6
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+52)
ORI	R2, R2, lo_addr(_gc+52)
...
;Main.c,414 :: 		gc.next_position[i] += gc.position[i]; // Incremental mode
SEH	R2, R6
SLL	R3, R2, 2
LUI	R2, hi_addr(_gc+52)
ORI	R2, R2, lo_addr(_gc+52)
...
;Main.c,417 :: 		gc.next_position[i] = gc.position[i]; // No axis word in block. Keep same axis position.
SEH	R2, R6
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+52)
ORI	R2, R2, lo_addr(_gc+52)
...
J	L_Main_Modal_Group_Actions027
NOP	
;Main.c,422 :: 		case 64:   //NON_MODAL_SET_HOME_1
L_Main_Modal_Group_Actions083:
;Main.c,423 :: 		temp = SETTING_INDEX_G28;
; temp start address is: 16 (R4)
ORI	R4, R0, 10
;Main.c,424 :: 		if (action == NON_MODAL_SET_HOME_1_BIT) { temp = SETTING_INDEX_G30; }
SEH	R3, R25
ORI	R2, R0, 64
...
;Main.c,425 :: 		settings_write_coord_data(temp,gc.position);
; temp start address is: 8 (R2)
SH	R25, 12(SP)
LUI	R26, hi_addr(_gc+28)
ORI	R26, R26, lo_addr(_gc+28)
...
J	L_Main_Modal_Group_Actions027
NOP	
;Main.c,427 :: 		case 128:  //NON_MODAL_SET_COORDINATE_OFFSET NOT WRITTEN TO FLASH LOST ON RESET
L_Main_Modal_Group_Actions085:
;Main.c,428 :: 		axis_words = Get_Axisword();
SH	R25, 12(SP)
JAL	_Get_Axisword+0
NOP	
LH	R25, 12(SP)
; axis_words start address is: 20 (R5)
SEH	R5, R2
;Main.c,430 :: 		if (!axis_words) { // No axis words
...
; axis_words end address is: 20 (R5)
;Main.c,431 :: 		FAIL(STATUS_INVALID_STATEMENT);
SH	R25, 12(SP)
ORI	R25, R0, 6
JAL	_FAIL+0
NOP	
...
;Main.c,436 :: 		for (i=0; i<=2; i++) { // Axes indices are consistent, so loop may be used.
; i start address is: 24 (R6)
; axis_words start address is: 20 (R5)
MOVZ	R6, R0, R0
; i end address is: 24 (R6)
...
; axis_words end address is: 20 (R5)
;Main.c,437 :: 		if (bit_istrue(axis_words,bit(i)) ) {
; axis_words start address is: 20 (R5)
SEH	R3, R6
ORI	R2, R0, 1
SLLV	R2, R2, R3
...
;Main.c,438 :: 		gc.coord_offset[i] = gc.position[i]-gc.coord_system[i]-gc.next_position[i];
SEH	R2, R6
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+44)
ORI	R2, R2, lo_addr(_gc+44)
...
;Main.c,443 :: 		case 256: //NON_MODAL_RESET_COORDINATE_OFFSET
L_Main_Modal_Group_Actions092:
;Main.c,445 :: 		clear_vector(gc.coord_offset);
SH	R25, 12(SP)
ORI	R27, R0, 8
MOVZ	R26, R0, R0
...
J	L_Main_Modal_Group_Actions027
NOP	
;Main.c,447 :: 		default: action = -1; //error in action msg ???
...
;Main.c,456 :: 		static int Modal_Group_Actions1(int action){
ADDIU	SP, SP, -16
SW	RA, 0(SP)
;Main.c,461 :: 		switch(action){
SW	R26, 4(SP)
SW	R27, 8(SP)
J	L_Main_Modal_Group_Actions194
NOP	
;Main.c,462 :: 		case 1: //b0000 0001
L_Main_Modal_Group_Actions196:
;Main.c,463 :: 		SingleAxisStep(gc.next_position[X],gc.frequency,X);
SH	R25, 12(SP)
MOVZ	R26, R0, R0
...
NOP	
;Main.c,465 :: 		case 2://b0000 0010
L_Main_Modal_Group_Actions197:
;Main.c,466 :: 		SingleAxisStep(gc.next_position[Y],gc.frequency,Y);
SH	R25, 12(SP)
ORI	R26, R0, 1
LW	R25, Offset(_gc+20)(GP)
...
J	L_Main_Modal_Group_Actions195
NOP	
;Main.c,468 :: 		case 3://b0000 0011
L_Main_Modal_Group_Actions198:
;Main.c,469 :: 		DualAxisStep(gc.next_position[X], gc.next_position[Y],X,Y,gc.frequency);
SH	R25, 12(SP)
...
J	L_Main_Modal_Group_Actions195
NOP	
;Main.c,471 :: 		case 4://b0000 0100
L_Main_Modal_Group_Actions199:
;Main.c,472 :: 		SingleAxisStep(gc.next_position[Z],gc.frequency,Z);
SH	R25, 12(SP)
ORI	R26, R0, 2
LW	R25, Offset(_gc+20)(GP)
...
J	L_Main_Modal_Group_Actions195
NOP	
;Main.c,474 :: 		case 5://b0000 0101
L_Main_Modal_Group_Actions1100:
;Main.c,475 :: 		DualAxisStep(gc.next_position[X], gc.next_position[Z],X,Z,gc.frequency);
SH	R25, 12(SP)
LW	R27, Offset(_gc+20)(GP)
ORI	R26, R0, 2
MOVZ	R25, R0, R0
...
J	L_Main_Modal_Group_Actions195
NOP	
;Main.c,477 :: 		case 6://b0000 0110
L_Main_Modal_Group_Actions1101:
;Main.c,478 :: 		DualAxisStep(gc.next_position[Y], gc.next_position[Z],Y,Z,gc.frequency);
SH	R25, 12(SP)
LW	R27, Offset(_gc+20)(GP)
ORI	R26, R0, 2
ORI	R25, R0, 1
...
J	L_Main_Modal_Group_Actions195
NOP	
;Main.c,480 :: 		case 8://b0000 1000
L_Main_Modal_Group_Actions1102:
;Main.c,481 :: 		SingleAxisStep(gc.next_position[A],gc.frequency,A);
SH	R25, 12(SP)
ORI	R26, R0, 3
LW	R25, Offset(_gc+20)(GP)
...
J	L_Main_Modal_Group_Actions195
NOP	
;Main.c,483 :: 		case 9://b0000 1001
L_Main_Modal_Group_Actions1103:
;Main.c,484 :: 		DualAxisStep(gc.next_position[X], gc.next_position[A],X,A,gc.frequency);
SH	R25, 12(SP)
LW	R27, Offset(_gc+20)(GP)
ORI	R26, R0, 3
MOVZ	R25, R0, R0
...
J	L_Main_Modal_Group_Actions195
NOP	
;Main.c,486 :: 		case 10://b0000 1010
L_Main_Modal_Group_Actions1104:
;Main.c,487 :: 		DualAxisStep(gc.next_position[Y], gc.next_position[A],Y,A,gc.frequency);
SH	R25, 12(SP)
LW	R27, Offset(_gc+20)(GP)
ORI	R26, R0, 3
ORI	R25, R0, 1
...
J	L_Main_Modal_Group_Actions195
NOP	
;Main.c,489 :: 		case 12://b0000 1100
L_Main_Modal_Group_Actions1105:
;Main.c,490 :: 		DualAxisStep(gc.next_position[Z], gc.next_position[A],Z,A,gc.frequency);
SH	R25, 12(SP)
LW	R27, Offset(_gc+20)(GP)
ORI	R26, R0, 3
ORI	R25, R0, 2
...
J	L_Main_Modal_Group_Actions195
NOP	
;Main.c,492 :: 		case 15://Circle interpolation
L_Main_Modal_Group_Actions1106:
;Main.c,493 :: 		sys_sync_current_position();
SH	R25, 12(SP)
JAL	_sys_sync_current_position+0
NOP	
;Main.c,494 :: 		r_or_ijk(gc.position[X],gc.position[Y],gc.next_position[X],gc.next_position[Y],gc.R,gc.I,gc.J,gc.K,X,Y,gc.DIR);
...
J	L_Main_Modal_Group_Actions195
NOP	
;Main.c,496 :: 		case ALL_AXIS://Homing X axis
L_Main_Modal_Group_Actions1107:
;Main.c,497 :: 		axis_to_home = Home(axis_to_home);
SH	R25, 12(SP)
...
;Main.c,498 :: 		LED2 = TMR.clock >> 3;
LBU	R3, Offset(_TMR+0)(GP)
SRL	R4, R3, 3
_LX	
INS	R3, R4, BitPos(LED2+0), 1
_SX	
;Main.c,503 :: 		if(axis_to_home < 2){
SEH	R2, R2
SLTI	R2, R2, 2
...
;Main.c,507 :: 		action =(ALARM_ABORT_CYCLE);
ORI	R25, R0, 65534
;Main.c,508 :: 		}
...
;Main.c,514 :: 		action = 0;
MOVZ	R25, R0, R0
;Main.c,515 :: 		for(l=0;l<NoOfAxis;l++){
...
;Main.c,518 :: 		STPS[l].steps_abs_position = 0;
SEH	R3, R5
ORI	R2, R0, 100
MULTU	R2, R3
MFLO	R3
...
ADDIU	R2, R2, 80
SW	R0, 0(R2)
;Main.c,519 :: 		sys.position[l] = STPS[l].steps_abs_position;
SEH	R2, R5
SLL	R3, R2, 2
...
LW	R2, 0(R2)
SW	R2, 0(R4)
;Main.c,522 :: 		if(STPS[l].run_state != STOP)
SEH	R3, R5
ORI	R2, R0, 100
MULTU	R2, R3
MFLO	R3
...
LHU	R2, 0(R2)
ANDI	R2, R2, 65535
BNE	R2, R0, L_Main_Modal_Group_Actions1246
NOP	
J	L_Main_Modal_Group_Actions1114
NOP	
L_Main_Modal_Group_Actions1246:
;Main.c,523 :: 		STPS[l].run_state = STOP;
SEH	R3, R5
ORI	R2, R0, 100
MULTU	R2, R3
MFLO	R3
...
;Main.c,527 :: 		sys_sync_current_position();
SH	R25, 12(SP)
JAL	_sys_sync_current_position+0
NOP	
LH	R25, 12(SP)
;Main.c,530 :: 		while(axis_to_home)
...
;Main.c,531 :: 		axis_to_home = Rst_Axisword();
SH	R25, 12(SP)
JAL	_Rst_Axisword+0
NOP	
LH	R25, 12(SP)
...
;Main.c,534 :: 		sys.state = STATE_IDLE;
SH	R0, Offset(_sys+2)(GP)
;Main.c,535 :: 		SV.homed = false;
LBU	R2, Offset(_SV+2)(GP)
INS	R2, R0, 2, 1
...
J	L_Main_Modal_Group_Actions195
NOP	
;Main.c,538 :: 		default: return action = 0;
...
;Main.c,549 :: 		static int Modal_Group_Actions3(int action){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Main.c,551 :: 		if(gc.inches_mode > 1)
LBU	R2, Offset(_gc+3)(GP)
SLTIU	R2, R2, 2
BEQ	R2, R0, L_Main_Modal_Group_Actions3274
NOP	
J	L_Main_Modal_Group_Actions3118
NOP	
L_Main_Modal_Group_Actions3274:
;Main.c,552 :: 		FAIL(STATUS_SETTING_READ_FAIL);
SH	R25, 4(SP)
ORI	R25, R0, 10
JAL	_FAIL+0
NOP	
...
;Main.c,560 :: 		static int Modal_Group_Actions4(int action){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Main.c,566 :: 		gc.program_flow > PROGRAM_FLOW_COMPLETED)
LBU	R2, Offset(_gc+9)(GP)
SLTIU	R2, R2, 0
BEQ	R2, R0, L_Main_Modal_Group_Actions4276
NOP	
...
LBU	R2, Offset(_gc+9)(GP)
SLTIU	R2, R2, 3
BNE	R2, R0, L_Main_Modal_Group_Actions4277
NOP	
J	L_Main_Modal_Group_Actions4132
NOP	
L_Main_Modal_Group_Actions4277:
...
;Main.c,575 :: 		static int Modal_Group_Actions7(int action){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Main.c,580 :: 		if(gc.spindle_direction < -1 || gc.spindle_direction > 1)
LBU	R2, Offset(_gc+6)(GP)
SLTI	R2, R2, -1
BEQ	R2, R0, L_Main_Modal_Group_Actions7279
NOP	
...
;Main.c,589 :: 		static int Modal_Group_Actions12(int action){
;Main.c,594 :: 		return action;
SEH	R2, R25
;Main.c,595 :: 		}
