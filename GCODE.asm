...
;GCODE.c,125 :: 		FAIL(STATUS_OK);
SH	R0, Offset(GCODE_status_code+0)(GP)
;GCODE.c,126 :: 		if(group_number == MODAL_GROUP_NONE)
LH	R2, Offset(_group_number+0)(GP)
...
;GCODE.c,127 :: 		FAIL(STATUS_MODAL_GROUP_VIOLATION);
ORI	R2, R0, 5
SH	R2, Offset(GCODE_status_code+0)(GP)
L_Check_group_multiple_violations1:
;GCODE.c,129 :: 		if(group_number != last_group_number)
...
;GCODE.c,136 :: 		dma_printf("group_number:= %d\n",group_number);
ADDIU	R23, SP, 8
ADDIU	R22, R23, 19
...
NOP	
ADDIU	SP, SP, 8
;GCODE.c,139 :: 		last_group_number = group_number;
LH	R2, Offset(_group_number+0)(GP)
SH	R2, Offset(Check_group_multiple_violations_last_group_number_L0+0)(GP)
;GCODE.c,140 :: 		if (group_number == MODAL_GROUP_0){
LH	R3, Offset(_group_number+0)(GP)
ORI	R2, R0, 1
...
;GCODE.c,142 :: 		if(non_modal_action != last_non_modal_action){
...
;GCODE.c,143 :: 		Rst_modalword();
JAL	_Rst_modalword+0
NOP	
;GCODE.c,144 :: 		bit_true( non_modal_words,bit( non_modal_action));
...
;GCODE.c,148 :: 		dma_printf("non_modal_action:= %d\tnon_modal_words:=%d\n",
ADDIU	R23, SP, 27
ADDIU	R22, R23, 43
...
NOP	
ADDIU	R3, SP, 27
;GCODE.c,149 :: 		non_modal_action,non_modal_words);
LH	R2, Offset(GCODE_non_modal_words+0)(GP)
ADDIU	SP, SP, -12
SH	R2, 8(SP)
LH	R2, Offset(_non_modal_action+0)(GP)
...
NOP	
ADDIU	SP, SP, 12
;GCODE.c,151 :: 		last_non_modal_action = non_modal_action;
LH	R2, Offset(_non_modal_action+0)(GP)
SH	R2, Offset(Check_group_multiple_violations_last_non_modal_action_L0+0)(GP)
;GCODE.c,152 :: 		return status_code;
...
;GCODE.c,159 :: 		for (i=0; i<=2; i++) { // Axes indices are consistent, so loop may be used to save flash space.
...
;GCODE.c,160 :: 		if ( bit_istrue(axis_words,bit(i))) {
SEH	R3, R5
ORI	R2, R0, 1
SLLV	R3, R2, R3
LBU	R2, Offset(GCODE_axis_words+0)(GP)
...
;GCODE.c,161 :: 		if (!absolute_override) { // Do not update target in absolute override mode
...
;GCODE.c,162 :: 		if (gc.absolute_mode) {
...
;GCODE.c,163 :: 		gc.next_position[i] += gc.position[i] + gc.coord_system[i] + gc.coord_offset[i]; // Absolute mode
SEH	R2, R5
SLL	R4, R2, 2
...
;GCODE.c,171 :: 		gc.next_position[i] += gc.coord_offset[i]; // No axis word in block. Keep same axis position.
SEH	R2, R5
SLL	R3, R2, 2
LUI	R2, hi_addr(_gc+92)
ORI	R2, R2, lo_addr(_gc+92)
...
;GCODE.c,178 :: 		switch (motion_mode) {
J	L_Check_group_multiple_violations17
NOP	
;GCODE.c,180 :: 		case MOTION_MODE_CANCEL:
L_Check_group_multiple_violations19:
;GCODE.c,181 :: 		if (axis_words) { FAIL(STATUS_INVALID_STATEMENT); } // No axis words allowed while active.
LBU	R2, Offset(GCODE_axis_words+0)(GP)
...
NOP	
;GCODE.c,183 :: 		case MOTION_MODE_SEEK:
L_Check_group_multiple_violations21:
;GCODE.c,184 :: 		if (axis_words == 0) {
LBU	R2, Offset(GCODE_axis_words+0)(GP)
...
;GCODE.c,189 :: 		gc.frequency = 5000;//settings.default_seek_rate;
ORI	R2, R0, 5000
SH	R2, Offset(_gc+12)(GP)
;GCODE.c,190 :: 		FAIL(STATUS_OK);
SH	R0, Offset(GCODE_status_code+0)(GP)
;GCODE.c,191 :: 		}
...
J	L_Check_group_multiple_violations18
NOP	
;GCODE.c,193 :: 		case MOTION_MODE_LINEAR:
L_Check_group_multiple_violations24:
;GCODE.c,199 :: 		while(DMA_Busy(1));
...
;GCODE.c,200 :: 		dma_printf("axis_words:= %d\n",(int)axis_words & 0x00FF);
LBU	R2, Offset(GCODE_axis_words+0)(GP)
ANDI	R3, R2, 255
ADDIU	R23, SP, 70
ADDIU	R22, R23, 17
...
J	L_Check_group_multiple_violations18
NOP	
;GCODE.c,209 :: 		case MOTION_MODE_CW_ARC: case MOTION_MODE_CCW_ARC:
L_Check_group_multiple_violations29:
L_Check_group_multiple_violations30:
;GCODE.c,212 :: 		if ( !( bit_false(axis_words,bit(gc.plane_axis_2)) ) ||
...
;GCODE.c,213 :: 		( !gc.r && gc.offset[gc.plane_axis_0] == 0.0 && gc.offset[gc.plane_axis_1] == 0.0 )){
LBU	R2, Offset(GCODE_axis_words+0)(GP)
...
;GCODE.c,214 :: 		FAIL(STATUS_INVALID_STATEMENT);
ORI	R2, R0, 6
SH	R2, Offset(GCODE_status_code+0)(GP)
;GCODE.c,215 :: 		} else {
...
;GCODE.c,224 :: 		for(i=0;i<NoOfAxis;i++){
; i start address is: 20 (R5)
MOVZ	R5, R0, R0
; i end address is: 20 (R5)
...
;GCODE.c,225 :: 		gc.position[i] =  gc.next_position[i];
SEH	R2, R5
SLL	R4, R2, 2
LUI	R2, hi_addr(_gc+20)
ORI	R2, R2, lo_addr(_gc+20)
...
;GCODE.c,228 :: 		return status_code;
LH	R2, Offset(GCODE_status_code+0)(GP)
;GCODE.c,229 :: 		}
;GCODE.c,228 :: 		return status_code;
;GCODE.c,229 :: 		}
...
_Instruction_Values:
;GCODE.c,232 :: 		int Instruction_Values(char *c,void *any){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;GCODE.c,236 :: 		switch(c[0]){
MOVZ	R2, R25, R0
SW	R2, 4(SP)
...
;GCODE.c,239 :: 		gc.next_position[X] = To_Millimeters(XYZ_Val);
LWC1	S12, 0(R26)
JAL	GCODE_To_Millimeters+0
NOP	
SWC1	S0, Offset(_gc+92)(GP)
;GCODE.c,240 :: 		bit_true(axis_words,bit(X));
LBU	R2, Offset(GCODE_axis_words+0)(GP)
ORI	R2, R2, 1
SB	R2, Offset(GCODE_axis_words+0)(GP)
;GCODE.c,241 :: 		break;
...
;GCODE.c,244 :: 		gc.next_position[Y] = To_Millimeters(XYZ_Val);
LWC1	S12, 0(R26)
JAL	GCODE_To_Millimeters+0
NOP	
SWC1	S0, Offset(_gc+96)(GP)
;GCODE.c,245 :: 		bit_true(axis_words,bit(Y));
LBU	R2, Offset(GCODE_axis_words+0)(GP)
ORI	R2, R2, 2
SB	R2, Offset(GCODE_axis_words+0)(GP)
;GCODE.c,246 :: 		break;
...
;GCODE.c,249 :: 		gc.next_position[Z] = To_Millimeters(XYZ_Val);
LWC1	S12, 0(R26)
JAL	GCODE_To_Millimeters+0
NOP	
SWC1	S0, Offset(_gc+100)(GP)
;GCODE.c,250 :: 		bit_true(axis_words,bit(Z));
LBU	R2, Offset(GCODE_axis_words+0)(GP)
ORI	R2, R2, 4
SB	R2, Offset(GCODE_axis_words+0)(GP)
;GCODE.c,251 :: 		break;
...
;GCODE.c,254 :: 		gc.next_position[A] = To_Millimeters(XYZ_Val);
LWC1	S12, 0(R26)
JAL	GCODE_To_Millimeters+0
NOP	
SWC1	S0, Offset(_gc+104)(GP)
;GCODE.c,255 :: 		bit_true(axis_words,bit(A));
LBU	R2, Offset(GCODE_axis_words+0)(GP)
ORI	R2, R2, 8
SB	R2, Offset(GCODE_axis_words+0)(GP)
;GCODE.c,256 :: 		break;
...
;GCODE.c,259 :: 		gc.next_position[B] = To_Millimeters(XYZ_Val);
LWC1	S12, 0(R26)
JAL	GCODE_To_Millimeters+0
NOP	
SWC1	S0, Offset(_gc+108)(GP)
;GCODE.c,260 :: 		bit_true(axis_words,bit(B));
LBU	R2, Offset(GCODE_axis_words+0)(GP)
ORI	R2, R2, 16
SB	R2, Offset(GCODE_axis_words+0)(GP)
;GCODE.c,261 :: 		break;
...
;GCODE.c,264 :: 		gc.R = To_Millimeters(XYZ_Val);
LWC1	S12, 0(R26)
JAL	GCODE_To_Millimeters+0
NOP	
...
;GCODE.c,269 :: 		XYZ_Val = *(float*)any;
LWC1	S0, 0(R26)
;GCODE.c,270 :: 		gc.I = XYZ_Val;
SWC1	S0, Offset(_gc+132)(GP)
;GCODE.c,271 :: 		gc.offset[I] = To_Millimeters(XYZ_Val);
MOV.S 	S12, S0
JAL	GCODE_To_Millimeters+0
NOP	
...
;GCODE.c,274 :: 		XYZ_Val = *(float*)any;
LWC1	S0, 0(R26)
;GCODE.c,275 :: 		gc.J = XYZ_Val;
SWC1	S0, Offset(_gc+136)(GP)
;GCODE.c,276 :: 		gc.offset[J] = To_Millimeters(XYZ_Val);
MOV.S 	S12, S0
JAL	GCODE_To_Millimeters+0
NOP	
...
;GCODE.c,279 :: 		XYZ_Val = *(float*)any;
LWC1	S0, 0(R26)
;GCODE.c,280 :: 		gc.K = XYZ_Val;
SWC1	S0, Offset(_gc+140)(GP)
;GCODE.c,281 :: 		gc.offset[K] = To_Millimeters(XYZ_Val);
MOV.S 	S12, S0
JAL	GCODE_To_Millimeters+0
NOP	
...
;GCODE.c,284 :: 		F_Val = *(int*)any;
LH	R2, 0(R26)
; F_Val start address is: 12 (R3)
SEH	R3, R2
;GCODE.c,285 :: 		if(F_Val < 0){
SEH	R2, R2
SLTI	R2, R2, 0
...
;GCODE.c,286 :: 		FAIL(STATUS_SPEED_ERROR);
ORI	R2, R0, 13
SH	R2, Offset(GCODE_status_code+0)(GP)
;GCODE.c,287 :: 		}
L_Instruction_Values53:
;GCODE.c,294 :: 		gc.frequency = F_Val;
...
;GCODE.c,297 :: 		O_Val = *(int*)any;
LH	R2, 0(R26)
; O_Val start address is: 12 (R3)
SEH	R3, R2
;GCODE.c,298 :: 		if(O_Val < 0){
SEH	R2, R2
SLTI	R2, R2, 0
...
;GCODE.c,299 :: 		FAIL(STATUS_SPEED_ERROR);
ORI	R2, R0, 13
SH	R2, Offset(GCODE_status_code+0)(GP)
;GCODE.c,300 :: 		}
L_Instruction_Values55:
;GCODE.c,301 :: 		gc.P = O_Val;
...
;GCODE.c,305 :: 		O_Val = *(int*)any;
LH	R2, 0(R26)
; O_Val start address is: 12 (R3)
SEH	R3, R2
;GCODE.c,306 :: 		if(O_Val < 0){
SEH	R2, R2
SLTI	R2, R2, 0
...
;GCODE.c,307 :: 		FAIL(STATUS_SPEED_ERROR);
ORI	R2, R0, 13
SH	R2, Offset(GCODE_status_code+0)(GP)
;GCODE.c,308 :: 		}
L_Instruction_Values57:
;GCODE.c,309 :: 		gc.S = O_Val;
...
;GCODE.c,313 :: 		O_Val = *(int*)any;
LH	R2, 0(R26)
; O_Val start address is: 12 (R3)
SEH	R3, R2
;GCODE.c,314 :: 		if(O_Val < 0){
SEH	R2, R2
SLTI	R2, R2, 0
...
;GCODE.c,315 :: 		FAIL(STATUS_SPEED_ERROR);
ORI	R2, R0, 13
SH	R2, Offset(GCODE_status_code+0)(GP)
;GCODE.c,316 :: 		}
L_Instruction_Values59:
;GCODE.c,317 :: 		gc.L = O_Val;
SH	R3, Offset(_gc+14)(GP)
; O_Val end address is: 12 (R3)
;GCODE.c,318 :: 		break; //L2 tells the G10 we’re setting standard work offsets
J	L_Instruction_Values42
NOP	
;GCODE.c,319 :: 		default:FAIL(STATUS_UNSUPPORTED_STATEMENT);break;
...
;GCODE.c,330 :: 		return status_code;
LH	R2, Offset(GCODE_status_code+0)(GP)
;GCODE.c,331 :: 		}
...
;GCODE.c,336 :: 		static float To_Millimeters(float value){
;GCODE.c,337 :: 		return(gc.inches_mode) ? (value * MM_PER_INCH) : value;
...
MTC1	R2, S0
MUL.S 	S0, S12, S0
; ?FLOC__GCODE_To_Millimeters?T172 start address is: 0 (R0)
; ?FLOC__GCODE_To_Millimeters?T172 end address is: 0 (R0)
...
GCODE_Select_Plane:
;GCODE.c,341 :: 		static void Select_Plane(long x,long y,long z){
;GCODE.c,342 :: 		gc.position[X] = x/settings.steps_per_mm[X];
MTC1	R25, S0
CVT32.W 	S1, S0
LWC1	S0, Offset(_settings+4)(GP)
DIV.S 	S0, S1, S0
SWC1	S0, Offset(_gc+20)(GP)
;GCODE.c,343 :: 		gc.position[Y] = y/settings.steps_per_mm[Y];
MTC1	R26, S0
CVT32.W 	S1, S0
LWC1	S0, Offset(_settings+8)(GP)
DIV.S 	S0, S1, S0
SWC1	S0, Offset(_gc+24)(GP)
;GCODE.c,344 :: 		gc.position[Z] = z/settings.steps_per_mm[Z];
MTC1	R27, S0
CVT32.W 	S1, S0
...
;GCODE.c,349 :: 		static int Set_Modal_Groups(int mode){
ADDIU	SP, SP, -4
;GCODE.c,351 :: 		switch(mode) {
J	L_GCODE_Set_Modal_Groups63
NOP	
;GCODE.c,352 :: 		case 4: case 10: case 280:case 281: case 300:case 301: case 53: case 92:
...
;GCODE.c,353 :: 		gp_num = MODAL_GROUP_0; break;
ORI	R2, R0, 1
SH	R2, 0(SP)
J	L_GCODE_Set_Modal_Groups64
NOP	
;GCODE.c,354 :: 		case 0: case 1: case 2: case 3: case 80: gp_num = MODAL_GROUP_1; break;
...
J	L_GCODE_Set_Modal_Groups64
NOP	
;GCODE.c,355 :: 		case 17: case 18: case 19: gp_num = MODAL_GROUP_2; break;
...
J	L_GCODE_Set_Modal_Groups64
NOP	
;GCODE.c,356 :: 		case 90: case 91: gp_num = MODAL_GROUP_3; break;
...
J	L_GCODE_Set_Modal_Groups64
NOP	
;GCODE.c,357 :: 		case 93: case 94: gp_num = MODAL_GROUP_5; break;
...
J	L_GCODE_Set_Modal_Groups64
NOP	
;GCODE.c,358 :: 		case 20: case 21: gp_num = MODAL_GROUP_6; break;
...
J	L_GCODE_Set_Modal_Groups64
NOP	
;GCODE.c,359 :: 		case 54: case 55: case 56: case 57: case 58: case 59: gp_num = MODAL_GROUP_12; break;
...
;GCODE.c,365 :: 		static int Set_Motion_Mode(int mode){
ADDIU	SP, SP, -44
SW	RA, 0(SP)
;GCODE.c,367 :: 		switch(mode){
SW	R25, 4(SP)
SW	R26, 8(SP)
SW	R27, 12(SP)
J	L_GCODE_Set_Motion_Mode93
NOP	
;GCODE.c,368 :: 		case 0: motion_mode    = MOTION_MODE_SEEK;    break;
L_GCODE_Set_Motion_Mode95:
SH	R0, Offset(GCODE_motion_mode+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,369 :: 		case 1: motion_mode    = MOTION_MODE_LINEAR;  break;
L_GCODE_Set_Motion_Mode96:
ORI	R2, R0, 1
SH	R2, Offset(GCODE_motion_mode+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,370 :: 		case 2: motion_mode    = MOTION_MODE_CW_ARC;  break;
L_GCODE_Set_Motion_Mode97:
ORI	R2, R0, 2
SH	R2, Offset(GCODE_motion_mode+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,371 :: 		case 3: motion_mode    = MOTION_MODE_CCW_ARC; break;
L_GCODE_Set_Motion_Mode98:
ORI	R2, R0, 3
SH	R2, Offset(GCODE_motion_mode+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,372 :: 		case 4: non_modal_action  = NON_MODAL_DWELL;     break;
L_GCODE_Set_Motion_Mode99:
ORI	R2, R0, 1
SH	R2, Offset(_non_modal_action+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,373 :: 		case 10: non_modal_action = NON_MODAL_SET_COORDINATE_DATA; break;
L_GCODE_Set_Motion_Mode100:
ORI	R2, R0, 2
SH	R2, Offset(_non_modal_action+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,374 :: 		case 17: Select_Plane(X, Y, Z); break;
...
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,375 :: 		case 18: Select_Plane(X, Z, Y); break;
...
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,376 :: 		case 19: Select_Plane(Y, Z, X); break;
...
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,377 :: 		case 20: gc.inches_mode = 1; break;
L_GCODE_Set_Motion_Mode104:
ORI	R2, R0, 1
SB	R2, Offset(_gc+2)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,378 :: 		case 21: gc.inches_mode = 0; break;
L_GCODE_Set_Motion_Mode105:
SB	R0, Offset(_gc+2)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,379 :: 		case 53: absolute_override = true; break;
L_GCODE_Set_Motion_Mode106:
ORI	R2, R0, 1
SB	R2, Offset(GCODE_absolute_override+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,380 :: 		case 54: case 55: case 56: case 57: case 58: case 59:
...
;GCODE.c,381 :: 		gc.coord_select = int_value-54;
...
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,383 :: 		case 80: motion_mode = MOTION_MODE_CANCEL; break;
L_GCODE_Set_Motion_Mode113:
ORI	R2, R0, 4
SH	R2, Offset(GCODE_motion_mode+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,384 :: 		case 90: gc.absolute_mode = true; break;
L_GCODE_Set_Motion_Mode114:
ORI	R2, R0, 1
SB	R2, Offset(_gc+3)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,385 :: 		case 91: gc.absolute_mode = false; break;
L_GCODE_Set_Motion_Mode115:
SB	R0, Offset(_gc+3)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,386 :: 		case 93: gc.inverse_feed_rate_mode = true; break;
L_GCODE_Set_Motion_Mode116:
ORI	R2, R0, 1
SB	R2, Offset(_gc+1)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,387 :: 		case 94: gc.inverse_feed_rate_mode = false; break;
L_GCODE_Set_Motion_Mode117:
SB	R0, Offset(_gc+1)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,388 :: 		case 280: non_modal_action = NON_MODAL_GO_HOME_0; break;
L_GCODE_Set_Motion_Mode118:
ORI	R2, R0, 3
SH	R2, Offset(_non_modal_action+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,389 :: 		case 281: non_modal_action = NON_MODAL_SET_HOME_0; break;
L_GCODE_Set_Motion_Mode119:
ORI	R2, R0, 4
SH	R2, Offset(_non_modal_action+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,390 :: 		case 300: non_modal_action = NON_MODAL_GO_HOME_1; break;
L_GCODE_Set_Motion_Mode120:
ORI	R2, R0, 5
SH	R2, Offset(_non_modal_action+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,391 :: 		case 301: non_modal_action = NON_MODAL_SET_HOME_1; break;
L_GCODE_Set_Motion_Mode121:
ORI	R2, R0, 6
SH	R2, Offset(_non_modal_action+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,392 :: 		case 920: non_modal_action = NON_MODAL_SET_COORDINATE_OFFSET; break;
L_GCODE_Set_Motion_Mode122:
ORI	R2, R0, 7
SH	R2, Offset(_non_modal_action+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,393 :: 		case 921: non_modal_action = NON_MODAL_RESET_COORDINATE_OFFSET; break;
L_GCODE_Set_Motion_Mode123:
ORI	R2, R0, 8
SH	R2, Offset(_non_modal_action+0)(GP)
J	L_GCODE_Set_Motion_Mode94
NOP	
;GCODE.c,394 :: 		default: FAIL(STATUS_UNSUPPORTED_STATEMENT);break;
...
SH	R0, Offset(GCODE_status_code+0)(GP)
;GCODE.c,401 :: 		if ( bit_istrue(modal_group_words,bit(MODAL_GROUP_1)) || axis_words ) {
LH	R2, Offset(GCODE_modal_group_words+0)(GP)
ANDI	R2, R2, 4
SEH	R2, R2
...
;GCODE.c,403 :: 		if ( gc.inverse_feed_rate_mode ) {
...
;GCODE.c,404 :: 		if (inverse_feed_rate < 0 && motion_mode != MOTION_MODE_CANCEL) {
...
;GCODE.c,405 :: 		FAIL(STATUS_INVALID_STATEMENT);
ORI	R2, R0, 6
SH	R2, Offset(GCODE_status_code+0)(GP)
;GCODE.c,404 :: 		if (inverse_feed_rate < 0 && motion_mode != MOTION_MODE_CANCEL) {
...
;GCODE.c,409 :: 		if ( absolute_override && !(motion_mode == MOTION_MODE_SEEK || motion_mode == MOTION_MODE_LINEAR)) {
LBU	R2, Offset(GCODE_absolute_override+0)(GP)
...
;GCODE.c,410 :: 		FAIL(STATUS_INVALID_STATEMENT);
ORI	R2, R0, 6
SH	R2, Offset(GCODE_status_code+0)(GP)
;GCODE.c,409 :: 		if ( absolute_override && !(motion_mode == MOTION_MODE_SEEK || motion_mode == MOTION_MODE_LINEAR)) {
L_GCODE_Set_Motion_Mode169:
L_GCODE_Set_Motion_Mode168:
;GCODE.c,413 :: 		if (status_code) { return(status_code); }
LH	R2, Offset(GCODE_status_code+0)(GP)
...
;GCODE.c,418 :: 		dma_printf("non_modal_action:= %d\n",non_modal_action);
ADDIU	R23, SP, 18
ADDIU	R22, R23, 23
...
ADDIU	SP, SP, 8
LH	R25, 16(SP)
;GCODE.c,420 :: 		return motion_mode;
LH	R2, Offset(GCODE_motion_mode+0)(GP)
;GCODE.c,421 :: 		}
;GCODE.c,420 :: 		return motion_mode;
;GCODE.c,421 :: 		}
...
;GCODE.c,423 :: 		static void Set_M_Modal_Commands(int flow){
;GCODE.c,425 :: 		switch(flow) {
J	L_GCODE_Set_M_Modal_Commands140
NOP	
;GCODE.c,426 :: 		case 0: case 1: case 2: case 30: group_number = MODAL_GROUP_4; break;
...
NOP	
;GCODE.c,427 :: 		case 3: case 4: case 5: group_number = MODAL_GROUP_7; break;
...
;GCODE.c,432 :: 		static int Set_M_Commands(int flow){
;GCODE.c,434 :: 		switch(flow) {
J	L_GCODE_Set_M_Commands149
NOP	
;GCODE.c,435 :: 		case 0: gc.program_flow = PROGRAM_FLOW_PAUSED; break; // Program pause
L_GCODE_Set_M_Commands151:
ORI	R2, R0, 1
SB	R2, Offset(_gc+4)(GP)
J	L_GCODE_Set_M_Commands150
NOP	
;GCODE.c,436 :: 		case 1: break; // Optional stop not supported. Ignore.
L_GCODE_Set_M_Commands152:
J	L_GCODE_Set_M_Commands150
NOP	
;GCODE.c,437 :: 		case 2:
L_GCODE_Set_M_Commands153:
;GCODE.c,438 :: 		case 30: gc.program_flow = PROGRAM_FLOW_COMPLETED; break; // Program end and reset
L_GCODE_Set_M_Commands154:
ORI	R2, R0, 2
SB	R2, Offset(_gc+4)(GP)
J	L_GCODE_Set_M_Commands150
NOP	
;GCODE.c,439 :: 		case 3: gc.spindle_direction = 1; break;
L_GCODE_Set_M_Commands155:
ORI	R2, R0, 1
SB	R2, Offset(_gc+5)(GP)
J	L_GCODE_Set_M_Commands150
NOP	
;GCODE.c,440 :: 		case 4: gc.spindle_direction = -1; break;
L_GCODE_Set_M_Commands156:
ORI	R2, R0, 255
SB	R2, Offset(_gc+5)(GP)
J	L_GCODE_Set_M_Commands150
NOP	
;GCODE.c,441 :: 		case 5: gc.spindle_direction = 0; break;
L_GCODE_Set_M_Commands157:
SB	R0, Offset(_gc+5)(GP)
J	L_GCODE_Set_M_Commands150
NOP	
;GCODE.c,445 :: 		case 8: gc.coolant_mode = COOLANT_FLOOD_ENABLE; break;
L_GCODE_Set_M_Commands158:
ORI	R2, R0, 1
SB	R2, Offset(_gc+6)(GP)
J	L_GCODE_Set_M_Commands150
NOP	
;GCODE.c,446 :: 		case 9: gc.coolant_mode = COOLANT_DISABLE; break;
L_GCODE_Set_M_Commands159:
SB	R0, Offset(_gc+6)(GP)
J	L_GCODE_Set_M_Commands150
NOP	
;GCODE.c,447 :: 		default: FAIL(STATUS_UNSUPPORTED_STATEMENT);break;
...
;GCODE.c,449 :: 		return status_code;
LH	R2, Offset(GCODE_status_code+0)(GP)
;GCODE.c,450 :: 		}
