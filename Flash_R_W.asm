...
_NVMWriteQuad:
;Flash_R_W.c,48 :: 		unsigned int NVMWriteQuad (void *address, unsigned long *_data){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
...
_NVMWriteRow:
;Flash_R_W.c,73 :: 		unsigned int NVMWriteRow (void  *address, void *_data){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
...
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Flash_R_W.c,103 :: 		NVMADDR = address & FLASH_PADDRESS_TRANSLATE;
SW	R25, 4(SP)
LUI	R2, 8191
ORI	R2, R2, 65535
AND	R2, R25, R2
SW	R2, Offset(NVMADDR+0)(GP)
;Flash_R_W.c,106 :: 		res = NVMUnlock(0x4004);
ORI	R25, R0, 16388
JAL	Flash_R_W_NVMUnlock+0
NOP	
SH	R2, Offset(_res+0)(GP)
;Flash_R_W.c,109 :: 		return res;
LHU	R2, Offset(_res+0)(GP)
;Flash_R_W.c,110 :: 		}
;Flash_R_W.c,109 :: 		return res;
;Flash_R_W.c,110 :: 		}
...
Flash_R_W_NVMUnlock:
;Flash_R_W.c,114 :: 		static unsigned int NVMUnlock (unsigned long nvmop){
ADDIU	SP, SP, -12
SW	RA, 0(SP)
;Flash_R_W.c,115 :: 		unsigned int I_status = 0;
;Flash_R_W.c,116 :: 		unsigned int dma_ = 0;
;Flash_R_W.c,118 :: 		NVMCON &= 0xFFF0;
LW	R2, Offset(NVMCON+0)(GP)
ANDI	R2, R2, 65520
SW	R2, Offset(NVMCON+0)(GP)
;Flash_R_W.c,119 :: 		NVMCON |=  nvmop; //Set NVMOP bits
LW	R2, Offset(NVMCON+0)(GP)
OR	R2, R2, R25
SW	R2, Offset(NVMCON+0)(GP)
...
DI	R30
SH	R30, 8(SP)
;Flash_R_W.c,126 :: 		dma_ = DMA_Suspend();
JAL	_DMA_Suspend+0
NOP	
;Flash_R_W.c,127 :: 		if(dma_){
BNE	R2, R0, L_Flash_R_W_NVMUnlock43
NOP	
J	L_Flash_R_W_NVMUnlock0
NOP	
L_Flash_R_W_NVMUnlock43:
;Flash_R_W.c,128 :: 		LED2 = false;
_LX	
INS	R2, R0, BitPos(LED2+0), 1
_SX	
;Flash_R_W.c,129 :: 		while(DMA_Busy());
...
;Flash_R_W.c,139 :: 		NVMKEY = 0x0U;
SW	R0, Offset(NVMKEY+0)(GP)
;Flash_R_W.c,140 :: 		NVMKEY = 0xAA996655;
LUI	R2, 43673
ORI	R2, R2, 26197
SW	R2, Offset(NVMKEY+0)(GP)
;Flash_R_W.c,141 :: 		NVMKEY = 0x556699AA;
LUI	R2, 21862
ORI	R2, R2, 39338
SW	R2, Offset(NVMKEY+0)(GP)
;Flash_R_W.c,142 :: 		NVMCONSET = 0x8000;//1 << 15;
ORI	R2, R0, 32768
SW	R2, Offset(NVMCONSET+0)(GP)
;Flash_R_W.c,143 :: 		asm{NOP};
NOP	
;Flash_R_W.c,146 :: 		DMA_Resume();
SW	R25, 4(SP)
JAL	_DMA_Resume+0
NOP	
LW	R25, 4(SP)
;Flash_R_W.c,149 :: 		if (I_status != 0)
LHU	R2, 8(SP)
...
;Flash_R_W.c,153 :: 		NVMCONCLR = 0x4000;
ORI	R2, R0, 16384
...
;Flash_R_W.c,159 :: 		return (NVMCON & 0x3000)>>12;
LW	R2, Offset(NVMCON+0)(GP)
ANDI	R2, R2, 12288
SRL	R2, R2, 12
;Flash_R_W.c,160 :: 		}
...
_NVM_ERROR_Rst:
;Flash_R_W.c,165 :: 		unsigned int NVM_ERROR_Rst(){
ADDIU	SP, SP, -4
SW	RA, 0(SP)
;Flash_R_W.c,166 :: 		unsigned int error= 0;
;Flash_R_W.c,167 :: 		NVMCON = 0;
SW	R0, Offset(NVMCON+0)(GP)
;Flash_R_W.c,170 :: 		NVM_WREN_Set();
JAL	Flash_R_W_NVM_WREN_Set+0
NOP	
;Flash_R_W.c,172 :: 		NVM_WR_Set();
JAL	Flash_R_W_NVM_WR_Set+0
NOP	
;Flash_R_W.c,174 :: 		while(NVM_WR_Wait());
L_NVM_ERROR_Rst6:
JAL	Flash_R_W_NVM_WR_Wait+0
NOP	
...
NOP	
L_NVM_ERROR_Rst7:
;Flash_R_W.c,177 :: 		return (NVMCON & 0x3000)>>12;
LW	R2, Offset(NVMCON+0)(GP)
ANDI	R2, R2, 12288
SRL	R2, R2, 12
;Flash_R_W.c,178 :: 		}
...
;Flash_R_W.c,182 :: 		static void NVM_WR_Set(){
ADDIU	SP, SP, -4
SW	RA, 0(SP)
;Flash_R_W.c,184 :: 		NVMCON |= 1 << 15;
LW	R2, Offset(NVMCON+0)(GP)
ORI	R2, R2, 32768
SW	R2, Offset(NVMCON+0)(GP)
;Flash_R_W.c,185 :: 		while(!NVM_WR_Wait());
...
;Flash_R_W.c,190 :: 		static unsigned int NVM_WR_Wait(){
;Flash_R_W.c,191 :: 		return (NVMCON & 0x8000) >> 15;
LW	R2, Offset(NVMCON+0)(GP)
ANDI	R2, R2, 32768
SRL	R2, R2, 15
;Flash_R_W.c,192 :: 		}
...
;Flash_R_W.c,196 :: 		static void NVM_WREN_Set(){
ADDIU	SP, SP, -4
SW	RA, 0(SP)
;Flash_R_W.c,197 :: 		NVMCON |= 1 << 14;
LW	R2, Offset(NVMCON+0)(GP)
ORI	R2, R2, 16384
SW	R2, Offset(NVMCON+0)(GP)
;Flash_R_W.c,198 :: 		while(!NVM_WREN_Wait());
...
;Flash_R_W.c,203 :: 		static void NVM_WREN_Rst(){
ADDIU	SP, SP, -4
SW	RA, 0(SP)
;Flash_R_W.c,204 :: 		NVMCONCLR |= 1<<14;
LW	R2, Offset(NVMCONCLR+0)(GP)
ORI	R2, R2, 16384
SW	R2, Offset(NVMCONCLR+0)(GP)
;Flash_R_W.c,205 :: 		while(NVM_WREN_Wait());
...
;Flash_R_W.c,210 :: 		static unsigned int NVM_WREN_Wait(){
;Flash_R_W.c,211 :: 		return (NVMCON & 0x4000) >> 14;
LW	R2, Offset(NVMCON+0)(GP)
ANDI	R2, R2, 16384
SRL	R2, R2, 14
;Flash_R_W.c,212 :: 		}
...
_NVM_PWPAGE_Lock:
;Flash_R_W.c,216 :: 		void NVM_PWPAGE_Lock(){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Flash_R_W.c,219 :: 		unsigned int dma_susp=0;   // storage for current DMA state
; dma_susp start address is: 16 (R4)
MOVZ	R4, R0, R0
;Flash_R_W.c,221 :: 		I_status = (unsigned int)DI();
DI	R30
SH	R30, 4(SP)
; dma_susp end address is: 16 (R4)
ANDI	R3, R4, 65535
;Flash_R_W.c,224 :: 		while(!dma_susp)
L_NVM_PWPAGE_Lock14:
...
;Flash_R_W.c,225 :: 		dma_susp = DMA_Suspend();
JAL	_DMA_Suspend+0
NOP	
; dma_susp start address is: 16 (R4)
ANDI	R4, R2, 65535
...
NOP	
L_NVM_PWPAGE_Lock15:
;Flash_R_W.c,228 :: 		NVMCONSET = 4;
; dma_susp start address is: 12 (R3)
ORI	R2, R0, 4
SW	R2, Offset(NVMCONSET+0)(GP)
;Flash_R_W.c,230 :: 		NVM_WREN_Set();
JAL	Flash_R_W_NVM_WREN_Set+0
NOP	
;Flash_R_W.c,234 :: 		NVMKEY = 0x0U;
SW	R0, Offset(NVMKEY+0)(GP)
;Flash_R_W.c,235 :: 		NVMKEY = 0xAA996655;
LUI	R2, 43673
ORI	R2, R2, 26197
SW	R2, Offset(NVMKEY+0)(GP)
;Flash_R_W.c,236 :: 		NVMKEY = 0x556699AA;
LUI	R2, 21862
ORI	R2, R2, 39338
SW	R2, Offset(NVMKEY+0)(GP)
;Flash_R_W.c,237 :: 		NVMPWPCLR = 0x8000C000;
LUI	R2, 32768
ORI	R2, R2, 49152
...
;Flash_R_W.c,241 :: 		dma_susp = DMA_Resume();
JAL	_DMA_Resume+0
NOP	
; dma_susp start address is: 8 (R2)
;Flash_R_W.c,242 :: 		}
...
_NVMReadRow:
;Flash_R_W.c,257 :: 		void NVMReadRow(unsigned long addr,unsigned long *buff){
;Flash_R_W.c,262 :: 		ptr = (unsigned long*)addr;
; ptr start address is: 16 (R4)
MOVZ	R4, R25, R0
;Flash_R_W.c,264 :: 		for(j = 0;j < 128;j++){
...
; end of _NVMReadRow
_NVMReadQuad:
;Flash_R_W.c,271 :: 		void NVMReadQuad(unsigned long addr,unsigned long *words){
;Flash_R_W.c,275 :: 		ptr = (unsigned long*)addr;
; ptr start address is: 16 (R4)
MOVZ	R4, R25, R0
;Flash_R_W.c,277 :: 		for(j = 0;j < 4;j++){
; j start address is: 20 (R5)
MOVZ	R5, R0, R0
; j end address is: 20 (R5)
L_NVMReadQuad22:
...
; end of _NVMReadQuad
_NVMReadWord:
;Flash_R_W.c,284 :: 		unsigned long NVMReadWord(void *addr){
;Flash_R_W.c,287 :: 		val = *(unsigned long*)addr;
LW	R2, 0(R25)
;Flash_R_W.c,289 :: 		return val;
;Flash_R_W.c,290 :: 		}
L_end_NVMReadWord:
JR	RA
NOP	
; end of _NVMReadWord
_Get_Address_Pval:
;Flash_R_W.c,294 :: 		unsigned long Get_Address_Pval(int recipe){
;Flash_R_W.c,295 :: 		unsigned long addr = 0;
; addr start address is: 16 (R4)
MOVZ	R4, R0, R0
;Flash_R_W.c,296 :: 		switch(recipe){
J	L_Get_Address_Pval25
NOP	
;Flash_R_W.c,297 :: 		case 0:break;
L_Get_Address_Pval27:
MOVZ	R2, R4, R0
; addr end address is: 16 (R4)
J	L_Get_Address_Pval26
NOP	
;Flash_R_W.c,298 :: 		case 1:addr =(unsigned long)FLASH_Settings_VAddr_P1;break;
...
J	L_Get_Address_Pval26
NOP	
;Flash_R_W.c,299 :: 		case 2:addr =(unsigned long)FLASH_Settings_VAddr_P2;break;
...
J	L_Get_Address_Pval26
NOP	
;Flash_R_W.c,300 :: 		case 3:addr =(unsigned long)FLASH_Settings_VAddr_P3;break;
...
J	L_Get_Address_Pval26
NOP	
;Flash_R_W.c,301 :: 		case 4:addr =(unsigned long)FLASH_Settings_VAddr_P4;break;
...
J	L_Get_Address_Pval26
NOP	
;Flash_R_W.c,302 :: 		case 5:addr =(unsigned long)FLASH_Settings_VAddr_P5;break;
...
J	L_Get_Address_Pval26
NOP	
;Flash_R_W.c,303 :: 		case 6:addr =(unsigned long)FLASH_Settings_VAddr_P6;break;
...
J	L_Get_Address_Pval26
NOP	
;Flash_R_W.c,304 :: 		case 7:addr =(unsigned long)FLASH_Settings_VAddr_P7;break;
...
J	L_Get_Address_Pval26
NOP	
;Flash_R_W.c,305 :: 		case 8:addr =(unsigned long)FLASH_Settings_VAddr_P8;break;
...
J	L_Get_Address_Pval26
NOP	
;Flash_R_W.c,306 :: 		case 9:addr =(unsigned long)FLASH_Settings_VAddr_P9;break;
