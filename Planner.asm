...
SEH	R2, R5
SLTI	R2, R2, 4
BNE	R2, R0, L__set_calculation_constants56
NOP	
J	L_set_calculation_constants4
NOP	
L__set_calculation_constants56:
...
SB	R2, 148(SP)
L_r_or_ijk29:
;Planner.c,356 :: 		while(DMA_IsOn(1));
L_r_or_ijk30:
SH	R27, 8(SP)
SH	R26, 10(SP)
SH	R25, 12(SP)
...
NOP	
L_r_or_ijk31:
;Planner.c,359 :: 		,position[X],position[Y],position[Z],target[X],target[Y],target[Z]);
ADDIU	R3, SP, 116
ADDIU	R2, R3, 8
LWC1	S5, 0(R2)
ADDIU	R2, R3, 4
...
LWC1	S1, 0(R2)
LWC1	S0, 0(R3)
;Planner.c,358 :: 		[pos[X]:= %f\tpos[Y]:= %f\tpos[Z]:= %f][tar[X]:= %f\ttar[Y]:= %f\ttar[Z]:= %f]\n\n"
ADDIU	R23, SP, 22
ADDIU	R22, R23, 78
...
SEH	R2, R3
SLTI	R2, R2, 4
BNE	R2, R0, L__plan_reset_absolute_position84
NOP	
J	L_plan_reset_absolute_position36
NOP	
L__plan_reset_absolute_position84:
