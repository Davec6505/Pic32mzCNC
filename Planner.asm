...
;Planner.c,125 :: 		STPS[axis_No].StartUp_delay = minSpeed;
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
NOP	
L_speed_cntr_Move10:
;Planner.c,127 :: 		STPS[axis_No].StartUp_delay = STPS[axis_No].step_delay ;
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
SW	R2, 0(R3)
L_speed_cntr_Move11:
;Planner.c,133 :: 		STPS[axis_No].max_step_lim = (long)((temp_speed*temp_speed)/((alpha[axis_No]*x20000*(float)acc)/100.00));
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
MFC1	R2, S0
SW	R2, 0(R4)
;Planner.c,137 :: 		if(STPS[axis_No].max_step_lim == 0){
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
;Planner.c,138 :: 		STPS[axis_No].max_step_lim = 1;
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
;Planner.c,139 :: 		}
L_speed_cntr_Move12:
;Planner.c,143 :: 		STPS[axis_No].accel_lim = (abs_mmSteps * dec) / (acc + dec);
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
;Planner.c,145 :: 		STPS[axis_No].accel_lim = STPS[axis_No].max_step_lim;
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
SW	R2, 0(R3)
L_speed_cntr_Move13:
;Planner.c,148 :: 		if(STPS[axis_No].accel_lim == 0){
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
;Planner.c,149 :: 		STPS[axis_No].accel_lim = 1;
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
;Planner.c,150 :: 		}
L_speed_cntr_Move14:
;Planner.c,153 :: 		if(STPS[axis_No].accel_lim < STPS[axis_No].max_step_lim){
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
;Planner.c,154 :: 		STPS[axis_No].decel_val = STPS[axis_No].accel_lim - mmSteps;//-(abs_mmSteps - STPS[axis_No].max_step_lim);
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
NOP	
L_speed_cntr_Move15:
;Planner.c,156 :: 		STPS[axis_No].decel_val = -((STPS[axis_No].max_step_lim * acc)/dec);
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
;Planner.c,157 :: 		}
L_speed_cntr_Move16:
;Planner.c,159 :: 		if(STPS[axis_No].decel_val == 0)
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
;Planner.c,160 :: 		STPS[axis_No].decel_val = -1;
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
NOP	
L_speed_cntr_Move18:
;Planner.c,167 :: 		STPS[axis_No].decel_start = mmSteps + STPS[axis_No].decel_val;
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
;Planner.c,168 :: 		}
L_speed_cntr_Move19:
;Planner.c,172 :: 		if(STPS[axis_No].StartUp_delay <= STPS[axis_No].min_delay){
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
;Planner.c,173 :: 		STPS[axis_No].step_delay = labs(STPS[axis_No].min_delay);
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
NOP	
L_speed_cntr_Move20:
;Planner.c,176 :: 		STPS[axis_No].step_delay = labs(STPS[axis_No].StartUp_delay);
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
;Planner.c,180 :: 		}
L_speed_cntr_Move4:
;Planner.c,182 :: 		STPS[axis_No].step_count  = 0;
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
ADDIU	R2, R2, 32
SW	R0, 0(R2)
;Planner.c,183 :: 		STPS[axis_No].rest        = 0;
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
ADDIU	R2, R2, 60
SW	R0, 0(R2)
;Planner.c,184 :: 		STPS[axis_No].accel_count = 1;
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
NOP	
L_speed_cntr_Move23:
;Planner.c,222 :: 		,STPS[axis_No].min_delay
SEH	R3, R26
ORI	R2, R0, 80
MULTU	R2, R3
MFLO	R3
...
ADDIU	R2, R3, 20
LW	R10, 0(R2)
;Planner.c,221 :: 		,STPS[axis_No].step_delay
ADDIU	R2, R3, 8
LW	R9, 0(R2)
;Planner.c,220 :: 		,STPS[axis_No].decel_start
ADDIU	R2, R3, 12
LW	R8, 0(R2)
;Planner.c,219 :: 		,STPS[axis_No].decel_val
ADDIU	R2, R3, 16
LW	R7, 0(R2)
;Planner.c,218 :: 		,STPS[axis_No].accel_lim
ADDIU	R2, R3, 52
LW	R6, 0(R2)
;Planner.c,217 :: 		,STPS[axis_No].max_step_lim
ADDIU	R2, R3, 56
LW	R5, 0(R2)
;Planner.c,216 :: 		,a_t_x100[axis_No]
SEH	R2, R26
SLL	R3, R2, 2
LUI	R2, 40960
ORI	R2, R2, 9792
ADDU	R2, R2, R3
LWC1	S1, 0(R2)
;Planner.c,214 :: 		,alpha[axis_No]
LUI	R2, 40960
ORI	R2, R2, 10016
ADDU	R2, R2, R3
LWC1	S0, 0(R2)
;Planner.c,212 :: 		,a_sq[axis_No]
LUI	R2, 40960
ORI	R2, R2, 9856
ADDU	R2, R2, R3
LW	R4, 0(R2)
...
;Planner.c,216 :: 		,a_t_x100[axis_No]
SWC1	S1, 40(SP)
;Planner.c,215 :: 		,axis_No
SH	R26, 36(SP)
;Planner.c,214 :: 		,alpha[axis_No]
SWC1	S0, 32(SP)
;Planner.c,213 :: 		,axis_No
SH	R26, 28(SP)
;Planner.c,212 :: 		,a_sq[axis_No]
SW	R4, 24(SP)
;Planner.c,211 :: 		,axis_No
SH	R26, 20(SP)
;Planner.c,210 :: 		,abs_mmSteps
SW	R11, 16(SP)
...
_r_or_ijk:
;Planner.c,239 :: 		float r, float i, float j, float k, int axis_A,int axis_B,int dir){
ADDIU	SP, SP, -100
SW	RA, 0(SP)
SW	R25, 4(SP)
SW	R26, 8(SP)
...
;Planner.c,240 :: 		char isclockwise = 0;
;Planner.c,242 :: 		float position[NoOfAxis]={0.0};
ADDIU	R23, SP, 48
ADDIU	R22, R23, 48
...
JAL	___CC2DW+0
NOP	
;Planner.c,243 :: 		float target[NoOfAxis]={0.0};
;Planner.c,244 :: 		float offset[NoOfAxis]={0.0};
;Planner.c,245 :: 		float x = 0.00;
;Planner.c,246 :: 		float y = 0.00;
;Planner.c,247 :: 		float h_x2_div_d = 0.00;
;Planner.c,248 :: 		float speed = 0.00;
;Planner.c,253 :: 		position[axis_A] = Cur_axis_a;
ADDIU	R3, SP, 48
SEH	R2, R25
SLL	R2, R2, 2
ADDU	R2, R3, R2
SWC1	S12, 0(R2)
;Planner.c,254 :: 		position[axis_B] = Cur_axis_b;
SEH	R2, R26
SLL	R2, R2, 2
ADDU	R2, R3, R2
SWC1	S13, 0(R2)
;Planner.c,255 :: 		position[2] = 0;
ADDIU	R2, R3, 8
SW	R0, 0(R2)
;Planner.c,256 :: 		target[axis_A] = Fin_axis_a;
ADDIU	R3, SP, 64
SEH	R2, R25
SLL	R2, R2, 2
ADDU	R2, R3, R2
SWC1	S14, 0(R2)
;Planner.c,257 :: 		target[axis_B] = Fin_axis_b;
SEH	R2, R26
SLL	R2, R2, 2
ADDU	R2, R3, R2
SWC1	S15, 0(R2)
;Planner.c,258 :: 		target[2] = 0;
ADDIU	R2, R3, 8
SW	R0, 0(R2)
;Planner.c,259 :: 		offset[axis_A] = i;
ADDIU	R3, SP, 80
SEH	R2, R25
SLL	R2, R2, 2
ADDU	R2, R3, R2
SWC1	S1, 0(R2)
;Planner.c,260 :: 		offset[axis_B] = j;
SEH	R2, R26
SLL	R2, R2, 2
ADDU	R2, R3, R2
SWC1	S2, 0(R2)
;Planner.c,262 :: 		if (r != 0.00) { // Arc Radius Mode
MOVZ	R2, R0, R0
MTC1	R2, S0
C.EQ.S 	0, S18, S0
...
;Planner.c,326 :: 		x = target[axis_plane_a] - position[axis_plane_a];
ADDIU	R5, SP, 64
LH	R2, 42(SP)
SLL	R3, R2, 2
ADDU	R2, R5, R3
...
MOV.S 	S16, S3
;Planner.c,328 :: 		y = target[axis_plane_b] - position[axis_plane_b];
LH	R2, 44(SP)
SLL	R3, R2, 2
ADDU	R2, R5, R3
LWC1	S1, 0(R2)
...
MOV.S 	S17, S2
;Planner.c,332 :: 		h_x2_div_d = 4 * r*r - x*x - y*y;
LUI	R2, 16512
ORI	R2, R2, 0
MTC1	R2, S0
MUL.S 	S0, S0, S18
...
MUL.S 	S0, S2, S2
SUB.S 	S0, S1, S0
;Planner.c,335 :: 		h_x2_div_d = -sqrt(h_x2_div_d)/hypot(x,y); // == -(h * 2 / d)
SH	R25, 20(SP)
MOV.S 	S12, S0
JAL	_sqrt+0
NOP	
...
MOV.S 	S19, S0
;Planner.c,337 :: 		if (Get_motionmode() == MOTION_MODE_CCW_ARC) { h_x2_div_d = -h_x2_div_d; }
SWC1	S19, 20(SP)
SWC1	S17, 24(SP)
SWC1	S16, 28(SP)
SWC1	S18, 32(SP)
...
NOP	
L__r_or_ijk78:
;Planner.c,361 :: 		h_x2_div_d = -h_x2_div_d;
MOVZ	R2, R0, R0
MTC1	R2, S0
SUB.S 	S0, S0, S1
; h_x2_div_d end address is: 8 (R2)
...
MOV.S 	S3, S0
;Planner.c,362 :: 		r = -r; // Finished with r. Set to positive for mc_arc
MOVZ	R2, R0, R0
MTC1	R2, S0
SUB.S 	S0, S0, S18
...
;Planner.c,363 :: 		}
L_r_or_ijk26:
;Planner.c,366 :: 		i =  0.5*(x-(y*h_x2_div_d));
; r start address is: 16 (R4)
; h_x2_div_d start address is: 24 (R6)
MUL.S 	S0, S17, S3
SUB.S 	S1, S16, S0
...
;Planner.c,368 :: 		j =  0.5*(y+(x*h_x2_div_d));
MUL.S 	S0, S16, S3
...
NOP	
L_r_or_ijk24:
;Planner.c,372 :: 		r = hypot(i, j); // Compute arc radius for mc_arc
; j start address is: 16 (R4)
; i start address is: 8 (R2)
SH	R27, 20(SP)
; j end address is: 16 (R4)
...
;Planner.c,373 :: 		}
L_r_or_ijk27:
;Planner.c,375 :: 		isclockwise = 0;
; r start address is: 16 (R4)
; isclockwise start address is: 20 (R5)
MOVZ	R5, R0, R0
;Planner.c,376 :: 		if (dir == CW) { isclockwise = 1; }
SEH	R2, R27
...
L__r_or_ijk52:
L_r_or_ijk28:
;Planner.c,385 :: 		speed = RPS_FROM_MMPMIN(gc.feed_rate);
; isclockwise start address is: 20 (R5)
...
MTC1	R2, S0
DIV.S 	S0, S1, S0
;Planner.c,387 :: 		speed = Get_Step_Rate(speed,axis_A);
MOV.S 	S12, S0
JAL	_Get_Step_Rate+0
NOP	
;Planner.c,390 :: 		mc_arc(position, target, offset, axis_A, axis_B, Z,
ADDIU	R4, SP, 80
ADDIU	R3, SP, 64
ADDIU	R2, SP, 48
;Planner.c,391 :: 		speed, gc.inverse_feed_rate_mode,r, isclockwise);
ADDIU	SP, SP, -8
SB	R5, 5(SP)
; r end address is: 16 (R4)
...
;Planner.c,397 :: 		void sys_sync_current_position(){
ADDIU	SP, SP, -4
SW	RA, 0(SP)
;Planner.c,399 :: 		plan_set_current_position();
...
;Planner.c,403 :: 		void plan_set_current_position(){
ADDIU	SP, SP, -40
SW	RA, 0(SP)
;Planner.c,404 :: 		int i = 0;
SW	R25, 4(SP)
SW	R26, 8(SP)
;Planner.c,405 :: 		for(i=0;i<NoOfAxis;i++)
...
;Planner.c,406 :: 		gc.position[i] = beltsteps2mm(STPS[i].steps_abs_position,i);
SEH	R2, R4
SLL	R3, R2, 2
...
;Planner.c,409 :: 		while(DMA_IsOn(1));
...
;Planner.c,416 :: 		void plan_reset_absolute_position(){
;Planner.c,417 :: 		int i = 0;
;Planner.c,418 :: 		for(i=0;i<NoOfAxis;i++)
...
;Planner.c,419 :: 		STPS[X].steps_abs_position = 0;
...
_sqrt_:
;Planner.c,437 :: 		long sqrt_(long x){
;Planner.c,443 :: 		xr = 0;                     // clear result
; xr start address is: 20 (R5)
MOVZ	R5, R0, R0
;Planner.c,444 :: 		q2 = 0x40000000L;           // higest possible result bit
...
NOP	
L__sqrt_45:
;Planner.c,459 :: 		} while(q2 >>= 2);          // shift twice
MOVZ	R3, R2, R0
MOVZ	R5, R4, R0
;Planner.c,445 :: 		do
L_sqrt_37:
;Planner.c,447 :: 		if((xr + q2) <= x)
; q2 start address is: 12 (R3)
; xr start address is: 20 (R5)
ADDU	R2, R5, R3
SLTU	R2, R25, R2
...
NOP	
L__sqrt_88:
;Planner.c,449 :: 		x -= xr + q2;
ADDU	R2, R5, R3
SUBU	R2, R25, R2
MOVZ	R25, R2, R0
;Planner.c,450 :: 		f = 1;                  // set flag
...
NOP	
L__sqrt_92:
;Planner.c,461 :: 		return xr +1;             // add for rounding
ADDIU	R2, R4, 1
; xr end address is: 16 (R4)
J	L_end_sqrt_
NOP	
