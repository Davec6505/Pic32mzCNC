_SetInitialSizes:
;Kinematics.c,33 :: 		void SetInitialSizes(STP axis[6]){
;Kinematics.c,34 :: 		int i = 0;
;Kinematics.c,36 :: 		gc.absolute_mode = 0;
SB	R0, Offset(_gc+5)(GP)
;Kinematics.c,38 :: 		for (i = 0;i<NoOfAxis;i++){
; i start address is: 20 (R5)
MOVZ	R5, R0, R0
; i end address is: 20 (R5)
L_SetInitialSizes0:
; i start address is: 20 (R5)
SEH	R2, R5
SLTI	R2, R2, 4
BNE	R2, R0, L__SetInitialSizes107
NOP	
J	L_SetInitialSizes1
NOP	
L__SetInitialSizes107:
;Kinematics.c,39 :: 		axis[i].max_travel = max_sizes[i];//SIZE_LIST[i];
SEH	R3, R5
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R2
ADDU	R2, R25, R2
ADDIU	R4, R2, 64
SEH	R2, R5
SLL	R3, R2, 2
LUI	R2, hi_addr(_max_sizes+0)
ORI	R2, R2, lo_addr(_max_sizes+0)
ADDU	R2, R2, R3
LWC1	S0, 0(R2)
SWC1	S0, 0(R4)
;Kinematics.c,38 :: 		for (i = 0;i<NoOfAxis;i++){
ADDIU	R2, R5, 1
SEH	R5, R2
;Kinematics.c,40 :: 		}
; i end address is: 20 (R5)
J	L_SetInitialSizes0
NOP	
L_SetInitialSizes1:
;Kinematics.c,41 :: 		}
L_end_SetInitialSizes:
JR	RA
NOP	
; end of _SetInitialSizes
Kinematics_Set_Axisdirection:
;Kinematics.c,46 :: 		static void Set_Axisdirection(long temp,int axis){
;Kinematics.c,47 :: 		int dir = (temp < 0)? CCW : CW;
SLTI	R2, R25, 0
BNE	R2, R0, L_Kinematics_Set_Axisdirection109
NOP	
J	L_Kinematics_Set_Axisdirection3
NOP	
L_Kinematics_Set_Axisdirection109:
; ?FLOC__Kinematics_Set_Axisdirection?T17 start address is: 8 (R2)
ORI	R2, R0, 1
; ?FLOC__Kinematics_Set_Axisdirection?T17 end address is: 8 (R2)
J	L_Kinematics_Set_Axisdirection4
NOP	
L_Kinematics_Set_Axisdirection3:
; ?FLOC__Kinematics_Set_Axisdirection?T17 start address is: 8 (R2)
MOVZ	R2, R0, R0
; ?FLOC__Kinematics_Set_Axisdirection?T17 end address is: 8 (R2)
L_Kinematics_Set_Axisdirection4:
; ?FLOC__Kinematics_Set_Axisdirection?T17 start address is: 8 (R2)
; dir start address is: 16 (R4)
SEB	R4, R2
; ?FLOC__Kinematics_Set_Axisdirection?T17 end address is: 8 (R2)
;Kinematics.c,48 :: 		switch(axis){
J	L_Kinematics_Set_Axisdirection5
NOP	
;Kinematics.c,49 :: 		case X:
L_Kinematics_Set_Axisdirection7:
;Kinematics.c,50 :: 		DIR_StepX = (X_DIR_DIR ^ dir) & 0x0001;//(X_DIR_DIR)?dir:~dir;
XORI	R2, R4, 1
; dir end address is: 16 (R4)
ANDI	R3, R2, 1
_LX	
INS	R2, R3, BitPos(DIR_StepX+0), 1
_SX	
;Kinematics.c,51 :: 		break;
J	L_Kinematics_Set_Axisdirection6
NOP	
;Kinematics.c,52 :: 		case Y:
L_Kinematics_Set_Axisdirection8:
;Kinematics.c,53 :: 		DIR_StepY = (Y_DIR_DIR ^ dir) & 0x0001;
; dir start address is: 16 (R4)
ANDI	R3, R4, 1
; dir end address is: 16 (R4)
_LX	
INS	R2, R3, BitPos(DIR_StepY+0), 1
_SX	
;Kinematics.c,54 :: 		break;
J	L_Kinematics_Set_Axisdirection6
NOP	
;Kinematics.c,55 :: 		case Z:
L_Kinematics_Set_Axisdirection9:
;Kinematics.c,56 :: 		DIR_StepZ = (Z_DIR_DIR ^ dir) & 0x0001;
; dir start address is: 16 (R4)
ANDI	R3, R4, 1
; dir end address is: 16 (R4)
_LX	
INS	R2, R3, BitPos(DIR_StepZ+0), 1
_SX	
;Kinematics.c,57 :: 		break;
J	L_Kinematics_Set_Axisdirection6
NOP	
;Kinematics.c,58 :: 		case A:
L_Kinematics_Set_Axisdirection10:
;Kinematics.c,59 :: 		DIR_StepA = (A_DIR_DIR ^ dir) & 0x0001;
; dir start address is: 16 (R4)
ANDI	R3, R4, 1
; dir end address is: 16 (R4)
_LX	
INS	R2, R3, BitPos(DIR_StepA+0), 1
_SX	
;Kinematics.c,60 :: 		break;
J	L_Kinematics_Set_Axisdirection6
NOP	
;Kinematics.c,61 :: 		default: break;
L_Kinematics_Set_Axisdirection11:
J	L_Kinematics_Set_Axisdirection6
NOP	
;Kinematics.c,62 :: 		}
L_Kinematics_Set_Axisdirection5:
; dir start address is: 16 (R4)
SEH	R2, R26
BNE	R2, R0, L_Kinematics_Set_Axisdirection111
NOP	
J	L_Kinematics_Set_Axisdirection7
NOP	
L_Kinematics_Set_Axisdirection111:
SEH	R3, R26
ORI	R2, R0, 1
BNE	R3, R2, L_Kinematics_Set_Axisdirection113
NOP	
J	L_Kinematics_Set_Axisdirection8
NOP	
L_Kinematics_Set_Axisdirection113:
SEH	R3, R26
ORI	R2, R0, 2
BNE	R3, R2, L_Kinematics_Set_Axisdirection115
NOP	
J	L_Kinematics_Set_Axisdirection9
NOP	
L_Kinematics_Set_Axisdirection115:
SEH	R3, R26
ORI	R2, R0, 3
BNE	R3, R2, L_Kinematics_Set_Axisdirection117
NOP	
J	L_Kinematics_Set_Axisdirection10
NOP	
L_Kinematics_Set_Axisdirection117:
; dir end address is: 16 (R4)
J	L_Kinematics_Set_Axisdirection11
NOP	
L_Kinematics_Set_Axisdirection6:
;Kinematics.c,63 :: 		}
L_end_Set_Axisdirection:
JR	RA
NOP	
; end of Kinematics_Set_Axisdirection
_SingleAxisStep:
;Kinematics.c,77 :: 		void SingleAxisStep(float newxyz,float speed,int axis_No){
ADDIU	SP, SP, -20
SW	RA, 0(SP)
;Kinematics.c,78 :: 		long  absxyz = 0;
SW	R25, 4(SP)
SW	R26, 8(SP)
;Kinematics.c,79 :: 		long  tempA  = 0;
;Kinematics.c,80 :: 		int   dir    = 0;
;Kinematics.c,83 :: 		speed = RPS_FROM_MMPMIN(speed);
LUI	R2, 16928
ORI	R2, R2, 0
MTC1	R2, S0
DIV.S 	S1, S13, S0
LUI	R2, 17008
ORI	R2, R2, 0
MTC1	R2, S0
DIV.S 	S0, S1, S0
MOV.S 	S13, S0
;Kinematics.c,92 :: 		if(gc.absolute_mode == true){
LBU	R3, Offset(_gc+5)(GP)
ORI	R2, R0, 1
BEQ	R3, R2, L__SingleAxisStep119
NOP	
J	L_SingleAxisStep12
NOP	
L__SingleAxisStep119:
;Kinematics.c,94 :: 		tempA = belt_steps(newxyz,axis_No);
SH	R25, 12(SP)
SWC1	S13, 16(SP)
JAL	_belt_steps+0
NOP	
LWC1	S13, 16(SP)
LH	R25, 12(SP)
;Kinematics.c,97 :: 		tempA = tempA - STPS[axis_No].steps_abs_position;
SEH	R4, R25
ORI	R3, R0, 68
MULTU	R3, R4
MFLO	R4
LUI	R3, hi_addr(_STPS+0)
ORI	R3, R3, lo_addr(_STPS+0)
ADDU	R3, R3, R4
ADDIU	R3, R3, 60
LW	R3, 0(R3)
SUBU	R2, R2, R3
; tempA start address is: 12 (R3)
MOVZ	R3, R2, R0
;Kinematics.c,99 :: 		if(tempA== 0){
BEQ	R2, R0, L__SingleAxisStep120
NOP	
J	L_SingleAxisStep13
NOP	
L__SingleAxisStep120:
; tempA end address is: 12 (R3)
;Kinematics.c,100 :: 		return;
J	L_end_SingleAxisStep
NOP	
;Kinematics.c,101 :: 		}
L_SingleAxisStep13:
;Kinematics.c,102 :: 		}else{
; tempA start address is: 12 (R3)
MOVZ	R2, R3, R0
; tempA end address is: 12 (R3)
J	L_SingleAxisStep14
NOP	
L_SingleAxisStep12:
;Kinematics.c,103 :: 		tempA = belt_steps(newxyz,axis_No);
SH	R25, 12(SP)
SWC1	S13, 16(SP)
JAL	_belt_steps+0
NOP	
LWC1	S13, 16(SP)
LH	R25, 12(SP)
; tempA start address is: 12 (R3)
MOVZ	R3, R2, R0
; tempA end address is: 12 (R3)
MOVZ	R2, R3, R0
;Kinematics.c,104 :: 		}
L_SingleAxisStep14:
;Kinematics.c,105 :: 		SingleAxisStart(tempA,speed,axis_No);
; tempA start address is: 8 (R2)
MOV.S 	S12, S13
SEH	R26, R25
MOVZ	R25, R2, R0
; tempA end address is: 8 (R2)
JAL	Kinematics_SingleAxisStart+0
NOP	
;Kinematics.c,107 :: 		}
L_end_SingleAxisStep:
LW	R26, 8(SP)
LW	R25, 4(SP)
LW	RA, 0(SP)
ADDIU	SP, SP, 20
JR	RA
NOP	
; end of _SingleAxisStep
Kinematics_SingleAxisStart:
;Kinematics.c,109 :: 		static void SingleAxisStart(long dist,float speed,int axis_No){
ADDIU	SP, SP, -24
SW	RA, 0(SP)
;Kinematics.c,110 :: 		long speed_ = 0;
SW	R25, 4(SP)
;Kinematics.c,112 :: 		STPS[axis_No].dist = labs(dist);
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 36
SW	R2, 20(SP)
JAL	_labs+0
NOP	
LW	R3, 20(SP)
SW	R2, 0(R3)
;Kinematics.c,113 :: 		Single_Axis_Enable(axis_No);
SH	R26, 8(SP)
SW	R25, 12(SP)
SWC1	S12, 16(SP)
SEH	R25, R26
JAL	_Single_Axis_Enable+0
NOP	
LWC1	S12, 16(SP)
LW	R25, 12(SP)
LH	R26, 8(SP)
;Kinematics.c,114 :: 		speed_cntr_Move(dist , speed, axis_No);
SH	R26, 8(SP)
SW	R25, 12(SP)
JAL	_speed_cntr_Move+0
NOP	
LW	R25, 12(SP)
LH	R26, 8(SP)
;Kinematics.c,116 :: 		Set_Axisdirection(dist,axis_No);
JAL	Kinematics_Set_Axisdirection+0
NOP	
;Kinematics.c,117 :: 		STPS[axis_No].axis_dir = Direction(dist);
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R3, R2, 2
SLTI	R2, R25, 0
BNE	R2, R0, L_Kinematics_SingleAxisStart122
NOP	
J	L_Kinematics_SingleAxisStart15
NOP	
L_Kinematics_SingleAxisStart122:
; ?FLOC__Kinematics_SingleAxisStart?T55 start address is: 16 (R4)
ORI	R4, R0, 255
; ?FLOC__Kinematics_SingleAxisStart?T55 end address is: 16 (R4)
J	L_Kinematics_SingleAxisStart16
NOP	
L_Kinematics_SingleAxisStart15:
; ?FLOC__Kinematics_SingleAxisStart?T55 start address is: 16 (R4)
ORI	R4, R0, 1
; ?FLOC__Kinematics_SingleAxisStart?T55 end address is: 16 (R4)
L_Kinematics_SingleAxisStart16:
; ?FLOC__Kinematics_SingleAxisStart?T55 start address is: 16 (R4)
SEB	R2, R4
; ?FLOC__Kinematics_SingleAxisStart?T55 end address is: 16 (R4)
SH	R2, 0(R3)
;Kinematics.c,118 :: 		SV.Single_Dual = SINGLE;
LBU	R2, Offset(_SV+0)(GP)
INS	R2, R0, 2, 1
SB	R2, Offset(_SV+0)(GP)
;Kinematics.c,119 :: 		STPS[axis_No].step_count = 0;
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 32
SW	R0, 0(R2)
;Kinematics.c,120 :: 		STPS[axis_No].mmToTravel = dist;
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 56
SW	R25, 0(R2)
;Kinematics.c,122 :: 		Step_Cycle(axis_No);
SEH	R25, R26
JAL	_Step_Cycle+0
NOP	
;Kinematics.c,123 :: 		}
L_end_SingleAxisStart:
LW	R25, 4(SP)
LW	RA, 0(SP)
ADDIU	SP, SP, 24
JR	RA
NOP	
; end of Kinematics_SingleAxisStart
_DualAxisStep:
;Kinematics.c,128 :: 		void DualAxisStep(float axis_a,float axis_b,int axisA,int axisB,float speed){
ADDIU	SP, SP, -152
SW	RA, 0(SP)
;Kinematics.c,131 :: 		SV.dif  = 0;
SW	R0, Offset(_SV+20)(GP)
;Kinematics.c,133 :: 		while(DMA_IsOn(1));
L_DualAxisStep17:
SH	R26, 4(SP)
SH	R25, 6(SP)
SWC1	S14, 8(SP)
SWC1	S13, 12(SP)
SWC1	S12, 16(SP)
ORI	R25, R0, 1
JAL	_DMA_IsOn+0
NOP	
LWC1	S12, 16(SP)
LWC1	S13, 12(SP)
LWC1	S14, 8(SP)
LH	R25, 6(SP)
LH	R26, 4(SP)
BNE	R2, R0, L__DualAxisStep125
NOP	
J	L_DualAxisStep18
NOP	
L__DualAxisStep125:
J	L_DualAxisStep17
NOP	
L_DualAxisStep18:
;Kinematics.c,135 :: 		,axis_a,STPS[axisA].steps_abs_position,axis_b,STPS[axisB].steps_abs_position);
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 60
LW	R4, 0(R2)
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 60
LW	R3, 0(R2)
;Kinematics.c,134 :: 		dma_printf("axis_a:= %f\tabs_posA:= %l\taxis_b:= %f\tabs_posB:= %l\n"
ADDIU	R23, SP, 28
ADDIU	R22, R23, 53
LUI	R24, hi_addr(?ICS?lstr1_Kinematics+0)
ORI	R24, R24, lo_addr(?ICS?lstr1_Kinematics+0)
JAL	___CC2DW+0
NOP	
ADDIU	R2, SP, 28
;Kinematics.c,135 :: 		,axis_a,STPS[axisA].steps_abs_position,axis_b,STPS[axisB].steps_abs_position);
SH	R26, 4(SP)
SH	R25, 6(SP)
SWC1	S14, 8(SP)
SWC1	S13, 12(SP)
SWC1	S12, 16(SP)
ADDIU	SP, SP, -20
SW	R4, 16(SP)
SWC1	S13, 12(SP)
SW	R3, 8(SP)
SWC1	S12, 4(SP)
;Kinematics.c,134 :: 		dma_printf("axis_a:= %f\tabs_posA:= %l\taxis_b:= %f\tabs_posB:= %l\n"
SW	R2, 0(SP)
;Kinematics.c,135 :: 		,axis_a,STPS[axisA].steps_abs_position,axis_b,STPS[axisB].steps_abs_position);
JAL	_dma_printf+0
NOP	
ADDIU	SP, SP, 20
LWC1	S12, 16(SP)
LWC1	S13, 12(SP)
LWC1	S14, 8(SP)
LH	R25, 6(SP)
LH	R26, 4(SP)
;Kinematics.c,138 :: 		SV.Single_Dual = DUAL;
LBU	R2, Offset(_SV+0)(GP)
ORI	R2, R2, 4
SB	R2, Offset(_SV+0)(GP)
;Kinematics.c,139 :: 		Single_Axis_Enable(axisA);
SH	R26, 4(SP)
SH	R25, 6(SP)
SWC1	S14, 8(SP)
SWC1	S13, 12(SP)
SWC1	S12, 16(SP)
JAL	_Single_Axis_Enable+0
NOP	
LWC1	S12, 16(SP)
LWC1	S13, 12(SP)
LWC1	S14, 8(SP)
LH	R25, 6(SP)
LH	R26, 4(SP)
;Kinematics.c,140 :: 		Single_Axis_Enable(axisB);
SH	R26, 4(SP)
SH	R25, 6(SP)
SWC1	S14, 8(SP)
SWC1	S13, 12(SP)
SWC1	S12, 16(SP)
SEH	R25, R26
JAL	_Single_Axis_Enable+0
NOP	
LWC1	S12, 16(SP)
LWC1	S13, 12(SP)
LWC1	S14, 8(SP)
LH	R25, 6(SP)
LH	R26, 4(SP)
;Kinematics.c,143 :: 		if(!SV.cir)
LBU	R2, Offset(_SV+0)(GP)
EXT	R2, R2, 1, 1
BEQ	R2, R0, L__DualAxisStep126
NOP	
J	L_DualAxisStep19
NOP	
L__DualAxisStep126:
;Kinematics.c,144 :: 		speed = RPS_FROM_MMPMIN(speed);
LUI	R2, 16928
ORI	R2, R2, 0
MTC1	R2, S0
DIV.S 	S1, S14, S0
LUI	R2, 17008
ORI	R2, R2, 0
MTC1	R2, S0
DIV.S 	S0, S1, S0
MOV.S 	S14, S0
L_DualAxisStep19:
;Kinematics.c,147 :: 		tempA = belt_steps(axis_a,axisA);
SH	R26, 4(SP)
SH	R25, 6(SP)
SWC1	S14, 8(SP)
SWC1	S13, 12(SP)
SWC1	S12, 16(SP)
JAL	_belt_steps+0
NOP	
LWC1	S12, 16(SP)
LWC1	S13, 12(SP)
LWC1	S14, 8(SP)
LH	R25, 6(SP)
LH	R26, 4(SP)
; tempA start address is: 16 (R4)
MOVZ	R4, R2, R0
;Kinematics.c,148 :: 		tempB = belt_steps(axis_b,axisB);
SW	R4, 4(SP)
SH	R26, 8(SP)
SH	R25, 10(SP)
SWC1	S14, 12(SP)
SWC1	S13, 16(SP)
SWC1	S12, 20(SP)
SEH	R25, R26
MOV.S 	S12, S13
JAL	_belt_steps+0
NOP	
LWC1	S12, 20(SP)
LWC1	S13, 16(SP)
LWC1	S14, 12(SP)
LH	R25, 10(SP)
LH	R26, 8(SP)
LW	R4, 4(SP)
; tempB start address is: 20 (R5)
MOVZ	R5, R2, R0
;Kinematics.c,151 :: 		if(gc.absolute_mode == true){
LBU	R3, Offset(_gc+5)(GP)
ORI	R2, R0, 1
BEQ	R3, R2, L__DualAxisStep127
NOP	
J	L__DualAxisStep97
NOP	
L__DualAxisStep127:
;Kinematics.c,153 :: 		tempA = tempA - STPS[axisA].steps_abs_position;
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 60
LW	R2, 0(R2)
SUBU	R2, R4, R2
; tempA end address is: 16 (R4)
; tempA start address is: 24 (R6)
MOVZ	R6, R2, R0
;Kinematics.c,154 :: 		tempB = tempB - STPS[axisB].steps_abs_position;
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 60
LW	R2, 0(R2)
SUBU	R2, R5, R2
MOVZ	R5, R2, R0
; tempA end address is: 24 (R6)
; tempB end address is: 20 (R5)
;Kinematics.c,155 :: 		}
J	L_DualAxisStep20
NOP	
L__DualAxisStep97:
;Kinematics.c,151 :: 		if(gc.absolute_mode == true){
MOVZ	R6, R4, R0
;Kinematics.c,155 :: 		}
L_DualAxisStep20:
;Kinematics.c,158 :: 		Set_Axisdirection(tempA,axisA);
; tempB start address is: 20 (R5)
; tempA start address is: 24 (R6)
SH	R26, 4(SP)
SH	R25, 6(SP)
SEH	R26, R25
MOVZ	R25, R6, R0
JAL	Kinematics_Set_Axisdirection+0
NOP	
LH	R25, 6(SP)
LH	R26, 4(SP)
;Kinematics.c,159 :: 		STPS[axisA].axis_dir = Direction(tempA);
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R3, R2, 2
SLTI	R2, R6, 0
BNE	R2, R0, L__DualAxisStep128
NOP	
J	L_DualAxisStep21
NOP	
L__DualAxisStep128:
; ?FLOC___DualAxisStep?T115 start address is: 16 (R4)
ORI	R4, R0, 255
; ?FLOC___DualAxisStep?T115 end address is: 16 (R4)
J	L_DualAxisStep22
NOP	
L_DualAxisStep21:
; ?FLOC___DualAxisStep?T115 start address is: 16 (R4)
ORI	R4, R0, 1
; ?FLOC___DualAxisStep?T115 end address is: 16 (R4)
L_DualAxisStep22:
; ?FLOC___DualAxisStep?T115 start address is: 16 (R4)
SEB	R2, R4
; ?FLOC___DualAxisStep?T115 end address is: 16 (R4)
SH	R2, 0(R3)
;Kinematics.c,160 :: 		Set_Axisdirection(tempB,axisB);
SH	R25, 4(SP)
MOVZ	R25, R5, R0
JAL	Kinematics_Set_Axisdirection+0
NOP	
LH	R25, 4(SP)
;Kinematics.c,161 :: 		STPS[axisB].axis_dir = Direction(tempB);
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R3, R2, 2
SLTI	R2, R5, 0
BNE	R2, R0, L__DualAxisStep129
NOP	
J	L_DualAxisStep23
NOP	
L__DualAxisStep129:
; ?FLOC___DualAxisStep?T123 start address is: 16 (R4)
ORI	R4, R0, 255
; ?FLOC___DualAxisStep?T123 end address is: 16 (R4)
J	L_DualAxisStep24
NOP	
L_DualAxisStep23:
; ?FLOC___DualAxisStep?T123 start address is: 16 (R4)
ORI	R4, R0, 1
; ?FLOC___DualAxisStep?T123 end address is: 16 (R4)
L_DualAxisStep24:
; ?FLOC___DualAxisStep?T123 start address is: 16 (R4)
SEB	R2, R4
; ?FLOC___DualAxisStep?T123 end address is: 16 (R4)
SH	R2, 0(R3)
;Kinematics.c,164 :: 		STPS[axisA].dist =  labs(tempA);
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 36
SW	R2, 148(SP)
SH	R25, 4(SP)
MOVZ	R25, R6, R0
JAL	_labs+0
NOP	
LW	R3, 148(SP)
SW	R2, 0(R3)
;Kinematics.c,165 :: 		STPS[axisB].dist =  labs(tempB);
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 36
SW	R2, 148(SP)
MOVZ	R25, R5, R0
JAL	_labs+0
NOP	
LH	R25, 4(SP)
LW	R3, 148(SP)
SW	R2, 0(R3)
; tempA end address is: 24 (R6)
; tempB end address is: 20 (R5)
;Kinematics.c,168 :: 		while(DMA_IsOn(1));
L_DualAxisStep25:
; tempA start address is: 24 (R6)
; tempB start address is: 20 (R5)
SW	R5, 4(SP)
SW	R6, 8(SP)
SH	R26, 12(SP)
SH	R25, 14(SP)
SWC1	S14, 16(SP)
SWC1	S13, 20(SP)
SWC1	S12, 24(SP)
ORI	R25, R0, 1
JAL	_DMA_IsOn+0
NOP	
LWC1	S12, 24(SP)
LWC1	S13, 20(SP)
LWC1	S14, 16(SP)
LH	R25, 14(SP)
LH	R26, 12(SP)
LW	R6, 8(SP)
LW	R5, 4(SP)
BNE	R2, R0, L__DualAxisStep131
NOP	
J	L_DualAxisStep26
NOP	
L__DualAxisStep131:
J	L_DualAxisStep25
NOP	
L_DualAxisStep26:
;Kinematics.c,170 :: 		,tempA,STPS[axisA].dist,tempB,STPS[axisB].dist);
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 36
LW	R4, 0(R2)
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 36
LW	R3, 0(R2)
;Kinematics.c,169 :: 		dma_printf("tempA:= %l\tSTPS[axisA].dist:= %l\ttempB:= %l\tSTPS[axisB].dist:= %l\n"
ADDIU	R23, SP, 81
ADDIU	R22, R23, 67
LUI	R24, hi_addr(?ICS?lstr2_Kinematics+0)
ORI	R24, R24, lo_addr(?ICS?lstr2_Kinematics+0)
JAL	___CC2DW+0
NOP	
ADDIU	R2, SP, 81
;Kinematics.c,170 :: 		,tempA,STPS[axisA].dist,tempB,STPS[axisB].dist);
SW	R5, 4(SP)
SW	R6, 8(SP)
SH	R26, 12(SP)
SH	R25, 14(SP)
SWC1	S14, 16(SP)
SWC1	S13, 20(SP)
SWC1	S12, 24(SP)
ADDIU	SP, SP, -20
SW	R4, 16(SP)
SW	R5, 12(SP)
SW	R3, 8(SP)
SW	R6, 4(SP)
;Kinematics.c,169 :: 		dma_printf("tempA:= %l\tSTPS[axisA].dist:= %l\ttempB:= %l\tSTPS[axisB].dist:= %l\n"
SW	R2, 0(SP)
;Kinematics.c,170 :: 		,tempA,STPS[axisA].dist,tempB,STPS[axisB].dist);
JAL	_dma_printf+0
NOP	
ADDIU	SP, SP, 20
LWC1	S12, 24(SP)
LWC1	S13, 20(SP)
LWC1	S14, 16(SP)
LH	R25, 14(SP)
LH	R26, 12(SP)
LW	R6, 8(SP)
LW	R5, 4(SP)
;Kinematics.c,174 :: 		if(STPS[axisA].dist >= STPS[axisB].dist){
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 36
LW	R4, 0(R2)
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 36
LW	R2, 0(R2)
SLT	R2, R4, R2
BEQ	R2, R0, L__DualAxisStep132
NOP	
J	L_DualAxisStep27
NOP	
L__DualAxisStep132:
; tempB end address is: 20 (R5)
;Kinematics.c,175 :: 		/* if(!SV.cir)*/speed_cntr_Move(tempA,speed,axisA);
SH	R26, 4(SP)
SH	R25, 6(SP)
SWC1	S13, 8(SP)
SWC1	S12, 12(SP)
SEH	R26, R25
MOV.S 	S12, S14
MOVZ	R25, R6, R0
; tempA end address is: 24 (R6)
JAL	_speed_cntr_Move+0
NOP	
LWC1	S12, 12(SP)
LWC1	S13, 8(SP)
LH	R25, 6(SP)
LH	R26, 4(SP)
;Kinematics.c,176 :: 		STPS[axisB].step_delay = STPS[axisA].step_delay;
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R4, R2, 8
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 8
LW	R2, 0(R2)
SW	R2, 0(R4)
;Kinematics.c,177 :: 		STPS[axisB].accel_count = STPS[axisA].accel_count;
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R4, R2, 24
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 24
LW	R2, 0(R2)
SW	R2, 0(R4)
;Kinematics.c,178 :: 		SV.dif = BresDiffVal(STPS[axisB].dist,STPS[axisA].dist);//2*(SV.dy - SV.dx);
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 36
LW	R4, 0(R2)
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R3, R2, R3
ADDIU	R2, R3, 36
LW	R2, 0(R2)
SUBU	R2, R4, R2
SLL	R2, R2, 1
SW	R2, Offset(_SV+20)(GP)
;Kinematics.c,179 :: 		STPS[axisA].master = MASTER;
LBU	R2, 0(R3)
ORI	R2, R2, 1
SB	R2, 0(R3)
;Kinematics.c,180 :: 		STPS[axisB].master = SLAVE;
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R3, R2, R3
LBU	R2, 0(R3)
INS	R2, R0, 0, 1
SB	R2, 0(R3)
;Kinematics.c,181 :: 		if(SV.prevA == axis_a){
LWC1	S0, Offset(_SV+36)(GP)
C.EQ.S 	0, S0, S12
BC1T	0, L__DualAxisStep133
NOP	
J	L_DualAxisStep28
NOP	
L__DualAxisStep133:
;Kinematics.c,182 :: 		bit_false(SV.mode_complete,bit(axisA));
SEH	R3, R25
ORI	R2, R0, 1
SLLV	R2, R2, R3
NOR	R3, R2, R0
LH	R2, Offset(_SV+2)(GP)
AND	R2, R2, R3
SH	R2, Offset(_SV+2)(GP)
;Kinematics.c,183 :: 		}
L_DualAxisStep28:
;Kinematics.c,184 :: 		}else{
J	L_DualAxisStep29
NOP	
L_DualAxisStep27:
;Kinematics.c,185 :: 		/* if(!SV.cir)*/speed_cntr_Move(tempB,speed,axisB);
; tempB start address is: 20 (R5)
SH	R26, 4(SP)
SH	R25, 6(SP)
SWC1	S13, 8(SP)
SWC1	S12, 12(SP)
MOV.S 	S12, S14
MOVZ	R25, R5, R0
; tempB end address is: 20 (R5)
JAL	_speed_cntr_Move+0
NOP	
LWC1	S12, 12(SP)
LWC1	S13, 8(SP)
LH	R25, 6(SP)
LH	R26, 4(SP)
;Kinematics.c,186 :: 		STPS[axisA].step_delay = STPS[axisB].step_delay;
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R4, R2, 8
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 8
LW	R2, 0(R2)
SW	R2, 0(R4)
;Kinematics.c,187 :: 		STPS[axisA].accel_count = STPS[axisB].accel_count;
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R4, R2, 24
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 24
LW	R2, 0(R2)
SW	R2, 0(R4)
;Kinematics.c,188 :: 		SV.dif = BresDiffVal(STPS[axisA].dist,STPS[axisB].dist);//2* (SV.dx - SV.dy);
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R5, R2, R3
ADDIU	R2, R5, 36
LW	R4, 0(R2)
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 36
LW	R2, 0(R2)
SUBU	R2, R4, R2
SLL	R2, R2, 1
SW	R2, Offset(_SV+20)(GP)
;Kinematics.c,189 :: 		STPS[axisA].master = SLAVE;
LBU	R2, 0(R5)
INS	R2, R0, 0, 1
SB	R2, 0(R5)
;Kinematics.c,190 :: 		STPS[axisB].master = MASTER;
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R3, R2, R3
LBU	R2, 0(R3)
ORI	R2, R2, 1
SB	R2, 0(R3)
;Kinematics.c,191 :: 		if(SV.prevB == axis_b){
LWC1	S0, Offset(_SV+40)(GP)
C.EQ.S 	0, S0, S13
BC1T	0, L__DualAxisStep134
NOP	
J	L_DualAxisStep30
NOP	
L__DualAxisStep134:
;Kinematics.c,192 :: 		bit_false(SV.mode_complete,bit(axisB));
SEH	R3, R26
ORI	R2, R0, 1
SLLV	R2, R2, R3
NOR	R3, R2, R0
LH	R2, Offset(_SV+2)(GP)
AND	R2, R2, R3
SH	R2, Offset(_SV+2)(GP)
;Kinematics.c,193 :: 		}
L_DualAxisStep30:
;Kinematics.c,194 :: 		}
L_DualAxisStep29:
;Kinematics.c,197 :: 		SV.prevA = axis_a;
SWC1	S12, Offset(_SV+36)(GP)
;Kinematics.c,198 :: 		SV.prevB = axis_b;
SWC1	S13, Offset(_SV+40)(GP)
;Kinematics.c,200 :: 		STPS[axisA].step_count = 0;
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 32
SW	R0, 0(R2)
;Kinematics.c,201 :: 		STPS[axisB].step_count = 0;
SEH	R3, R26
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 32
SW	R0, 0(R2)
;Kinematics.c,205 :: 		if(SV.mode_complete == 0){
LH	R2, Offset(_SV+2)(GP)
BEQ	R2, R0, L__DualAxisStep135
NOP	
J	L_DualAxisStep31
NOP	
L__DualAxisStep135:
;Kinematics.c,206 :: 		StopAxis(axisA);
SH	R26, 4(SP)
SH	R25, 6(SP)
JAL	_StopAxis+0
NOP	
LH	R25, 6(SP)
LH	R26, 4(SP)
;Kinematics.c,207 :: 		StopAxis(axisB);
SH	R26, 4(SP)
SH	R25, 6(SP)
SEH	R25, R26
JAL	_StopAxis+0
NOP	
LH	R25, 6(SP)
LH	R26, 4(SP)
;Kinematics.c,208 :: 		return;
J	L_end_DualAxisStep
NOP	
;Kinematics.c,209 :: 		}
L_DualAxisStep31:
;Kinematics.c,211 :: 		Start_Interpolation(axisA,axisB);
SH	R26, 4(SP)
SH	R25, 6(SP)
JAL	_Start_Interpolation+0
NOP	
LH	R25, 6(SP)
LH	R26, 4(SP)
;Kinematics.c,212 :: 		}
L_end_DualAxisStep:
LW	RA, 0(SP)
ADDIU	SP, SP, 152
JR	RA
NOP	
; end of _DualAxisStep
_mc_arc:
;Kinematics.c,248 :: 		, float radius, char isclockwise){
ADDIU	SP, SP, -212
SW	RA, 0(SP)
LH	R2, 212(SP)
SH	R2, 212(SP)
LH	R2, 214(SP)
SH	R2, 214(SP)
; invert_feed_rate start address is: 8 (R2)
LBU	R2, 216(SP)
; invert_feed_rate end address is: 8 (R2)
; isclockwise start address is: 28 (R7)
LBU	R7, 217(SP)
;Kinematics.c,249 :: 		float arc_target[3] = {0};
ADDIU	R23, SP, 196
ADDIU	R22, R23, 14
LUI	R24, hi_addr(?ICSmc_arc_arc_target_L0+0)
ORI	R24, R24, lo_addr(?ICSmc_arc_arc_target_L0+0)
JAL	___CC2DW+0
NOP	
;Kinematics.c,255 :: 		int count = 0;
;Kinematics.c,256 :: 		char limit_error = 0;
;Kinematics.c,259 :: 		center_axis0            = position[axis_0] + offset[axis_0];
SEH	R2, R28
SLL	R6, R2, 2
ADDU	R2, R25, R6
LWC1	S1, 0(R2)
ADDU	R5, R27, R6
LWC1	S0, 0(R5)
ADD.S 	S3, S1, S0
SWC1	S3, 24(SP)
;Kinematics.c,260 :: 		center_axis1            = position[axis_1] + offset[axis_1];
LH	R2, 212(SP)
SLL	R4, R2, 2
ADDU	R2, R25, R4
LWC1	S1, 0(R2)
ADDU	R3, R27, R4
LWC1	S0, 0(R3)
ADD.S 	S2, S1, S0
SWC1	S2, 28(SP)
;Kinematics.c,261 :: 		r_axis0                 = -offset[axis_0];  // Radius vector from center to current location
LWC1	S1, 0(R5)
MOVZ	R2, R0, R0
MTC1	R2, S0
SUB.S 	S0, S0, S1
SWC1	S0, 32(SP)
;Kinematics.c,262 :: 		r_axis1                 = -offset[axis_1];
LWC1	S1, 0(R3)
MOVZ	R2, R0, R0
MTC1	R2, S0
SUB.S 	S0, S0, S1
SWC1	S0, 36(SP)
;Kinematics.c,263 :: 		rt_axis0                = target[axis_0] - center_axis0;
ADDU	R2, R26, R6
LWC1	S0, 0(R2)
SUB.S 	S0, S0, S3
; rt_axis0 start address is: 32 (R8)
MOV.S 	S4, S0
;Kinematics.c,264 :: 		rt_axis1                = target[axis_1] - center_axis1;
ADDU	R2, R26, R4
LWC1	S0, 0(R2)
SUB.S 	S0, S0, S2
; rt_axis1 start address is: 40 (R10)
MOV.S 	S5, S0
;Kinematics.c,265 :: 		linear_travel           = target[axis_linear] - position[axis_linear];
LH	R2, 214(SP)
SLL	R4, R2, 2
ADDU	R2, R26, R4
LWC1	S1, 0(R2)
ADDU	R3, R25, R4
LWC1	S0, 0(R3)
SUB.S 	S0, S1, S0
; linear_travel start address is: 24 (R6)
MOV.S 	S3, S0
;Kinematics.c,267 :: 		arc_target[axis_linear] = position[axis_linear];
ADDIU	R2, SP, 196
ADDU	R2, R2, R4
LWC1	S0, 0(R3)
SWC1	S0, 0(R2)
;Kinematics.c,274 :: 		x = r_axis0*rt_axis1-r_axis1*rt_axis0;
LWC1	S0, 32(SP)
MUL.S 	S1, S0, S5
LWC1	S0, 36(SP)
MUL.S 	S0, S0, S4
SUB.S 	S2, S1, S0
;Kinematics.c,275 :: 		y = r_axis0*rt_axis0+r_axis1*rt_axis1;
LWC1	S0, 32(SP)
MUL.S 	S1, S0, S4
; rt_axis0 end address is: 32 (R8)
LWC1	S0, 36(SP)
MUL.S 	S0, S0, S5
; rt_axis1 end address is: 40 (R10)
ADD.S 	S0, S1, S0
;Kinematics.c,283 :: 		angular_travel = atan2(y,x);
SW	R26, 4(SP)
SWC1	S13, 8(SP)
SWC1	S12, 12(SP)
MOV.S 	S13, S2
MOV.S 	S12, S0
JAL	_atan2+0
NOP	
LWC1	S12, 12(SP)
LWC1	S13, 8(SP)
LW	R26, 4(SP)
; angular_travel start address is: 32 (R8)
MOV.S 	S4, S0
;Kinematics.c,286 :: 		if(isclockwise > 0) {
ANDI	R2, R7, 255
; isclockwise end address is: 28 (R7)
SLTIU	R2, R2, 1
BEQ	R2, R0, L__mc_arc137
NOP	
J	L_mc_arc32
NOP	
L__mc_arc137:
;Kinematics.c,287 :: 		if (angular_travel >= 0)
MOVZ	R2, R0, R0
MTC1	R2, S0
C.LT.S 	0, S4, S0
BC1F	0, L__mc_arc138
NOP	
J	L__mc_arc101
NOP	
L__mc_arc138:
;Kinematics.c,288 :: 		angular_travel -= PIx2;
LUI	R2, 16585
ORI	R2, R2, 4059
MTC1	R2, S0
SUB.S 	S0, S4, S0
; angular_travel end address is: 32 (R8)
; angular_travel start address is: 8 (R2)
MOV.S 	S1, S0
; angular_travel end address is: 8 (R2)
MOV.S 	S4, S1
J	L_mc_arc33
NOP	
L__mc_arc101:
;Kinematics.c,287 :: 		if (angular_travel >= 0)
;Kinematics.c,288 :: 		angular_travel -= PIx2;
L_mc_arc33:
;Kinematics.c,289 :: 		}else {
; angular_travel start address is: 32 (R8)
J	L_mc_arc34
NOP	
L_mc_arc32:
;Kinematics.c,290 :: 		if(angular_travel <= 0)
MOVZ	R2, R0, R0
MTC1	R2, S0
C.LT.S 	0, S0, S4
BC1F	0, L__mc_arc139
NOP	
J	L__mc_arc102
NOP	
L__mc_arc139:
;Kinematics.c,291 :: 		angular_travel += PIx2;
LUI	R2, 16585
ORI	R2, R2, 4059
MTC1	R2, S0
ADD.S 	S0, S4, S0
; angular_travel end address is: 32 (R8)
; angular_travel start address is: 8 (R2)
MOV.S 	S1, S0
; angular_travel end address is: 8 (R2)
MOV.S 	S4, S1
J	L_mc_arc35
NOP	
L__mc_arc102:
;Kinematics.c,290 :: 		if(angular_travel <= 0)
;Kinematics.c,291 :: 		angular_travel += PIx2;
L_mc_arc35:
;Kinematics.c,292 :: 		}
; angular_travel start address is: 32 (R8)
; angular_travel end address is: 32 (R8)
L_mc_arc34:
;Kinematics.c,311 :: 		mm_of_travel = hypot(angular_travel*radius, fabs(linear_travel));
; angular_travel start address is: 32 (R8)
SWC1	S12, 4(SP)
MOV.S 	S12, S3
JAL	_fabs+0
NOP	
LWC1	S12, 4(SP)
MUL.S 	S1, S4, S13
SW	R25, 4(SP)
SWC1	S12, 8(SP)
MOV.S 	S13, S0
MOV.S 	S12, S1
JAL	_hypot+0
NOP	
LWC1	S12, 8(SP)
LW	R25, 4(SP)
; mm_of_travel start address is: 16 (R4)
MOV.S 	S2, S0
;Kinematics.c,312 :: 		if (mm_of_travel == 0.0) { return; }
MOVZ	R2, R0, R0
MTC1	R2, S1
C.EQ.S 	0, S0, S1
BC1T	0, L__mc_arc140
NOP	
J	L_mc_arc36
NOP	
L__mc_arc140:
; linear_travel end address is: 24 (R6)
; angular_travel end address is: 32 (R8)
; mm_of_travel end address is: 16 (R4)
J	L_end_mc_arc
NOP	
L_mc_arc36:
;Kinematics.c,314 :: 		segments = floor(mm_of_travel/settings.mm_per_arc_segment);
; mm_of_travel start address is: 16 (R4)
; angular_travel start address is: 32 (R8)
; linear_travel start address is: 24 (R6)
LWC1	S0, Offset(_settings+36)(GP)
DIV.S 	S0, S2, S0
; mm_of_travel end address is: 16 (R4)
SWC1	S12, 4(SP)
MOV.S 	S12, S0
JAL	_floor+0
NOP	
LWC1	S12, 4(SP)
SWC1	S0, 48(SP)
;Kinematics.c,323 :: 		theta_per_segment = angular_travel/segments;
DIV.S 	S2, S4, S0
; angular_travel end address is: 32 (R8)
SWC1	S2, 40(SP)
;Kinematics.c,328 :: 		linear_per_segment = linear_travel/segments;
DIV.S 	S0, S3, S0
; linear_travel end address is: 24 (R6)
SWC1	S0, 44(SP)
;Kinematics.c,331 :: 		cos_T = 1-0.5*theta_per_segment*theta_per_segment; // Small angle approximation
LUI	R2, 16128
ORI	R2, R2, 0
MTC1	R2, S0
MUL.S 	S0, S0, S2
MUL.S 	S1, S0, S2
LUI	R2, 16256
ORI	R2, R2, 0
MTC1	R2, S0
SUB.S 	S0, S0, S1
SWC1	S0, 52(SP)
;Kinematics.c,332 :: 		sin_T = theta_per_segment;
SWC1	S2, 56(SP)
;Kinematics.c,335 :: 		nPx = arc_target[axis_0] = position[axis_0];
ADDIU	R4, SP, 196
SEH	R2, R28
SLL	R2, R2, 2
ADDU	R3, R4, R2
ADDU	R2, R25, R2
LWC1	S0, 0(R2)
SWC1	S0, 0(R3)
;Kinematics.c,336 :: 		nPy = arc_target[axis_1] = position[axis_1];
LH	R2, 212(SP)
SLL	R2, R2, 2
ADDU	R3, R4, R2
ADDU	R2, R25, R2
LWC1	S0, 0(R2)
SWC1	S0, 0(R3)
;Kinematics.c,337 :: 		DisableStepperInterrupt(X);
SH	R28, 4(SP)
SW	R27, 8(SP)
SW	R26, 12(SP)
SW	R25, 16(SP)
SWC1	S12, 20(SP)
MOVZ	R25, R0, R0
JAL	_DisableStepperInterrupt+0
NOP	
;Kinematics.c,338 :: 		DisableStepperInterrupt(Y);
ORI	R25, R0, 1
JAL	_DisableStepperInterrupt+0
NOP	
LWC1	S12, 20(SP)
LW	R25, 16(SP)
LW	R26, 12(SP)
LW	R27, 8(SP)
LH	R28, 4(SP)
;Kinematics.c,352 :: 		for (i = FLOAT_INC1; i<segments; i+=FLOAT_INC1) {
LUI	R2, 16256
ORI	R2, R2, 0
SW	R2, 60(SP)
L_mc_arc37:
LWC1	S1, 48(SP)
LWC1	S0, 60(SP)
C.LE.S 	0, S1, S0
BC1F	0, L__mc_arc141
NOP	
J	L_mc_arc38
NOP	
L__mc_arc141:
;Kinematics.c,353 :: 		if (count < settings.n_arc_correction) {
LHU	R3, Offset(_settings+48)(GP)
LH	R2, 208(SP)
SLTU	R2, R2, R3
BNE	R2, R0, L__mc_arc142
NOP	
J	L_mc_arc40
NOP	
L__mc_arc142:
;Kinematics.c,355 :: 		r_axisi = r_axis0*sin_T + r_axis1*cos_T;
LWC1	S1, 56(SP)
LWC1	S0, 32(SP)
MUL.S 	S2, S0, S1
LWC1	S1, 52(SP)
LWC1	S0, 36(SP)
MUL.S 	S0, S0, S1
ADD.S 	S3, S2, S0
;Kinematics.c,356 :: 		r_axis0 = r_axis0*cos_T - r_axis1*sin_T;
LWC1	S1, 52(SP)
LWC1	S0, 32(SP)
MUL.S 	S2, S0, S1
LWC1	S1, 56(SP)
LWC1	S0, 36(SP)
MUL.S 	S0, S0, S1
SUB.S 	S0, S2, S0
SWC1	S0, 32(SP)
;Kinematics.c,357 :: 		r_axis1 = r_axisi;
SWC1	S3, 36(SP)
;Kinematics.c,358 :: 		count++;
LH	R2, 208(SP)
ADDIU	R2, R2, 1
SH	R2, 208(SP)
;Kinematics.c,359 :: 		} else {
J	L_mc_arc41
NOP	
L_mc_arc40:
;Kinematics.c,362 :: 		cos_Ti = cos(i*theta_per_segment);
LWC1	S1, 40(SP)
LWC1	S0, 60(SP)
MUL.S 	S0, S0, S1
SW	R26, 4(SP)
SWC1	S12, 8(SP)
MOV.S 	S12, S0
JAL	_cos+0
NOP	
; cos_Ti start address is: 24 (R6)
MOV.S 	S3, S0
;Kinematics.c,363 :: 		sin_Ti = sin(i*theta_per_segment);
LWC1	S1, 40(SP)
LWC1	S0, 60(SP)
MUL.S 	S0, S0, S1
MOV.S 	S12, S0
JAL	_sin+0
NOP	
LWC1	S12, 8(SP)
LW	R26, 4(SP)
;Kinematics.c,364 :: 		r_axis0 = -offset[axis_0]*cos_Ti + offset[axis_1]*sin_Ti;
SEH	R2, R28
SLL	R2, R2, 2
ADDU	R4, R27, R2
LWC1	S2, 0(R4)
MOVZ	R2, R0, R0
MTC1	R2, S1
SUB.S 	S1, S1, S2
MUL.S 	S2, S1, S3
LH	R2, 212(SP)
SLL	R2, R2, 2
ADDU	R3, R27, R2
LWC1	S1, 0(R3)
MUL.S 	S1, S1, S0
ADD.S 	S1, S2, S1
SWC1	S1, 32(SP)
;Kinematics.c,365 :: 		r_axis1 = -offset[axis_0]*sin_Ti - offset[axis_1]*cos_Ti;
LWC1	S2, 0(R4)
MOVZ	R2, R0, R0
MTC1	R2, S1
SUB.S 	S1, S1, S2
MUL.S 	S1, S1, S0
LWC1	S0, 0(R3)
MUL.S 	S0, S0, S3
; cos_Ti end address is: 24 (R6)
SUB.S 	S0, S1, S0
SWC1	S0, 36(SP)
;Kinematics.c,366 :: 		count = 0;
SH	R0, 208(SP)
;Kinematics.c,367 :: 		}
L_mc_arc41:
;Kinematics.c,370 :: 		arc_target[axis_0] = center_axis0 + r_axis0;
ADDIU	R4, SP, 196
SEH	R2, R28
SLL	R2, R2, 2
ADDU	R2, R4, R2
LWC1	S1, 32(SP)
LWC1	S0, 24(SP)
ADD.S 	S0, S0, S1
SWC1	S0, 0(R2)
;Kinematics.c,371 :: 		arc_target[axis_1] = center_axis1 + r_axis1;
LH	R2, 212(SP)
SLL	R2, R2, 2
ADDU	R2, R4, R2
LWC1	S1, 36(SP)
LWC1	S0, 28(SP)
ADD.S 	S0, S0, S1
SWC1	S0, 0(R2)
;Kinematics.c,372 :: 		arc_target[axis_linear] += linear_per_segment;
LH	R2, 214(SP)
SLL	R2, R2, 2
ADDU	R2, R4, R2
LWC1	S1, 0(R2)
LWC1	S0, 44(SP)
ADD.S 	S0, S1, S0
SWC1	S0, 0(R2)
;Kinematics.c,373 :: 		nPx =  arc_target[axis_0] - position[axis_0];
SEH	R2, R28
SLL	R3, R2, 2
ADDU	R2, R4, R3
LWC1	S1, 0(R2)
ADDU	R2, R25, R3
LWC1	S0, 0(R2)
SUB.S 	S0, S1, S0
SWC1	S0, 64(SP)
;Kinematics.c,374 :: 		nPy =  arc_target[axis_1] - position[axis_1];
LH	R2, 212(SP)
SLL	R3, R2, 2
ADDU	R2, R4, R3
LWC1	S1, 0(R2)
ADDU	R2, R25, R3
LWC1	S0, 0(R2)
SUB.S 	S0, S1, S0
SWC1	S0, 68(SP)
;Kinematics.c,376 :: 		if(gc.absolute_mode){
LBU	R2, Offset(_gc+5)(GP)
BNE	R2, R0, L__mc_arc144
NOP	
J	L_mc_arc42
NOP	
L__mc_arc144:
;Kinematics.c,377 :: 		nPx += position[axis_0];// += nPx;//arc_target[axis_0];
SEH	R2, R28
SLL	R2, R2, 2
ADDU	R2, R25, R2
LWC1	S1, 0(R2)
LWC1	S0, 64(SP)
ADD.S 	S0, S0, S1
SWC1	S0, 64(SP)
;Kinematics.c,378 :: 		nPy += position[axis_1];// += nPy;//arc_target[axis_1];
LH	R2, 212(SP)
SLL	R2, R2, 2
ADDU	R2, R25, R2
LWC1	S1, 0(R2)
LWC1	S0, 68(SP)
ADD.S 	S0, S0, S1
SWC1	S0, 68(SP)
;Kinematics.c,379 :: 		}
L_mc_arc42:
;Kinematics.c,402 :: 		DualAxisStep(nPx,nPy,axis_0,axis_1,feed_rate);//,xy);
SH	R28, 4(SP)
SW	R27, 8(SP)
SW	R26, 12(SP)
SW	R25, 16(SP)
SWC1	S12, 20(SP)
LH	R26, 212(SP)
SEH	R25, R28
MOV.S 	S14, S12
LWC1	S13, 68(SP)
LWC1	S12, 64(SP)
JAL	_DualAxisStep+0
NOP	
LWC1	S12, 20(SP)
LW	R25, 16(SP)
LW	R26, 12(SP)
LW	R27, 8(SP)
LH	R28, 4(SP)
;Kinematics.c,407 :: 		while(1){
L_mc_arc43:
;Kinematics.c,412 :: 		if(!Get_Axis_IEnable_States()||SV.mode_complete < 1)
SH	R28, 4(SP)
SW	R27, 8(SP)
SW	R26, 12(SP)
SW	R25, 16(SP)
SWC1	S12, 20(SP)
JAL	_Get_Axis_IEnable_States+0
NOP	
LWC1	S12, 20(SP)
LW	R25, 16(SP)
LW	R26, 12(SP)
LW	R27, 8(SP)
LH	R28, 4(SP)
BNE	R2, R0, L__mc_arc146
NOP	
J	L__mc_arc100
NOP	
L__mc_arc146:
LH	R2, Offset(_SV+2)(GP)
SLTI	R2, R2, 1
BEQ	R2, R0, L__mc_arc147
NOP	
J	L__mc_arc99
NOP	
L__mc_arc147:
J	L_mc_arc47
NOP	
L__mc_arc100:
L__mc_arc99:
;Kinematics.c,413 :: 		break;
J	L_mc_arc44
NOP	
L_mc_arc47:
;Kinematics.c,414 :: 		}
J	L_mc_arc43
NOP	
L_mc_arc44:
;Kinematics.c,427 :: 		while(DMA_IsOn(1));
L_mc_arc48:
SH	R28, 4(SP)
SW	R27, 8(SP)
SW	R26, 12(SP)
SW	R25, 16(SP)
SWC1	S12, 20(SP)
ORI	R25, R0, 1
JAL	_DMA_IsOn+0
NOP	
LWC1	S12, 20(SP)
LW	R25, 16(SP)
LW	R26, 12(SP)
LW	R27, 8(SP)
LH	R28, 4(SP)
BNE	R2, R0, L__mc_arc149
NOP	
J	L_mc_arc49
NOP	
L__mc_arc149:
J	L_mc_arc48
NOP	
L_mc_arc49:
;Kinematics.c,433 :: 		,i,segments,nPx,nPy,target[axis_0],target[axis_1],SV.mode_complete);
LH	R2, 212(SP)
SLL	R2, R2, 2
ADDU	R2, R26, R2
LWC1	S1, 0(R2)
SEH	R2, R28
SLL	R2, R2, 2
ADDU	R2, R26, R2
LWC1	S0, 0(R2)
;Kinematics.c,432 :: 		[SV.mode_complete:= %d\r\n"
ADDIU	R23, SP, 72
ADDIU	R22, R23, 121
LUI	R24, hi_addr(?ICS?lstr3_Kinematics+0)
ORI	R24, R24, lo_addr(?ICS?lstr3_Kinematics+0)
JAL	___CC2DW+0
NOP	
ADDIU	R3, SP, 72
;Kinematics.c,433 :: 		,i,segments,nPx,nPy,target[axis_0],target[axis_1],SV.mode_complete);
LH	R2, Offset(_SV+2)(GP)
SH	R28, 4(SP)
SW	R27, 8(SP)
SW	R26, 12(SP)
SW	R25, 16(SP)
SWC1	S12, 20(SP)
ADDIU	SP, SP, -32
SH	R2, 28(SP)
SWC1	S1, 24(SP)
SWC1	S0, 20(SP)
LW	R2, 100(SP)
SW	R2, 16(SP)
LW	R2, 96(SP)
SW	R2, 12(SP)
LW	R2, 80(SP)
SW	R2, 8(SP)
LW	R2, 92(SP)
SW	R2, 4(SP)
;Kinematics.c,432 :: 		[SV.mode_complete:= %d\r\n"
SW	R3, 0(SP)
;Kinematics.c,433 :: 		,i,segments,nPx,nPy,target[axis_0],target[axis_1],SV.mode_complete);
JAL	_dma_printf+0
NOP	
ADDIU	SP, SP, 32
LWC1	S12, 20(SP)
LW	R25, 16(SP)
LW	R26, 12(SP)
LW	R27, 8(SP)
LH	R28, 4(SP)
;Kinematics.c,352 :: 		for (i = FLOAT_INC1; i<segments; i+=FLOAT_INC1) {
LWC1	S1, 60(SP)
LUI	R2, 16256
ORI	R2, R2, 0
MTC1	R2, S0
ADD.S 	S0, S1, S0
SWC1	S0, 60(SP)
;Kinematics.c,436 :: 		}
J	L_mc_arc37
NOP	
L_mc_arc38:
;Kinematics.c,439 :: 		DualAxisStep(target[axis_0],target[axis_1],axis_0,axis_1,feed_rate);
LH	R2, 212(SP)
SLL	R2, R2, 2
ADDU	R2, R26, R2
LWC1	S1, 0(R2)
SEH	R2, R28
SLL	R2, R2, 2
ADDU	R2, R26, R2
LWC1	S0, 0(R2)
SH	R28, 4(SP)
SW	R27, 8(SP)
SW	R26, 12(SP)
SW	R25, 16(SP)
LH	R26, 212(SP)
SEH	R25, R28
MOV.S 	S14, S12
MOV.S 	S13, S1
MOV.S 	S12, S0
JAL	_DualAxisStep+0
NOP	
LW	R25, 16(SP)
LW	R26, 12(SP)
LW	R27, 8(SP)
LH	R28, 4(SP)
;Kinematics.c,446 :: 		SV.mode_complete = 0;
SH	R0, Offset(_SV+2)(GP)
;Kinematics.c,447 :: 		}
L_end_mc_arc:
LW	RA, 0(SP)
ADDIU	SP, SP, 212
JR	RA
NOP	
; end of _mc_arc
_hypot:
;Kinematics.c,451 :: 		float hypot(float x, float y){
ADDIU	SP, SP, -4
SW	RA, 0(SP)
;Kinematics.c,452 :: 		return(sqrt((x*x) + (y*y)));
MUL.S 	S1, S12, S12
MUL.S 	S0, S13, S13
ADD.S 	S0, S1, S0
MOV.S 	S12, S0
JAL	_sqrt+0
NOP	
;Kinematics.c,453 :: 		}
L_end_hypot:
LW	RA, 0(SP)
ADDIU	SP, SP, 4
JR	RA
NOP	
; end of _hypot
_GetAxisDirection:
;Kinematics.c,461 :: 		int GetAxisDirection(long mm2move){
;Kinematics.c,462 :: 		return(mm2move < 0)? CCW_:CW_ ;
SLTI	R2, R25, 0
BNE	R2, R0, L__GetAxisDirection152
NOP	
J	L_GetAxisDirection50
NOP	
L__GetAxisDirection152:
; ?FLOC___GetAxisDirection?T460 start address is: 8 (R2)
ORI	R2, R0, 255
; ?FLOC___GetAxisDirection?T460 end address is: 8 (R2)
J	L_GetAxisDirection51
NOP	
L_GetAxisDirection50:
; ?FLOC___GetAxisDirection?T460 start address is: 8 (R2)
ORI	R2, R0, 1
; ?FLOC___GetAxisDirection?T460 end address is: 8 (R2)
L_GetAxisDirection51:
; ?FLOC___GetAxisDirection?T460 start address is: 8 (R2)
SEB	R2, R2
; ?FLOC___GetAxisDirection?T460 end address is: 8 (R2)
;Kinematics.c,463 :: 		}
L_end_GetAxisDirection:
JR	RA
NOP	
; end of _GetAxisDirection
__Home:
;Kinematics.c,475 :: 		int _Home(int axis){
ADDIU	SP, SP, -44
SW	RA, 0(SP)
;Kinematics.c,480 :: 		if(sys.state == STATE_IDLE){
LH	R2, Offset(_sys+2)(GP)
BEQ	R2, R0, L___Home154
NOP	
J	L__Home52
NOP	
L___Home154:
;Kinematics.c,482 :: 		speed = settings.homing_seek_rate;
LWC1	S0, Offset(_settings+28)(GP)
CVT36.S 	S0, S0
MFC1	R2, S0
SW	R2, Offset(_Home_speed_L0+0)(GP)
;Kinematics.c,485 :: 		one_shot_local = false;
LBU	R2, Offset(_Home_one_shot_local_L0+0)(GP)
INS	R2, R0, BitPos(_Home_one_shot_local_L0+0), 1
SB	R2, Offset(_Home_one_shot_local_L0+0)(GP)
;Kinematics.c,490 :: 		homing[axis].home_cnt = 0;
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 2
SH	R0, 0(R2)
;Kinematics.c,491 :: 		homing[axis].home_state = 0;
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R2, R2, R3
SH	R0, 0(R2)
;Kinematics.c,494 :: 		EnableStepper(axis);//sort this out
SH	R25, 4(SP)
JAL	_EnableStepper+0
NOP	
LH	R25, 4(SP)
;Kinematics.c,496 :: 		while(DMA_IsOn(1));
L__Home53:
SH	R25, 4(SP)
ORI	R25, R0, 1
JAL	_DMA_IsOn+0
NOP	
LH	R25, 4(SP)
BNE	R2, R0, L___Home156
NOP	
J	L__Home54
NOP	
L___Home156:
J	L__Home53
NOP	
L__Home54:
;Kinematics.c,497 :: 		dma_printf("\n%s\n"
ORI	R30, R0, 10
SB	R30, 6(SP)
ORI	R30, R0, 37
SB	R30, 7(SP)
ORI	R30, R0, 115
SB	R30, 8(SP)
ORI	R30, R0, 10
SB	R30, 9(SP)
MOVZ	R30, R0, R0
SB	R30, 10(SP)
ADDIU	R3, SP, 6
;Kinematics.c,498 :: 		,"START");
LUI	R2, hi_addr(?lstr_5_Kinematics+0)
ORI	R2, R2, lo_addr(?lstr_5_Kinematics+0)
SH	R25, 4(SP)
ADDIU	SP, SP, -8
SW	R2, 4(SP)
;Kinematics.c,497 :: 		dma_printf("\n%s\n"
SW	R3, 0(SP)
;Kinematics.c,498 :: 		,"START");
JAL	_dma_printf+0
NOP	
ADDIU	SP, SP, 8
LH	R25, 4(SP)
;Kinematics.c,500 :: 		}
L__Home52:
;Kinematics.c,504 :: 		switch(homing[axis].home_state){
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R2, R2, R3
SW	R2, 40(SP)
J	L__Home55
NOP	
;Kinematics.c,505 :: 		case 0: //start homing
L__Home57:
;Kinematics.c,507 :: 		sys.state = STATE_HOMING;
ORI	R2, R0, 5
SH	R2, Offset(_sys+2)(GP)
;Kinematics.c,510 :: 		if(!Test_Port_Pins(axis)){
SH	R25, 4(SP)
JAL	_Test_Port_Pins+0
NOP	
LH	R25, 4(SP)
BEQ	R2, R0, L___Home157
NOP	
J	L__Home58
NOP	
L___Home157:
;Kinematics.c,512 :: 		speed = settings.homing_feed_rate;
LWC1	S0, Offset(_settings+24)(GP)
CVT36.S 	S0, S0
MFC1	R2, S0
SW	R2, Offset(_Home_speed_L0+0)(GP)
;Kinematics.c,514 :: 		homing[axis].home_state = HOME_BACK_OFF;
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R3, R2, R3
ORI	R2, R0, 3
SH	R2, 0(R3)
;Kinematics.c,517 :: 		Home_Axis(12.0,settings.homing_feed_rate, axis);
LUI	R2, 16704
ORI	R2, R2, 0
SH	R25, 4(SP)
LWC1	S13, Offset(_settings+24)(GP)
MTC1	R2, S12
JAL	Kinematics_Home_Axis+0
NOP	
LH	R25, 4(SP)
;Kinematics.c,519 :: 		while(DMA_IsOn(1));
L__Home59:
SH	R25, 4(SP)
ORI	R25, R0, 1
JAL	_DMA_IsOn+0
NOP	
LH	R25, 4(SP)
BNE	R2, R0, L___Home159
NOP	
J	L__Home60
NOP	
L___Home159:
J	L__Home59
NOP	
L__Home60:
;Kinematics.c,520 :: 		dma_printf("\n%s\n"
ORI	R30, R0, 10
SB	R30, 11(SP)
ORI	R30, R0, 37
SB	R30, 12(SP)
ORI	R30, R0, 115
SB	R30, 13(SP)
ORI	R30, R0, 10
SB	R30, 14(SP)
MOVZ	R30, R0, R0
SB	R30, 15(SP)
ADDIU	R3, SP, 11
;Kinematics.c,521 :: 		,"BACK OFF");
LUI	R2, hi_addr(?lstr_7_Kinematics+0)
ORI	R2, R2, lo_addr(?lstr_7_Kinematics+0)
SH	R25, 4(SP)
ADDIU	SP, SP, -8
SW	R2, 4(SP)
;Kinematics.c,520 :: 		dma_printf("\n%s\n"
SW	R3, 0(SP)
;Kinematics.c,521 :: 		,"BACK OFF");
JAL	_dma_printf+0
NOP	
ADDIU	SP, SP, 8
LH	R25, 4(SP)
;Kinematics.c,523 :: 		}
J	L__Home61
NOP	
L__Home58:
;Kinematics.c,527 :: 		Home_Axis(-(max_sizes[axis]+100.0),speed,axis);
LWC1	S0, Offset(_Home_speed_L0+0)(GP)
CVT32.W 	S2, S0
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(_max_sizes+0)
ORI	R2, R2, lo_addr(_max_sizes+0)
ADDU	R2, R2, R3
LWC1	S1, 0(R2)
LUI	R2, 17096
ORI	R2, R2, 0
MTC1	R2, S0
ADD.S 	S1, S1, S0
MOVZ	R2, R0, R0
MTC1	R2, S0
SUB.S 	S0, S0, S1
SH	R25, 4(SP)
MOV.S 	S13, S2
MOV.S 	S12, S0
JAL	Kinematics_Home_Axis+0
NOP	
LH	R25, 4(SP)
;Kinematics.c,529 :: 		homing[axis].home_state = HOME;
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R3, R2, R3
ORI	R2, R0, 2
SH	R2, 0(R3)
;Kinematics.c,531 :: 		while(DMA_IsOn(1));
L__Home62:
SH	R25, 4(SP)
ORI	R25, R0, 1
JAL	_DMA_IsOn+0
NOP	
LH	R25, 4(SP)
BNE	R2, R0, L___Home161
NOP	
J	L__Home63
NOP	
L___Home161:
J	L__Home62
NOP	
L__Home63:
;Kinematics.c,532 :: 		dma_printf("\n%s\n"
ORI	R30, R0, 10
SB	R30, 16(SP)
ORI	R30, R0, 37
SB	R30, 17(SP)
ORI	R30, R0, 115
SB	R30, 18(SP)
ORI	R30, R0, 10
SB	R30, 19(SP)
MOVZ	R30, R0, R0
SB	R30, 20(SP)
ADDIU	R3, SP, 16
;Kinematics.c,533 :: 		,"HOME");
LUI	R2, hi_addr(?lstr_9_Kinematics+0)
ORI	R2, R2, lo_addr(?lstr_9_Kinematics+0)
SH	R25, 4(SP)
ADDIU	SP, SP, -8
SW	R2, 4(SP)
;Kinematics.c,532 :: 		dma_printf("\n%s\n"
SW	R3, 0(SP)
;Kinematics.c,533 :: 		,"HOME");
JAL	_dma_printf+0
NOP	
ADDIU	SP, SP, 8
LH	R25, 4(SP)
;Kinematics.c,535 :: 		}
L__Home61:
;Kinematics.c,537 :: 		break;
J	L__Home56
NOP	
;Kinematics.c,538 :: 		case HOME: //Home
L__Home64:
;Kinematics.c,555 :: 		if(Test_Port_Pins(axis)){
SH	R25, 4(SP)
JAL	_Test_Port_Pins+0
NOP	
LH	R25, 4(SP)
BNE	R2, R0, L___Home163
NOP	
J	L__Home65
NOP	
L___Home163:
;Kinematics.c,556 :: 		break;
J	L__Home56
NOP	
;Kinematics.c,557 :: 		}
L__Home65:
;Kinematics.c,559 :: 		speed = settings.homing_feed_rate;
LWC1	S0, Offset(_settings+24)(GP)
CVT36.S 	S0, S0
MFC1	R2, S0
SW	R2, Offset(_Home_speed_L0+0)(GP)
;Kinematics.c,561 :: 		homing[axis].home_state = HOME_BACK_OFF;
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R3, R2, R3
ORI	R2, R0, 3
SH	R2, 0(R3)
;Kinematics.c,567 :: 		if(homing[axis].home_state == HOME_BACK_OFF)
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R2, R2, R3
LHU	R2, 0(R2)
ANDI	R3, R2, 65535
ORI	R2, R0, 3
BEQ	R3, R2, L___Home164
NOP	
J	L__Home67
NOP	
L___Home164:
;Kinematics.c,569 :: 		Home_Axis(12.0,settings.homing_feed_rate, axis);
LUI	R2, 16704
ORI	R2, R2, 0
SH	R25, 4(SP)
LWC1	S13, Offset(_settings+24)(GP)
MTC1	R2, S12
JAL	Kinematics_Home_Axis+0
NOP	
LH	R25, 4(SP)
;Kinematics.c,572 :: 		while(DMA_IsOn(1));
L__Home68:
SH	R25, 4(SP)
ORI	R25, R0, 1
JAL	_DMA_IsOn+0
NOP	
LH	R25, 4(SP)
BNE	R2, R0, L___Home166
NOP	
J	L__Home69
NOP	
L___Home166:
J	L__Home68
NOP	
L__Home69:
;Kinematics.c,573 :: 		dma_printf("\n%s\n"
ORI	R30, R0, 10
SB	R30, 21(SP)
ORI	R30, R0, 37
SB	R30, 22(SP)
ORI	R30, R0, 115
SB	R30, 23(SP)
ORI	R30, R0, 10
SB	R30, 24(SP)
MOVZ	R30, R0, R0
SB	R30, 25(SP)
ADDIU	R3, SP, 21
;Kinematics.c,574 :: 		,"GOTO BACK OFF");
LUI	R2, hi_addr(?lstr_11_Kinematics+0)
ORI	R2, R2, lo_addr(?lstr_11_Kinematics+0)
SH	R25, 4(SP)
ADDIU	SP, SP, -8
SW	R2, 4(SP)
;Kinematics.c,573 :: 		dma_printf("\n%s\n"
SW	R3, 0(SP)
;Kinematics.c,574 :: 		,"GOTO BACK OFF");
JAL	_dma_printf+0
NOP	
ADDIU	SP, SP, 8
LH	R25, 4(SP)
;Kinematics.c,576 :: 		}
L__Home67:
;Kinematics.c,578 :: 		break;
J	L__Home56
NOP	
;Kinematics.c,579 :: 		case HOME_BACK_OFF: //Home retract off home position
L__Home70:
;Kinematics.c,581 :: 		if(!Test_Port_Pins(axis)){
SH	R25, 4(SP)
JAL	_Test_Port_Pins+0
NOP	
LH	R25, 4(SP)
BEQ	R2, R0, L___Home167
NOP	
J	L__Home71
NOP	
L___Home167:
;Kinematics.c,582 :: 		break;
J	L__Home56
NOP	
;Kinematics.c,583 :: 		}
L__Home71:
;Kinematics.c,584 :: 		if(!(Get_Axis_Run_States() & axis)){
SH	R25, 4(SP)
JAL	_Get_Axis_Run_States+0
NOP	
LH	R25, 4(SP)
AND	R2, R2, R25
BEQ	R2, R0, L___Home168
NOP	
J	L__Home72
NOP	
L___Home168:
;Kinematics.c,585 :: 		Home_Axis(-(max_sizes[axis]+100.0),speed,axis);
LWC1	S0, Offset(_Home_speed_L0+0)(GP)
CVT32.W 	S2, S0
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(_max_sizes+0)
ORI	R2, R2, lo_addr(_max_sizes+0)
ADDU	R2, R2, R3
LWC1	S1, 0(R2)
LUI	R2, 17096
ORI	R2, R2, 0
MTC1	R2, S0
ADD.S 	S1, S1, S0
MOVZ	R2, R0, R0
MTC1	R2, S0
SUB.S 	S0, S0, S1
SH	R25, 4(SP)
MOV.S 	S13, S2
MOV.S 	S12, S0
JAL	Kinematics_Home_Axis+0
NOP	
LH	R25, 4(SP)
;Kinematics.c,586 :: 		homing[axis].home_state = HOME_BACK;
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R3, R2, R3
ORI	R2, R0, 4
SH	R2, 0(R3)
;Kinematics.c,587 :: 		Home_Axis(-20.0,settings.homing_feed_rate, axis);
LUI	R2, 49568
ORI	R2, R2, 0
SH	R25, 4(SP)
LWC1	S13, Offset(_settings+24)(GP)
MTC1	R2, S12
JAL	Kinematics_Home_Axis+0
NOP	
LH	R25, 4(SP)
;Kinematics.c,589 :: 		while(DMA_IsOn(1));
L__Home73:
SH	R25, 4(SP)
ORI	R25, R0, 1
JAL	_DMA_IsOn+0
NOP	
LH	R25, 4(SP)
BNE	R2, R0, L___Home170
NOP	
J	L__Home74
NOP	
L___Home170:
J	L__Home73
NOP	
L__Home74:
;Kinematics.c,590 :: 		dma_printf("\n%s\n"
ORI	R30, R0, 10
SB	R30, 26(SP)
ORI	R30, R0, 37
SB	R30, 27(SP)
ORI	R30, R0, 115
SB	R30, 28(SP)
ORI	R30, R0, 10
SB	R30, 29(SP)
MOVZ	R30, R0, R0
SB	R30, 30(SP)
ADDIU	R3, SP, 26
;Kinematics.c,591 :: 		,"GO BACK HOME");
LUI	R2, hi_addr(?lstr_13_Kinematics+0)
ORI	R2, R2, lo_addr(?lstr_13_Kinematics+0)
SH	R25, 4(SP)
ADDIU	SP, SP, -8
SW	R2, 4(SP)
;Kinematics.c,590 :: 		dma_printf("\n%s\n"
SW	R3, 0(SP)
;Kinematics.c,591 :: 		,"GO BACK HOME");
JAL	_dma_printf+0
NOP	
ADDIU	SP, SP, 8
LH	R25, 4(SP)
;Kinematics.c,593 :: 		}
L__Home72:
;Kinematics.c,595 :: 		break;
J	L__Home56
NOP	
;Kinematics.c,596 :: 		case HOME_BACK: //Back to home slowly
L__Home75:
;Kinematics.c,607 :: 		if(!Test_Port_Pins(axis)){
SH	R25, 4(SP)
JAL	_Test_Port_Pins+0
NOP	
LH	R25, 4(SP)
BEQ	R2, R0, L___Home171
NOP	
J	L__Home76
NOP	
L___Home171:
;Kinematics.c,609 :: 		homing[axis].home_state = HOME_BACK_OFF;
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R3, R2, R3
ORI	R2, R0, 3
SH	R2, 0(R3)
;Kinematics.c,614 :: 		}
L__Home78:
;Kinematics.c,615 :: 		}
L__Home76:
;Kinematics.c,617 :: 		break;
J	L__Home56
NOP	
;Kinematics.c,618 :: 		case HOME_COMPLETE: //Home Complete
L__Home79:
;Kinematics.c,620 :: 		StopAxis(axis);
SH	R25, 4(SP)
JAL	_StopAxis+0
NOP	
LH	R25, 4(SP)
;Kinematics.c,624 :: 		axis++;
ADDIU	R2, R25, 1
SEH	R25, R2
;Kinematics.c,627 :: 		sys.state = STATE_IDLE;
SH	R0, Offset(_sys+2)(GP)
;Kinematics.c,630 :: 		while(DMA_IsOn(1));
L__Home80:
SH	R25, 4(SP)
ORI	R25, R0, 1
JAL	_DMA_IsOn+0
NOP	
LH	R25, 4(SP)
BNE	R2, R0, L___Home173
NOP	
J	L__Home81
NOP	
L___Home173:
J	L__Home80
NOP	
L__Home81:
;Kinematics.c,631 :: 		dma_printf("\n%s\t%d\n"
ADDIU	R23, SP, 31
ADDIU	R22, R23, 8
LUI	R24, hi_addr(?ICS?lstr14_Kinematics+0)
ORI	R24, R24, lo_addr(?ICS?lstr14_Kinematics+0)
JAL	___CC2DW+0
NOP	
ADDIU	R3, SP, 31
;Kinematics.c,632 :: 		,"COMPLETE",axis);
SH	R25, 4(SP)
ADDIU	SP, SP, -12
SH	R25, 8(SP)
LUI	R2, hi_addr(?lstr_15_Kinematics+0)
ORI	R2, R2, lo_addr(?lstr_15_Kinematics+0)
SW	R2, 4(SP)
;Kinematics.c,631 :: 		dma_printf("\n%s\t%d\n"
SW	R3, 0(SP)
;Kinematics.c,632 :: 		,"COMPLETE",axis);
JAL	_dma_printf+0
NOP	
ADDIU	SP, SP, 12
LH	R25, 4(SP)
;Kinematics.c,634 :: 		break;
J	L__Home56
NOP	
;Kinematics.c,635 :: 		case HOME_BACK_OFF:
L__Home82:
;Kinematics.c,637 :: 		if(!Test_Port_Pins(axis) && !one_shot_local){
SH	R25, 4(SP)
JAL	_Test_Port_Pins+0
NOP	
LH	R25, 4(SP)
BEQ	R2, R0, L___Home174
NOP	
J	L___Home105
NOP	
L___Home174:
LBU	R2, Offset(_Home_one_shot_local_L0+0)(GP)
EXT	R2, R2, BitPos(_Home_one_shot_local_L0+0), 1
BEQ	R2, R0, L___Home175
NOP	
J	L___Home104
NOP	
L___Home175:
L___Home103:
;Kinematics.c,638 :: 		one_shot_local = true;
LBU	R2, Offset(_Home_one_shot_local_L0+0)(GP)
ORI	R2, R2, BitMask(_Home_one_shot_local_L0+0)
SB	R2, Offset(_Home_one_shot_local_L0+0)(GP)
;Kinematics.c,640 :: 		Home_Axis(12.0,settings.homing_feed_rate, axis);
LUI	R2, 16704
ORI	R2, R2, 0
SH	R25, 4(SP)
LWC1	S13, Offset(_settings+24)(GP)
MTC1	R2, S12
JAL	Kinematics_Home_Axis+0
NOP	
LH	R25, 4(SP)
;Kinematics.c,641 :: 		}
J	L__Home86
NOP	
;Kinematics.c,637 :: 		if(!Test_Port_Pins(axis) && !one_shot_local){
L___Home105:
L___Home104:
;Kinematics.c,644 :: 		if(!(Get_Axis_Run_States() & axis)){
SH	R25, 4(SP)
JAL	_Get_Axis_Run_States+0
NOP	
LH	R25, 4(SP)
AND	R2, R2, R25
BEQ	R2, R0, L___Home176
NOP	
J	L__Home87
NOP	
L___Home176:
;Kinematics.c,645 :: 		homing[axis].home_state = HOME_COMPLETE;
SEH	R2, R25
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R3, R2, R3
ORI	R2, R0, 5
SH	R2, 0(R3)
;Kinematics.c,646 :: 		}
L__Home87:
;Kinematics.c,647 :: 		}
L__Home86:
;Kinematics.c,648 :: 		break;
J	L__Home56
NOP	
;Kinematics.c,649 :: 		}
L__Home55:
LW	R4, 40(SP)
LHU	R2, 0(R4)
ANDI	R2, R2, 65535
BNE	R2, R0, L___Home178
NOP	
J	L__Home57
NOP	
L___Home178:
LHU	R2, 0(R4)
ANDI	R3, R2, 65535
ORI	R2, R0, 2
BNE	R3, R2, L___Home180
NOP	
J	L__Home64
NOP	
L___Home180:
LHU	R2, 0(R4)
ANDI	R3, R2, 65535
ORI	R2, R0, 3
BNE	R3, R2, L___Home182
NOP	
J	L__Home70
NOP	
L___Home182:
LHU	R2, 0(R4)
ANDI	R3, R2, 65535
ORI	R2, R0, 4
BNE	R3, R2, L___Home184
NOP	
J	L__Home75
NOP	
L___Home184:
LHU	R2, 0(R4)
ANDI	R3, R2, 65535
ORI	R2, R0, 5
BNE	R3, R2, L___Home186
NOP	
J	L__Home79
NOP	
L___Home186:
LHU	R2, 0(R4)
ANDI	R3, R2, 65535
ORI	R2, R0, 3
BNE	R3, R2, L___Home188
NOP	
J	L__Home82
NOP	
L___Home188:
L__Home56:
;Kinematics.c,650 :: 		return axis;
SEH	R2, R25
;Kinematics.c,651 :: 		}
L_end__Home:
LW	RA, 0(SP)
ADDIU	SP, SP, 44
JR	RA
NOP	
; end of __Home
Kinematics_Home_Axis:
;Kinematics.c,655 :: 		static void Home_Axis(double distance,float speed,int axis){
ADDIU	SP, SP, -20
SW	RA, 0(SP)
;Kinematics.c,657 :: 		StopAxis(axis);
SH	R25, 4(SP)
SWC1	S13, 8(SP)
SWC1	S12, 12(SP)
JAL	_StopAxis+0
NOP	
LWC1	S12, 12(SP)
LWC1	S13, 8(SP)
LH	R25, 4(SP)
;Kinematics.c,658 :: 		STPS[axis].run_state = STOP ;
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 4
SH	R0, 0(R2)
;Kinematics.c,668 :: 		STPS[axis].mmToTravel = belt_steps(distance,axis);
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 56
SW	R2, 16(SP)
SH	R25, 4(SP)
SWC1	S13, 8(SP)
JAL	_belt_steps+0
NOP	
LWC1	S13, 8(SP)
LH	R25, 4(SP)
LW	R3, 16(SP)
SW	R2, 0(R3)
;Kinematics.c,670 :: 		speed = RPS_FROM_MMPMIN(speed);
LUI	R2, 16928
ORI	R2, R2, 0
MTC1	R2, S0
DIV.S 	S1, S13, S0
LUI	R2, 17008
ORI	R2, R2, 0
MTC1	R2, S0
DIV.S 	S0, S1, S0
MOV.S 	S13, S0
;Kinematics.c,672 :: 		speed = Get_Step_Rate(speed,axis);
SH	R25, 4(SP)
MOV.S 	S12, S0
JAL	_Get_Step_Rate+0
NOP	
LH	R25, 4(SP)
MOV.S 	S13, S0
;Kinematics.c,673 :: 		SingleAxisStep(STPS[axis].mmToTravel, speed,axis);
SEH	R3, R25
ORI	R2, R0, 68
MULTU	R2, R3
MFLO	R3
LUI	R2, hi_addr(_STPS+0)
ORI	R2, R2, lo_addr(_STPS+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 56
LW	R2, 0(R2)
MTC1	R2, S1
CVT32.W 	S1, S1
MOV.S 	S13, S0
MOV.S 	S12, S1
JAL	_SingleAxisStep+0
NOP	
;Kinematics.c,674 :: 		}
L_end_Home_Axis:
LW	RA, 0(SP)
ADDIU	SP, SP, 20
JR	RA
NOP	
; end of Kinematics_Home_Axis
Kinematics_ResetHoming:
;Kinematics.c,679 :: 		static void ResetHoming(){
;Kinematics.c,680 :: 		int i = 0;
;Kinematics.c,681 :: 		for(i = 0;i< NoOfAxis;i++){
; i start address is: 16 (R4)
MOVZ	R4, R0, R0
; i end address is: 16 (R4)
L_Kinematics_ResetHoming88:
; i start address is: 16 (R4)
SEH	R2, R4
SLTI	R2, R2, 4
BNE	R2, R0, L_Kinematics_ResetHoming191
NOP	
J	L_Kinematics_ResetHoming89
NOP	
L_Kinematics_ResetHoming191:
;Kinematics.c,682 :: 		homing[i].home_state = 0;
SEH	R2, R4
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R2, R2, R3
SH	R0, 0(R2)
;Kinematics.c,683 :: 		homing[i].home_cnt = 0;
SEH	R2, R4
SLL	R3, R2, 2
LUI	R2, hi_addr(Kinematics_homing+0)
ORI	R2, R2, lo_addr(Kinematics_homing+0)
ADDU	R2, R2, R3
ADDIU	R2, R2, 2
SH	R0, 0(R2)
;Kinematics.c,681 :: 		for(i = 0;i< NoOfAxis;i++){
ADDIU	R2, R4, 1
SEH	R4, R2
;Kinematics.c,684 :: 		}
; i end address is: 16 (R4)
J	L_Kinematics_ResetHoming88
NOP	
L_Kinematics_ResetHoming89:
;Kinematics.c,685 :: 		}
L_end_ResetHoming:
JR	RA
NOP	
; end of Kinematics_ResetHoming
_mc_reset:
;Kinematics.c,693 :: 		void mc_reset(){
ADDIU	SP, SP, -4
SW	RA, 0(SP)
;Kinematics.c,695 :: 		if (bit_isfalse(sys.execute, EXEC_RESET)) {
LH	R2, Offset(_sys+26)(GP)
ANDI	R2, R2, 16
SEH	R2, R2
BEQ	R2, R0, L__mc_reset193
NOP	
J	L_mc_reset91
NOP	
L__mc_reset193:
;Kinematics.c,696 :: 		sys.execute |= EXEC_RESET;
LH	R2, Offset(_sys+26)(GP)
ORI	R2, R2, 16
SH	R2, Offset(_sys+26)(GP)
;Kinematics.c,706 :: 		switch (sys.state) {
J	L_mc_reset92
NOP	
;Kinematics.c,707 :: 		case STATE_CYCLE: case STATE_HOLD: case STATE_HOMING: // case STATE_JOG:
L_mc_reset94:
L_mc_reset95:
L_mc_reset96:
;Kinematics.c,708 :: 		sys.execute |= EXEC_ALARM; // Execute alarm state.
LH	R2, Offset(_sys+26)(GP)
ORI	R2, R2, 32
SH	R2, Offset(_sys+26)(GP)
;Kinematics.c,709 :: 		disableOCx(); // Execute alarm force kills steppers. Position likely lost.
JAL	_disableOCx+0
NOP	
;Kinematics.c,710 :: 		DisableStepper();
JAL	_DisableStepper+0
NOP	
;Kinematics.c,711 :: 		ResetHoming();
JAL	Kinematics_ResetHoming+0
NOP	
;Kinematics.c,712 :: 		}
J	L_mc_reset93
NOP	
L_mc_reset92:
LH	R3, Offset(_sys+2)(GP)
ORI	R2, R0, 3
BNE	R3, R2, L__mc_reset195
NOP	
J	L_mc_reset94
NOP	
L__mc_reset195:
LH	R3, Offset(_sys+2)(GP)
ORI	R2, R0, 4
BNE	R3, R2, L__mc_reset197
NOP	
J	L_mc_reset95
NOP	
L__mc_reset197:
LH	R3, Offset(_sys+2)(GP)
ORI	R2, R0, 5
BNE	R3, R2, L__mc_reset199
NOP	
J	L_mc_reset96
NOP	
L__mc_reset199:
L_mc_reset93:
;Kinematics.c,713 :: 		}
L_mc_reset91:
;Kinematics.c,714 :: 		}
L_end_mc_reset:
LW	RA, 0(SP)
ADDIU	SP, SP, 4
JR	RA
NOP	
; end of _mc_reset
