...
; end of _DMA1_Enable
_DMA1_Disable:
;Serial_Dma.c,274 :: 		void DMA1_Disable(){
;Serial_Dma.c,275 :: 		DCH1CON |= 1<<7;
LW	R2, Offset(DCH1CON+0)(GP)
ORI	R2, R2, 128
SW	R2, Offset(DCH1CON+0)(GP)
;Serial_Dma.c,276 :: 		}
...
_DMA_Busy:
;Serial_Dma.c,282 :: 		int DMA_Busy(int channel){
;Serial_Dma.c,283 :: 		if(channel == 0)
SEH	R2, R25
BEQ	R2, R0, L__DMA_Busy68
NOP	
J	L_DMA_Busy9
NOP	
L__DMA_Busy68:
;Serial_Dma.c,284 :: 		return (DCH0CON & 0x8000)>>15;
LW	R2, Offset(DCH0CON+0)(GP)
ANDI	R2, R2, 32768
SRL	R2, R2, 15
J	L_end_DMA_Busy
NOP	
L_DMA_Busy9:
;Serial_Dma.c,286 :: 		return (DCH1CON & 0x8000)>>15;
LW	R2, Offset(DCH1CON+0)(GP)
ANDI	R2, R2, 32768
SRL	R2, R2, 15
;Serial_Dma.c,287 :: 		}
...
; end of _DMA_Busy
_DMA_CH1_ISR:
;Serial_Dma.c,295 :: 		void DMA_CH1_ISR() iv IVT_DMA1 ilevel 5 ics ICS_SRS {
RDPGPR	SP, SP
ADDIU	SP, SP, -12
MFC0	R30, 12, 2
SW	R30, 8(SP)
...
ORI	R30, R0, 5120
MTC0	R30, 12, 0
;Serial_Dma.c,298 :: 		dma1int_flag = DCH1INT & 0x00FF;
LW	R2, Offset(DCH1INT+0)(GP)
ANDI	R2, R2, 255
SB	R2, Offset(_dma1int_flag+0)(GP)
;Serial_Dma.c,300 :: 		if (DCH1INTbits.CHBCIF){
LBU	R2, Offset(DCH1INTbits+0)(GP)
EXT	R2, R2, 3, 1
BNE	R2, R0, L__DMA_CH1_ISR71
NOP	
J	L_DMA_CH1_ISR11
NOP	
L__DMA_CH1_ISR71:
;Serial_Dma.c,301 :: 		dma1int_flag = 1;
ORI	R2, R0, 1
SB	R2, Offset(_dma1int_flag+0)(GP)
;Serial_Dma.c,302 :: 		dma0int_flag = 0;
SB	R0, Offset(_dma0int_flag+0)(GP)
;Serial_Dma.c,304 :: 		}
L_DMA_CH1_ISR11:
;Serial_Dma.c,306 :: 		if( CHERIF_DCH1INT_bit == 1){
_LX	
EXT	R2, R2, BitPos(CHERIF_DCH1INT_bit+0), 1
BNE	R2, 1, L__DMA_CH1_ISR73
NOP	
J	L_DMA_CH1_ISR12
NOP	
L__DMA_CH1_ISR73:
;Serial_Dma.c,307 :: 		CABORT_DCH1ECON_bit = 1;
LUI	R2, BitMask(CABORT_DCH1ECON_bit+0)
ORI	R2, R2, BitMask(CABORT_DCH1ECON_bit+0)
_SX	
;Serial_Dma.c,308 :: 		}
L_DMA_CH1_ISR12:
;Serial_Dma.c,313 :: 		DCH1INTCLR  = 0x00FF;
ORI	R2, R0, 255
SW	R2, Offset(DCH1INTCLR+0)(GP)
;Serial_Dma.c,314 :: 		IFS4CLR     = 0x80;
ORI	R2, R0, 128
SW	R2, Offset(IFS4CLR+0)(GP)
;Serial_Dma.c,316 :: 		}
L_end_DMA_CH1_ISR:
...
; end of _DMA_CH1_ISR
_dma_printf:
;Serial_Dma.c,321 :: 		int dma_printf(const char* str,...){
ADDIU	SP, SP, -260
SW	RA, 0(SP)
;Serial_Dma.c,324 :: 		int i = 0, j = 0;
SW	R25, 4(SP)
SW	R26, 8(SP)
SW	R27, 12(SP)
;Serial_Dma.c,325 :: 		char buff[200]={0},tmp[20],tmp1[9];
ADDIU	R23, SP, 56
ADDIU	R22, R23, 200
...
JAL	___CC2DW+0
NOP	
;Serial_Dma.c,329 :: 		if(str == 0)
LW	R2, 260(SP)
BEQ	R2, R0, L__dma_printf75
NOP	
J	L_dma_printf13
NOP	
L__dma_printf75:
;Serial_Dma.c,330 :: 		return;
J	L_end_dma_printf
NOP	
L_dma_printf13:
;Serial_Dma.c,335 :: 		if(DMA_Busy(1)){
ORI	R25, R0, 1
JAL	_DMA_Busy+0
NOP	
BNE	R2, R0, L__dma_printf77
NOP	
J	L_dma_printf14
NOP	
L__dma_printf77:
;Serial_Dma.c,336 :: 		return 0;
MOVZ	R2, R0, R0
J	L_end_dma_printf
NOP	
;Serial_Dma.c,337 :: 		}
L_dma_printf14:
;Serial_Dma.c,341 :: 		va_start(va,str);
ADDIU	R3, SP, 16
ADDIU	R2, SP, 260
ADDIU	R2, R2, 4
SW	R2, 0(R3)
;Serial_Dma.c,343 :: 		i = j = 0;
...
;Serial_Dma.c,344 :: 		while(*(str+i) != '\0'){
L_dma_printf15:
; i start address is: 20 (R5)
; j start address is: 48 (R12)
SEH	R3, R5
...
LBU	R2, 0(R2)
ANDI	R2, R2, 255
BNE	R2, R0, L__dma_printf79
NOP	
J	L_dma_printf16
NOP	
L__dma_printf79:
;Serial_Dma.c,345 :: 		if(*(str+i) == '%'){
SEH	R3, R5
LW	R2, 260(SP)
ADDU	R2, R2, R3
LBU	R2, 0(R2)
ANDI	R3, R2, 255
ORI	R2, R0, 37
BEQ	R3, R2, L__dma_printf80
NOP	
J	L_dma_printf17
NOP	
L__dma_printf80:
;Serial_Dma.c,346 :: 		i++;  //step over % char
ADDIU	R2, R5, 1
; i end address is: 20 (R5)
; i start address is: 44 (R11)
SEH	R11, R2
;Serial_Dma.c,347 :: 		switch(*(str+i)){
SEH	R3, R2
LW	R2, 260(SP)
ADDU	R2, R2, R3
SW	R2, 256(SP)
J	L_dma_printf18
NOP	
;Serial_Dma.c,348 :: 		case 'c':
L_dma_printf20:
;Serial_Dma.c,350 :: 		buff[j] = (char)va_arg(va,char);
ADDIU	R3, SP, 56
SEH	R2, R12
ADDU	R5, R3, R2
ADDIU	R4, SP, 16
...
;Serial_Dma.c,352 :: 		break;
SEH	R12, R2
; j end address is: 8 (R2)
J	L_dma_printf19
NOP	
;Serial_Dma.c,353 :: 		case 'd':
L_dma_printf21:
;Serial_Dma.c,355 :: 		sprintf(tmp1,"%d",va_arg(va,int));
; j start address is: 48 (R12)
ADDIU	R4, SP, 16
LW	R3, 0(R4)
ADDIU	R2, R3, 4
...
NOP	
ADDIU	SP, SP, 12
;Serial_Dma.c,356 :: 		strcat(buff+j, tmp1);
ADDIU	R4, SP, 40
ADDIU	R3, SP, 56
SEH	R2, R12
ADDU	R2, R3, R2
...
JAL	_strcat+0
NOP	
;Serial_Dma.c,357 :: 		j += strlen(tmp1);
ADDIU	R2, SP, 40
MOVZ	R25, R2, R0
JAL	_strlen+0
NOP	
...
; j end address is: 48 (R12)
; j start address is: 8 (R2)
;Serial_Dma.c,358 :: 		break;
SEH	R12, R2
; j end address is: 8 (R2)
J	L_dma_printf19
NOP	
;Serial_Dma.c,359 :: 		case 'u':
L_dma_printf22:
;Serial_Dma.c,361 :: 		sprintf(tmp1,"%u",va_arg(va,unsigned int));
; j start address is: 48 (R12)
ADDIU	R4, SP, 16
LW	R3, 0(R4)
ADDIU	R2, R3, 4
...
NOP	
ADDIU	SP, SP, 12
;Serial_Dma.c,362 :: 		strcat(buff+j, tmp1);
ADDIU	R4, SP, 40
ADDIU	R3, SP, 56
SEH	R2, R12
ADDU	R2, R3, R2
...
JAL	_strcat+0
NOP	
;Serial_Dma.c,363 :: 		j += strlen(tmp1);
ADDIU	R2, SP, 40
MOVZ	R25, R2, R0
JAL	_strlen+0
NOP	
...
;Serial_Dma.c,364 :: 		case 'l':
L_dma_printf23:
;Serial_Dma.c,366 :: 		sprintf(tmp,"%ld",va_arg(va,long));
; j start address is: 48 (R12)
ADDIU	R4, SP, 16
LW	R3, 0(R4)
ADDIU	R2, R3, 4
...
NOP	
ADDIU	SP, SP, 12
;Serial_Dma.c,368 :: 		strcat(buff+j, tmp);
ADDIU	R4, SP, 20
ADDIU	R3, SP, 56
SEH	R2, R12
ADDU	R2, R3, R2
...
; j end address is: 48 (R12)
; j start address is: 8 (R2)
;Serial_Dma.c,370 :: 		break;
SEH	R12, R2
; j end address is: 8 (R2)
J	L_dma_printf19
NOP	
;Serial_Dma.c,371 :: 		case 'X':
L_dma_printf24:
;Serial_Dma.c,373 :: 		sprintf(tmp,"%X",va_arg(va,int));
; j start address is: 48 (R12)
ADDIU	R4, SP, 16
LW	R3, 0(R4)
ADDIU	R2, R3, 4
...
NOP	
ADDIU	SP, SP, 12
;Serial_Dma.c,374 :: 		strcat(buff+j, tmp);
ADDIU	R4, SP, 20
ADDIU	R3, SP, 56
SEH	R2, R12
ADDU	R2, R3, R2
...
; j end address is: 48 (R12)
; j start address is: 8 (R2)
;Serial_Dma.c,376 :: 		break;
SEH	R12, R2
; j end address is: 8 (R2)
J	L_dma_printf19
NOP	
;Serial_Dma.c,377 :: 		case 'X':
L_dma_printf25:
;Serial_Dma.c,379 :: 		sprintf(tmp,"%lX",va_arg(va,long));
; j start address is: 48 (R12)
ADDIU	R4, SP, 16
LW	R3, 0(R4)
ADDIU	R2, R3, 4
...
NOP	
ADDIU	SP, SP, 12
;Serial_Dma.c,380 :: 		strcat(buff+j, tmp);
ADDIU	R4, SP, 20
ADDIU	R3, SP, 56
SEH	R2, R12
ADDU	R2, R3, R2
...
; j end address is: 48 (R12)
; j start address is: 8 (R2)
;Serial_Dma.c,382 :: 		break;
SEH	R12, R2
; j end address is: 8 (R2)
J	L_dma_printf19
NOP	
;Serial_Dma.c,383 :: 		case 'f':
L_dma_printf26:
;Serial_Dma.c,384 :: 		sprintf(tmp,"%f",va_arg(va,float));
; j start address is: 48 (R12)
ADDIU	R4, SP, 16
LW	R3, 0(R4)
ADDIU	R2, R3, 4
...
NOP	
ADDIU	SP, SP, 12
;Serial_Dma.c,385 :: 		strcat(buff+j, tmp);
ADDIU	R4, SP, 20
ADDIU	R3, SP, 56
SEH	R2, R12
ADDU	R2, R3, R2
...
; j end address is: 48 (R12)
; j start address is: 8 (R2)
;Serial_Dma.c,387 :: 		break;
SEH	R12, R2
; j end address is: 8 (R2)
J	L_dma_printf19
NOP	
;Serial_Dma.c,388 :: 		case 'F':
L_dma_printf27:
;Serial_Dma.c,389 :: 		sprintf(tmp,"%E",va_arg(va,double));
; j start address is: 48 (R12)
ADDIU	R4, SP, 16
LW	R3, 0(R4)
ADDIU	R2, R3, 4
...
NOP	
ADDIU	SP, SP, 12
;Serial_Dma.c,390 :: 		strcat(buff+j, tmp);
ADDIU	R4, SP, 20
ADDIU	R3, SP, 56
SEH	R2, R12
ADDU	R2, R3, R2
...
; j end address is: 48 (R12)
; j start address is: 8 (R2)
;Serial_Dma.c,392 :: 		break;
SEH	R12, R2
; j end address is: 8 (R2)
J	L_dma_printf19
NOP	
;Serial_Dma.c,393 :: 		case 'p':
L_dma_printf28:
;Serial_Dma.c,394 :: 		sprintf(tmp,"%p",va_arg(va,void*));
; j start address is: 48 (R12)
ADDIU	R4, SP, 16
LW	R3, 0(R4)
ADDIU	R2, R3, 4
...
NOP	
ADDIU	SP, SP, 12
;Serial_Dma.c,395 :: 		strcat(buff+j, tmp);
ADDIU	R4, SP, 20
ADDIU	R3, SP, 56
SEH	R2, R12
ADDU	R2, R3, R2
...
; j end address is: 48 (R12)
; j start address is: 8 (R2)
;Serial_Dma.c,397 :: 		break;
SEH	R12, R2
; j end address is: 8 (R2)
J	L_dma_printf19
NOP	
;Serial_Dma.c,398 :: 		case 's':
L_dma_printf29:
;Serial_Dma.c,400 :: 		str_arg = va_arg( va, char* );
; j start address is: 48 (R12)
ADDIU	R4, SP, 16
LW	R3, 0(R4)
ADDIU	R2, R3, 4
...
LW	R4, 0(R3)
SW	R4, 52(SP)
;Serial_Dma.c,401 :: 		strcat(buff+j, str_arg);
ADDIU	R3, SP, 56
SEH	R2, R12
ADDU	R2, R3, R2
MOVZ	R26, R4, R0
...
JAL	_strcat+0
NOP	
;Serial_Dma.c,402 :: 		j += strlen(str_arg);
LW	R25, 52(SP)
JAL	_strlen+0
NOP	
ADDU	R2, R12, R2
...
NOP	
L_dma_printf16:
;Serial_Dma.c,411 :: 		*(buff+j+1) = 0;
ADDIU	R3, SP, 56
SEH	R2, R12
ADDU	R2, R3, R2
ADDIU	R2, R2, 1
SB	R0, 0(R2)
;Serial_Dma.c,412 :: 		strncpy(txBuf,buff,j+1);
ADDIU	R2, R12, 1
SEH	R27, R2
MOVZ	R26, R3, R0
LUI	R25, 40960
...
JAL	_strncpy+0
NOP	
;Serial_Dma.c,413 :: 		DCH1SSIZ    = j ;
SEH	R2, R12
SW	R2, Offset(DCH1SSIZ+0)(GP)
;Serial_Dma.c,414 :: 		DMA1_Enable();
JAL	_DMA1_Enable+0
NOP	
;Serial_Dma.c,415 :: 		return j;
SEH	R2, R12
; j end address is: 48 (R12)
;Serial_Dma.c,417 :: 		}
;Serial_Dma.c,415 :: 		return j;
...
; end of _dma_printf
_lTrim:
;Serial_Dma.c,421 :: 		void lTrim(char *d,char* s){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Serial_Dma.c,423 :: 		int i=0,j,k;
; i start address is: 20 (R5)
MOVZ	R5, R0, R0
;Serial_Dma.c,424 :: 		k = i;
; k start address is: 24 (R6)
SEH	R6, R5
;Serial_Dma.c,425 :: 		j = strlen(s);
SW	R25, 4(SP)
MOVZ	R25, R26, R0
JAL	_strlen+0
NOP	
...
NOP	
L__lTrim103:
;Serial_Dma.c,427 :: 		if((*s > 0x30)||(k>0)){
LBU	R2, 0(R26)
ANDI	R2, R2, 255
SLTIU	R2, R2, 49
...
L__lTrim42:
L__lTrim41:
;Serial_Dma.c,428 :: 		k = 1;
; k start address is: 12 (R3)
ORI	R3, R0, 1
;Serial_Dma.c,429 :: 		*d = *s;
LBU	R2, 0(R26)
SB	R2, 0(R25)
;Serial_Dma.c,430 :: 		d++;
ADDIU	R2, R25, 1
MOVZ	R25, R2, R0
;Serial_Dma.c,431 :: 		}else
J	L_lTrim36
NOP	
L_lTrim35:
;Serial_Dma.c,432 :: 		i++;
...
NOP	
L_lTrim32:
;Serial_Dma.c,435 :: 		if(i == j){
SEH	R3, R5
; i end address is: 20 (R5)
SEH	R2, R4
; j end address is: 16 (R4)
BEQ	R3, R2, L__lTrim106
NOP	
J	L_lTrim37
NOP	
L__lTrim106:
;Serial_Dma.c,436 :: 		*d = '0';
ORI	R2, R0, 48
SB	R2, 0(R25)
;Serial_Dma.c,437 :: 		d++;
ADDIU	R2, R25, 1
