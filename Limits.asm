...
;Limits.c,64 :: 		static void Y_Min_Limit_Setup(){
;Limits.c,73 :: 		IPC3SET = 11 << 8;
ORI	R2, R0, 2816
SW	R2, Offset(IPC3SET+0)(GP)
;Limits.c,76 :: 		IEC0SET = 1 << 13;
ORI	R2, R0, 8192
SW	R2, Offset(IEC0SET+0)(GP)
;Limits.c,78 :: 		IFS0CLR = (1 << 13);
ORI	R2, R0, 8192
SW	R2, Offset(IFS0CLR+0)(GP)
;Limits.c,80 :: 		}
...
;Limits.c,84 :: 		static void Z_Min_Limit_Setup(){
;Limits.c,94 :: 		IPC5SET = 11 << 24;
LUI	R2, 2816
SW	R2, Offset(IPC5SET+0)(GP)
;Limits.c,97 :: 		IEC0SET = 1 << 23;
LUI	R2, 128
SW	R2, Offset(IEC0SET+0)(GP)
;Limits.c,99 :: 		IFS0CLR = (1 << 23);
LUI	R2, 128
SW	R2, Offset(IFS0CLR+0)(GP)
;Limits.c,100 :: 		}
...
_X_Min_Limit:
;Limits.c,107 :: 		void X_Min_Limit() iv IVT_EXTERNAL_1 ilevel 4 ics ICS_AUTO {
RDPGPR	SP, SP
ADDIU	SP, SP, -16
SW	R30, 12(SP)
MFC0	R30, 12, 2
...
; end of _X_Min_Limit
_Y_Min_Limit:
;Limits.c,115 :: 		void Y_Min_Limit() iv IVT_EXTERNAL_2 ilevel 4 ics ICS_AUTO {
RDPGPR	SP, SP
ADDIU	SP, SP, -16
SW	R30, 12(SP)
MFC0	R30, 12, 2
...
; end of _Y_Min_Limit
_Z_Min_Limit:
;Limits.c,122 :: 		void Z_Min_Limit() iv IVT_EXTERNAL_4 ilevel 4 ics ICS_AUTO {
RDPGPR	SP, SP
ADDIU	SP, SP, -16
SW	R30, 12(SP)
MFC0	R30, 12, 2
...
; end of _Z_Min_Limit
_Test_Min:
;Limits.c,134 :: 		char Test_Min(int axis){
;Limits.c,135 :: 		return (Limit[axis].Limit_Min & 0x01)? 1:0;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
; end of _Test_Min
_Reset_Min_Limit:
;Limits.c,142 :: 		void Reset_Min_Limit(int axis){
;Limits.c,143 :: 		Limit[axis].Limit_Min = false;//INV ^ Limit[axis].Limit_Min;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
_XOR_Min_Limit:
;Limits.c,146 :: 		void XOR_Min_Limit(int axis){
;Limits.c,147 :: 		Limit[axis].Limit_Min = INV ^ Limit[axis].Limit_Min;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
_Invert_Min_Limit:
;Limits.c,150 :: 		void Invert_Min_Limit(int axis){
;Limits.c,151 :: 		Limit[axis].Limit_Min = (~Limit[axis].Limit_Min) & 0x01;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
_Set_Min_Limit:
;Limits.c,154 :: 		void Set_Min_Limit(int axis){
;Limits.c,156 :: 		if(!Limit[axis].Limit_Min)
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
NOP	
L__Set_Min_Limit51:
;Limits.c,157 :: 		Limit[axis].Limit_Min = true;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
;Limits.c,164 :: 		static void Reset_Min_Debounce(int axis){
;Limits.c,165 :: 		Limit[axis].Min_DeBnc = 0;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
ADDIU	R2, R2, 4
SH	R0, 0(R2)
;Limits.c,166 :: 		Limit[axis].last_cnt_min = 0;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
_Debounce_Limits:
;Limits.c,171 :: 		void Debounce_Limits(int axis){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Limits.c,172 :: 		Limit[axis].T0 = (TMR.clock >> BASE_TMR)&1;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
INS	R2, R3, 3, 1
SB	R2, 0(R4)
;Limits.c,176 :: 		Limit[axis].Pin = Test_Port_Pins(axis);
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
INS	R2, R4, 0, 1
SB	R2, 0(R3)
;Limits.c,179 :: 		if((!Limit[axis].Pin)&&(Limit[axis].Limit_Min)){
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
;Limits.c,181 :: 		if(!Limit[axis].T0 && !Limit[axis].T2){
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
ORI	R2, R2, 32
SB	R2, 0(R3)
;Limits.c,183 :: 		Limit[axis].Min_DeBnc++;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
ADDIU	R2, R2, 1
SH	R2, 0(R3)
;Limits.c,188 :: 		if(Limit[axis].Min_DeBnc > Limit[axis].last_cnt_min){
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
;Limits.c,189 :: 		Limit[axis].last_cnt_min = Limit[axis].Min_DeBnc;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
;Limits.c,190 :: 		}
L_Debounce_Limits9:
;Limits.c,191 :: 		}else if(Limit[axis].T0 && Limit[axis].T2)
...
;Limits.c,192 :: 		Limit[axis].T2 = 0;
L_Debounce_Limits10:
;Limits.c,195 :: 		if(Limit[axis].Min_DeBnc > DEBOUNCE_COUNT)
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
;Limits.c,196 :: 		Reset_Min_Limit(axis);
JAL	_Reset_Min_Limit+0
NOP	
L_Debounce_Limits14:
;Limits.c,198 :: 		}else if(Limit[axis].Pin){
...
;Limits.c,199 :: 		Reset_Min_Debounce(axis);
...
_FP:
;Limits.c,210 :: 		char FP(int axis){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Limits.c,211 :: 		char tmp = 0;
;Limits.c,212 :: 		Limit[axis].new_val = Test_Min(axis) & 0x0001;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
LW	R2, 4(SP)
SB	R3, 0(R2)
;Limits.c,213 :: 		if(Limit[axis].new_val > Limit[axis].old_Pval){
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
;Limits.c,217 :: 		}
L_FP18:
;Limits.c,218 :: 		Limit[axis].old_Pval = Limit[axis].new_val;
; tmp start address is: 16 (R4)
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
...
; end of _FP
_Rst_FP:
;Limits.c,223 :: 		void Rst_FP(int axis){
;Limits.c,224 :: 		Limit[axis].old_Pval = false;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
; end of _Rst_FP
_FN:
;Limits.c,228 :: 		char FN(int axis){
ADDIU	SP, SP, -8
SW	RA, 0(SP)
;Limits.c,229 :: 		char tmp = 0;
;Limits.c,230 :: 		Limit[axis].new_val = Test_Min(axis) & 0x01;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
LW	R2, 4(SP)
SB	R3, 0(R2)
;Limits.c,231 :: 		if(Limit[axis].new_val < Limit[axis].old_Fval){
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
; tmp end address is: 16 (R4)
L_FN20:
;Limits.c,235 :: 		Limit[axis].old_Fval = Limit[axis].new_val;
; tmp start address is: 16 (R4)
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
...
; end of _FN
_Rst_FN:
;Limits.c,240 :: 		void Rst_FN(int axis){
;Limits.c,241 :: 		Limit[axis].old_Fval = true;
SEH	R3, R25
ORI	R2, R0, 12
MULTU	R2, R3
MFLO	R3
...
; end of _Rst_FN
_Test_Port_Pins:
;Limits.c,248 :: 		char Test_Port_Pins(int axis){
;Limits.c,249 :: 		char tmp = 0;
; tmp start address is: 16 (R4)
MOVZ	R4, R0, R0
;Limits.c,250 :: 		switch(axis){
J	L_Test_Port_Pins21
NOP	
; tmp end address is: 16 (R4)
;Limits.c,251 :: 		case X:
L_Test_Port_Pins23:
;Limits.c,252 :: 		tmp = X_Min_Limit & 0x0001;
_LX	
...
;Limits.c,254 :: 		case Y:
L_Test_Port_Pins24:
;Limits.c,255 :: 		tmp = Y_Min_Limit & 0x0001;
_LX	
...
;Limits.c,257 :: 		case Z:
L_Test_Port_Pins25:
;Limits.c,258 :: 		tmp = Z_Min_Limit & 0x0001;
_LX	
